...
**手7 の Q6 に答えが出た。言語は障害にならなかった。**
英語の conventions header で、**3 周とも日本語の UI が出て、規約も守られた**
（`AppProviders` の配置・禁止語彙・`w-field-md` の採用）。
🟦 **交絡変数として立てておいた懸念は解消した**——Q2 が 🟦 だったので、言語を疑う必要が無くなった。

🟨 **§3（知識の結びつき）は 🟥 のまま閉じる。**本人の確認を取っていないので推測で埋めない。
**確定したのは「言語は効き目に影響しなかった」という観測だけ。**
...
flowchart TB
  ...
  subgraph A["🤖 design agent が読む面（見えない）"]
    HEADER["README.md conventions header<br/>→ system prompt に inline"]
...
キャプション: **この観点が問題にしているのは左下の `HEADER` 1 箇所。**
