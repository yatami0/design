...
> ★★★ 🆕 🟥 **2026-08-09（ユーザー指示）— 検査を止め、部品を作る側へ戻した。**
> **次のセッションは [工程5（稼働表・ピボット）](手順/工程5_稼働表_ピボット.md) の P5-01 から。**
> 🟦 **確認待ちは 1 件も無い**（手順書を更新済み・D7 は決着済み・§1 のベースラインは現行）。
>
...
### 🆕 部品6 完了時のベースライン（2026-08-09・**現行**・ブランチ `geneva`）
...
> ★★★ 🆕 **2026-08-09 更新（ユーザー指示）: 検査（部品軸）を止め、工程5 から部品を作る。**
>
> > 「一旦検査を止めてコンポーネントを作るように手順を直して、次のセッションはそこからやるから」（逐語）
>
> 🟥 **理由は実測。直近 4 回で部品が 1 件も増えていない**（部品1 +0 ／ 部品2 **+5** ／ 部品3 **+4** ／ 部品4・5・6 **+0**）。
> **検査の回は自分で次の宿題（OBS）を産み、handoff の「次」が毎回それを指していた**＝ **軸が自走していた。**
> 🟦 **部品の需要は題材の 5 画面から出る**（[DR-0078](DR/DR-0078-repo-becomes-a-ui-factory-for-a-core-design-system.md)）のに、
> 🟥 **5 画面のうち 3 画面（稼働表・EVM・ガント）が未着手**のままだった。→ [段取り §3b](工場の段取り.md) に中断を記録。
>
> ### 🟥 次のセッションはここから
>
> | # | やること | 誰がやるか |
> | --- | --- | --- |
> | **1** | ★★★ 🟥 **[工程5（稼働表・ピボット）](手順/工程5_稼働表_ピボット.md) を P5-01 から実行する。**🟦 **確認待ちは 1 件も無い**——**D7（addon-vitest）は部品3 D7=A で決着済み**（既に入っていてゲート 7 本目）なので **P5-02 は削除済み**。**§1 のベースラインは現行（`0bd09e7`）に更新済み**。🟥 **本体の問いは Q1「`DataGrid` を拡張するか `PivotTable` を新設するか」**——**まず `DataGrid` で組み、書けなかった項目を列挙してから決める**（D1=B） | **Claude** |
> | **2** | 🟨 **新部品を作ったら [完成バー](部品の完成バー.md) を通す。**面①・面② は `pnpm test-storybook` で**自動**、🟥 **面④（語彙の効果）は story に書かないと掛からない**（濃淡＝ D2 がまさにその対象）。締めに `pnpm coverage` で**被覆が下がっていないこと**を見る（部品6 D9） | Claude |
> | **3** | 🟥 ★ **部品6 の [PR #20](https://github.com/yatami0/design/pull/20) をマージする**（🟨 **工程5 をブロックしない**——先に進んでよい） | 🟥 **人** |
> | **4** | 🟨 **工程5 の次は [工程6（チャート・ガントの素材源調査）](工場の段取り.md)**——**工程7（EVM）・工程8（ガント）はここで素材源が決まらないと実装できない**。🟥 **ガントの既製品は見た目を自分で持つので、[DR-0070](DR/DR-0070-product-layer-boundary-rule.md)（層の原理）と正面衝突する可能性がある** | Claude |
>
> ### ⏸ 部品軸（検査）— 中断。再開するならここから
>
> 🟨 **成果は残る**（完成バー ＋ ゲート外の計測器 4 本）。**積み残しは 3 件**——
> ① `self-check` の検体が 2 本だけ（`a11y-scan` を含む 6 本には検体が無い・部品6 D12=B）
> ② **面④ の借金 12 件**（[台帳 §5.5](部品の完成バー_台帳.md)） ③ **面③ の借金 33 件**
>
