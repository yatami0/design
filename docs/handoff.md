...
最終更新: 2026-08-07（🆕 **手8c の PR #3 がマージされた＝設計は確定した。**→ **手8d（実装）の手順書を起こした**。🟥 **§2 の D1〜D9 はユーザー判断待ちで、実装は 1 行も始めていない**）
...
- ★★ 🆕 **手8d（実装）の手順書を起こした**（[手8d_製品層の部品実装.md](手順/手8d_製品層の部品実装.md)）。**実装は 1 行も始めていない。**
  - 🟥 **§2 の D1〜D9 がユーザー判断待ち**（推奨は全件 §2.1〜§2.9 に根拠つきで書いた）。D1 が「実装をどの手に割り当てるか」＝ 下記「次にやること 0-2」の未決そのもの
  - ★ **手順書を書く途中で、設計が実装判断として残していた 1 件に答えが出た**——**D4（`font-emphasis` を語彙表に足すか、`text-emphasis` に畳むか）は A で決まり**。
    現物を数えたら **`Section.tsx` L23 が `text-heading font-emphasis`** を使っており、**サイズと weight は直交している**。畳むと見出しが 1 段小さくなる（＝見た目が変わり手5 の目視をやり直す羽目になる）
  - 🟥 **我々自身のコードが、生成物と同じ逸脱を書いていた**——`src/app/page.tsx` L57・L74 の cell に **`font-mono` / `font-mono tabular-nums`**。
    面③（6/6 周）と**同じ形**。→ 手8d の **Q3** は「設計が効くなら自分のコードから先に消える」を測る問いにした。
    🟨 ただし設計 §3.2 の `kind:'numeric'` は **`tabular-nums` しか書いていない**（`font-mono` の扱いは H8D-05 の判断＝§2 への追記待ち）
  - 🟦 **設計の記述と現物が一致していることを確認した**——conventions header の **L53**（`className="w-field-md"` を教えている）と **L86**（`ColumnDef[]` を名指し）は行番号まで設計のとおり
  - 🟨 **`DataGrid` のセル既定は半分実装済みだった**（`text-table` / `text-label` は既に当たっている）。足すのは `kind` / `emphasis` の 2 語だけ
  - 🟥 **Q1 の赤テストは本物の検体で打てない**——`.design-sync/` に生成物が無い（gitignore）。→ **D6 で「ローカル `storybook build` の CSS を代理検体にする」を推奨**し、7 周目で本物を再確認する形にした
...
| **手8d** | 🆕 **製品層の部品を実装する**（設計を 4 件のコードに落とし、機械ゲートまで通す）。🟥 **逸脱が消えたかは測らない**——それは 7 周目 | [手8d](手順/手8d_製品層の部品実装.md) | 🟨 **planned**（手順書のみ・2026-08-07）。🟥 **§2 の D1〜D9 がユーザー判断待ち。着手していない** |
...
✅ **手8c は完了し、PR #3 でマージされた＝設計は確定した**（2026-08-07）。
🆕 **次の一手は「手8d の §2 を決めること」。**手順書は書けているので、**決まれば実行に入れる。**

### 🟥 0. 手8d の着手判断（ユーザー判断 — ここが今のボトルネック）

**[手8d_製品層の部品実装.md](手順/手8d_製品層の部品実装.md) §2 の D1〜D9。**推奨は全件 §2.1〜§2.9 に根拠つきで書いた。
**★ 決めるべきは実質 3 件**（残り 6 件は設計の帰結で、推奨から動かす理由が見当たらない）:

| # | 決めること | 推奨と根拠 |
| --- | --- | --- |
| **D1** | **この実装をどの手に割り当てるか**（＝下記の旧「未決 2」） | **A: 新しい手 手8d。**手9 は移送（🟥 **実行するのは人**）で実行者が違う。手6 で「人の操作に依存して手が止まる」形を 1 度踏んでいる |
| **D6** | **赤テストの検体**（`_ds_bundle.css` が手元に無い） | **A: ローカル `storybook build` の CSS を代理検体にする**＋7 周目で本物を再確認。B（ユーザーが `/design-sync` を打つ）は人の操作が 1 回増える |
| **D8** | **conventions header をどこまで変えるか** | **A+B+C+D（全量 6 箇所）。**🟥 header を変えると狙っていない所が壊れる（[DR-0069](DR/DR-0069-adding-prohibitions-to-the-header-degraded-the-output.md)）ので、**禁止文は 1 文も足さない**形にしてある |

| # | 論点 | 状態 |
| --- | --- | --- |
| 1 | ~~**PR を出してマージする**~~ | ✅ **完了**（PR #3 `9801b66`・2026-08-07） |
| 2 | ~~**設計の実装をどの手に割り当てるか**~~ | 🔀 **手8d §2 の D1 へ移した**（手順書を A 前提で書いた提案として起こしてある） |
