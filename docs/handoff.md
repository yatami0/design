...
- ★★★ 🆕 **部品6 が完了した — 「130/130 緑」を面積で読み直したら、79% は何も主張していなかった**
  ...
  **足したのは道具 2 本（`coverage-scan` / `self-check`）と型の口 1 本（`tsconfig.tools.json`）。**
  ...
  - 🟥 **次: この PR のマージ（人）→ D10（ゲート 8 本目・9 本目にするか・ユーザー判断）**
...
> 🆕 **部品6 も done**（ブランチ `geneva`）。🟥 **次の一手はこの PR のマージ（人）。**
>
> | # | やること | 誰がやるか |
> | --- | --- | --- |
> | **A** | 🟥 ★★ **部品6 の [PR #20](https://github.com/yatami0/design/pull/20) をマージする** | 🟥 **人**（[DR-0068](DR/DR-0068-merge-through-pull-requests.md)） |
...
> | **C** | 🟨 **次の一手の候補は 3 本**——① 🟥 **`self-check` の検体を `a11y-scan` に広げる**（部品6 D12=B が外した・**実際に壊れた 2 例のうち残り 1 例がこちら側**。**Storybook のビルドを要求するので検体の作り方が別問題**） ② **面④ の借金 12 件**（台帳 §5.5・**部品1 D6=B「触るときに返す」なので単独で立てるかは判断が要る**） ③ **面③ の借金 33 件**（同上） | Claude（マージ後） |
