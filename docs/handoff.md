...
最終更新: 2026-08-01（**手5 完了・`main` へマージ済み**。**手6 の手順書を起草**。Q1〜Q8 に答えを出し、OBS の棚卸しを初回実施した）

---

## 現在地

- **手0〜手5 が完了**（すべて `main` へ `--no-ff` マージ済み）。
  ✅ **手5 のマージも済んだ**——`e88311a`（`step/h5-token-swap` を `--no-ff`・2026-08-01）。`main` と作業ツリーは clean。
  ✅ **手6 の手順書を起草した**（ブランチ `step/h6-preview-html`・`status: planned`）。
  🟥 **次の一手は手6 §2 の D1〜D7 を決着させること**（下記「次にやること」）。**D5（外向き操作の承認）が唯一の不可逆。**
...
| 手6 | **プレビュー HTML を作り** `/design-sync` で登録 → フラグ無しで部品を選べるか | [手6](手順/手6_プレビューHTMLとdesign-sync登録.md) | 🟨 **planned**（§2 の D1〜D7 が未決） |
...
### 🟥 1. 手6 §2 の D1〜D7 をユーザーと決着させる

✅ **手順書は書いた** → [手6_プレビューHTMLとdesign-sync登録.md](手順/手6_プレビューHTMLとdesign-sync登録.md)（ブランチ `step/h6-preview-html`）。
問い **Q1〜Q8** と判断ポイント **D1〜D7** が立っている。**着手の順序は「手順書を書く → 問いを確定させる → 実行する」**なので、次は D の決着。

| # | 論点 | 推奨 | なぜ先に決めるか |
| --- | --- | --- | --- |
| 🟥 **D5** | **登録先と外向き操作の承認** | B（実験用プロジェクトを新規作成）＋ 実行は都度承認 | **本 repo で初めての「外へ出す」操作。**🟥 **唯一の不可逆**——上げたものは消してもキャッシュ・索引が残りうる |
| 🟥 **D1** | **何を登録するか** | D（① 1 + 製品層 11 + ③ 2 + ④ 1 = **15**）を 1 周目 | handoff が「未定」と名指ししていた論点。素材層 16 を出すと**手3 D3=B の枠が境界の向こうで破れる** |
| **D2** | プレビュー HTML の作り方 | B（`storybook-static` を Playwright で吸い出す） | 手書きだと**正本が 2 つに割れる。**`visual-probe.mjs` の経路が再利用できる＝**同じ需要の 2 回目** |
| **D4** | フラグ 5 種の扱い（未決 #10） | E（素直に落として手7 で実害を測る → 辞書は Claude Code 側へ） | 推測で作り込むと**手7 の失敗原因が切り分けられない** |
| D3 / D6 / D7 | CSS の版／登録方式／skill 不在 | A（tmp-admin 適用後）／ A（`@dsCard`）／ A（ツール直叩き） | いずれも戻せる |

🆕 **手順書を書く過程で環境の穴が 2 つ出た**（下記「この環境で効いていないもの」）。

### 🟥 この環境で効いていないもの（2026-08-01 実測）

| 欠けているもの | 影響 |
| --- | --- |
| 🟥 **`trace` plugin が未インストール**（`~/.claude/plugins/repos/` に実体なし） | `/dr` `/obs` skill が使えない。**新規 md の frontmatter と採番を止める hook が動いていない**。`docs-meta.mjs --check` も打てない。→ 手6 の手順書は**語彙を手で `.claude/trace.config.mjs` と突き合わせた**（機械検査はしていない） |
| 🟨 **`/design-sync` skill が無い**（`~/.claude/skills/` が空） | `DesignSync` **ツール**は使えるので手6 は進む（D7=A）。→ **手6 の §1 に不確定要素として記録済み** |
...
### 3. 🟥 [OBS-0009](OBS/OBS-0009_不透明度と状態面の概念を理解する.md) の学習（他をブロックしている唯一の観点）
