...
>
> 1. 🟥 **工程3 の PR（ブランチ `p3-review-and-fixes`）を人がマージする**。🟨 [手順書 §2](手順/工程3_共通シェル.md) の **D1〜D15 は推奨どおりの Claude 判断（事後承認待ち・全件 git で戻せる）**——とくに **D12**（`allowBuilds: false` を commit＝未決 #27 ① の決着）・**D11/D15**（画面層への lint 3 本）は新しい機械化なので異議があれば言ってほしい
