...
- **手0〜手4 が完了**（すべて `main` へ `--no-ff` マージ済み）。
  ✅ **手5 も完了した**——ただし**ブランチ `step/h5-token-swap` に 18 コミット未マージ**。
...
**ブランチ `step/h5-token-swap` に 18 コミット。作業ツリー clean。`main` へは未マージ。**
...
- 🟥 **次セッションの最初の一手は `main` へ `--no-ff` マージすること（人が実行）。**
  ブランチ `step/h5-token-swap` に **19 コミット未マージ**、作業ツリー clean。
