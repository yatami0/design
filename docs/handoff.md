...
- 🆕 **手7 に着手した。**ブランチ `step/h7-design-agent-behavior`（`b7a97f3` から分岐）。
  **[手順書](手順/手7_ClaudeDesignに一覧を組ませる.md)は起草済み・`status: planned`。**🟥 **§2 の D1〜D7 が未決で、次の一手はここ。**
- ★ **手5 は Q1〜Q8 すべてに答えが出て、完了条件 10 件も検証済み**（[実行記録 §手5 の締め](実行記録.md)）。
- **決定 23 件・発見 36 件を [docs/DR/](DR/index.md) に切り出し済み**（DR-0001〜**0059**）。
...
### 🟥 1. 手7 §2 の D1〜D7 を決める（**ユーザー判断**）

**★ 最優先は D1。**「1 周目の依頼に**デザインシステム準拠を明示するか**」——
**ここを決めないと Q1（使うか作り直すか）の測定が成立しない。**

| # | 論点 | 推奨 |
| --- | --- | --- |
| **D1** | ★ 1 周目に「デザインシステム準拠で」と明示するか | **C**（明示なしで 1 周 → 明示ありで 2 周）。「使わない」と「言わないと使わない」を切り分けるため |
| **D2** | 何を・どの忠実度で組ませるか | **A**（チケット一覧 1 画面・High-fi のみ）。Low-fi は忠実度と準拠の 2 変数が同時に動く |
| **D3** | 生成物の持ち帰り経路 | **A**（`DesignSync` の読み取り）を試し、駄目なら **C**（手でコピー） |
| **D4** | ★ 「使った」の判定基準 | **B**（層ごとに変える・DR-0033）＋ 素の HTML 要素の比率。🟥 **後から変えると生成し直し** |
| **D5** | 素材層の不足が出たら 2 周目を回すか | **B**（記録だけして 1 周で閉じる）。足すと手3 D3=B が破れる |
| **D6** | 誰が何周打つか | **C**（人が打ち、Claude が生成物を読んで追加指示を作る）。🟥 **design agent は Claude 側から起動できない** |
| **D7** | 外向き操作の承認（新規デザインプロジェクト） | **A**（新規）。既存資産に触らない |
