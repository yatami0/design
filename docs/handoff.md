...
- ★ **手4 で製品層が実際に使えることが実証された。**素材層への直 import **0 件**／`Box` への逃げ **0 回**。
- **次は ★手5（トークン差し替え実験）。**🟥 **着手前に未決 #18（事前特定 15 件の数え直し）が要る。**
...
## 次にやること（手4）— ③ Patterns / Templates 層

**まず手順書を書く。**問いの立っていない手は実行しない（DR-0004）。

| # | やること | 根拠 |
|---|---|---|
| 1 | **手順書 `docs/手順/手4_*.md` を書く**（問いと判断ポイントを先に立てる） | DR-0004 |
| 2 | ダミーデータの作り方を決める（未決 #7・**仮置きは使い捨ての手書き**） | 契約の正本を割らないため |
| 3 | チケット一覧を**製品層の部品だけで**組む | D3=B が本当に成立するかの実証 |
| 4 | 🟨 **`Box` への逃げ回数を数える**（いまは 0 回） | 増えたら D11 の枠に穴がある（未決 #21） |
| 5 | 🟨 **Cmd/Ctrl+B の衝突が起きるか観測する** | 起きたら D6 を B へ切り替える 1 回目の証明（未決 #22） |
| 6 | `AppProviders` の中身を埋める | [DR-0037](DR/DR-0037-providers-belong-to-product-layer.md)（場所は手3 で確定済み） |
| 7 | 🟥 **未決 #18（手5 の事前特定 15 件）を数え直す** | [DR-0029](DR/DR-0029-component-token-overridable-outside-layer.md)・[DR-0034](DR/DR-0034-touch-target-visual-32-hit-44.md) で解けるものが混ざっている |

> **手3 で作った製品層が実際に使えるかは、手4 で画面を組んで初めて分かる。**
> D3=B は「一旦仮」で確定しているので、**ここで再評価する**。
