...
最終更新: 2026-07-26（**未決 #18 を解き、手5 の手順書を書いた**。事前特定 15 件は実測で **1 件**だった）
...
- **決定 17 件・発見 26 件を [docs/DR/](DR/index.md) に切り出し済み**（DR-0001〜0043）。
- 🆕 ★ **未決 #18 が閉じた（[DR-0043](DR/DR-0043-recount-of-fifteen-unchanged-spots.md)）。事前特定 15 件は実測で 1 件だった。**
  数字がずれていたのではなく、**1 つの箱に 3 種類の別物が入っていた**（甲 ① 層だけで動く 2 件／乙 素材層を触らず接続できる 6 件／
  丙 本当に変わらない **1 件**／対象外＝差し替え先が無い 6 件／削除 1 件）。→ **手5 の問いは 3 つに割り直した。**
- 🆕 ★ **手5 の手順書を書いた**（[手5](手順/手5_トークン差し替え実験.md)）。**判断ポイント D1〜D6 が未決**で、
  **D1（影 7 箇所・スクリム 2 箇所をレイヤ外上書きで解くか）が最優先**——Q2 の答えを左右するので実行前に決める。
...
- **次は ★手5（トークン差し替え実験）の実行。**🟥 **着手前に手順書 §2 の D1〜D6 をユーザーと決着させる。**
...
| 手5 | ★ トークン差し替え実験 | [手5](手順/手5_トークン差し替え実験.md) | 🟨 **手順書あり・§2 が未決**（次はここ） |
...
### ✅ 着手前の 2 件は片付いた（2026-07-26）

| # | やったこと | 結果 |
|---|---|---|
| ~~1~~ | ✅ **未決 #18 — 事前特定 15 件の数え直し** | **15 件ではなく 1 件だった**（[DR-0043](DR/DR-0043-recount-of-fifteen-unchanged-spots.md)）。実測 2 本が効いた: [DR-0041](DR/DR-0041-tailwind-v4-seams-differ-per-utility.md)（継ぎ目は utility ごとに違う）・[DR-0042](DR/DR-0042-layer-external-override-reaches-properties.md)（レイヤ外上書きはプロパティにも届く） |
| ~~2~~ | ✅ **手順書を書いた** | [手5](手順/手5_トークン差し替え実験.md)。**問いを Q1〜Q3 に割り直した**（元の問いは Q3 だけに残る） |

### 🟥 いま着手前に片付けること

**手順書 §2 の判断ポイント D1〜D6 が全部未決。**ユーザー判断が要る。

| # | 論点 | 効く先 |
|---|---|---|
| **D1** | 🟥 **最優先。**乙のうち「上書きでしか解けない」9 箇所（影 7・スクリム 2）をどう扱うか（A 全部上書き／B 変数向け替えで解けるものだけ／C 製品層ラッパー＝**不可・実測済み**） | **Q2 の答えを左右する。**実行中に変えると before/after が比較できなくなる |
| D2 | 差し替えの単位（`tokens.css` 直書き／別ファイルを import 1 行で切り替え） | 実験の可逆性 |
| D3 | 差し替えの向きを両方試すか | (B) 群 6 件の頭打ちを観測できるか |
| D4 | **未決 #6** — `preset` ごと差し替える軸を手5 に含めるか | 手5 の範囲 |
| D5 | 角丸の非線形 5 段（8/12/18/28）を段ごとに上書きするか＝語彙の追加 | トークンマッピング 2.3 |
| D6 | **未決 #16** — 色空間の差（本体 hex+lab ／ Storybook oklch）を明示するか | C（本体側を揃える）は不可逆 |

### 手5 の手順書に書いた前提（確定済み・決め直さない）
...
| 🆕 **判定 3 段の前に「継ぎ目があるか」を測る第 0 段が要る。**ソースの `font-medium` を見ても分からず、**生成 CSS を見ないと分からない** | [DR-0041](DR/DR-0041-tailwind-v4-seams-differ-per-utility.md) |
| 🆕 **レイヤ外の上書きはプロパティにも届くが、変異（`data-[size=sm]` / `focus-visible:`）を潰す。**「向け替え」と「上書き」を別物として記録する | [DR-0042](DR/DR-0042-layer-external-override-reaches-properties.md) |
| 🟥 **「対象 0 件で緑」の 5 例目が手4 で出た。**トークンを消してもビルドは緑になるので、**壊れたら赤くなる仕組みがあるか**を先に確かめる（H5-02 の赤テスト） | [OBS-0003](OBS/OBS-0003_対象0件で緑が5回出た.md) |
...
| 6 | `preset` ごと差し替える軸を手5 に含めるか（トークン差し替えとは別の軸） | → **[手5](手順/手5_トークン差し替え実験.md) §2 D4 に移した**（DR-0016） |
