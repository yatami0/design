...
| 手8 | 🟥 **問いが書き換わった**——「出力は lint / validate.mjs を通るか」→ **受け手の lint と我々の lint はどこで食い違うか** | [手8](手順/手8_出力は機械ゲートを通るか.md) | 🟨 **進行中**（2026-08-02）。★ **Q1〜Q5・Q7 に答え。**残るのは Q6（効き目測定＝4 周目）と Q8（要判断） |
...
### 🟥 1. 手8 の残り 2 件を片付けて締める

**§2 の D1〜D13 は全件決着済み**（D1〜D8 はユーザー判断、D9〜D13 は実行中に §2 へ追記してから決めた）。

| # | 残り | 誰がやるか |
| --- | --- | --- |
| **Q6** | 🟥 **header を直した効き目が未測定。**`.design-sync/conventions.md` の宛先無し参照 **4 件**（`.prompt.md` ×2 ／ `.d.ts` ／ guidelines）を削り、**`<style>` 生 CSS の禁止**と**「CSS に存在することは使ってよいことを意味しない」**（`tabular-nums` を名指し）を足した。**測るには 4 周目の同期と生成が要る** | 🟥 **人が `/design-sync` と `claude.ai/design` を打つ** |
| **Q8** | 🟨 **思想への指摘 11 件目の候補が出た（未起票・要判断）**——**思想は「定義したものだけを使う」（許可リスト）で語るが、機械の網は両側とも禁止リストしか持てない。**列挙し忘れた実在クラス（`tabular-nums`）は全部素通りする | ユーザー判断 |

🟨 **完了条件はほぼ埋まっている。**残るのは上の 2 件と、実行記録へのチェック。
