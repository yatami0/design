...
最終更新: 2026-07-26（**手3 完了**。素材層と製品層の境界を確定し、枠を props と lint で閉じた）

---

## 現在地

- **手0・手1・手2・手2b・手3 が完了。**すべて `main` へ `--no-ff` マージ済み、作業ツリー clean。
...
- **決定 17 件・発見 21 件を [docs/DR/](DR/index.md) に切り出し済み**（DR-0001〜0038）。
- ★ **手3 で「素材層 / 製品層 / 棚」の三層が揃った。**役割 9 カテゴリが **story の title 文字列からコードのディレクトリになった**。
- **次は手4（③ Patterns / Templates 層）。**まず手順書を書き、問いを立ててから実行する（DR-0004）。
...
| 手4 | ③ Patterns / Templates 層 + ダミーデータで一覧を組む | 未作成 | ⬜ **次はここ** |
| 手5 | ★ トークン差し替え実験 | 未作成 | ⬜ |
...
| ゲート | 手2 完了時 | 手2b 完了時 | **手3 完了時** | 備考 |
|---|---|---|---|---|
| `pnpm typecheck` | 🟥 赤 1 | 🟥 赤 1 | 🟦 **緑** | 🟥 **借金で緑になった。**手3 D5=A で `exactOptionalPropertyTypes` を false にしただけ（DR-0014） |
| `pnpm lint` | 🟥 赤 33 | 🟥 赤 33 | 🟥 **赤 33** | 内訳も同一（任意値 24 / 型系 9）＝ **手3 の新しい赤ゼロ**。🟦 **製品層とアプリ層の赤は 0 件**（残り 33 は全部素材層） |
| `pnpm build` | 🟥 赤 | 🟥 赤 | 🟦 **緑** | typecheck と同一原因＝**同じ借金** |
| `pnpm format:check` | 🟦 緑 | 🟦 緑 | 🟦 **緑** | shadcn 出力は `.prettierignore`（DR-0007） |
| `pnpm spell` | 🟦 緑 | 🟦 緑 | 🟦 **緑** | 第 3 セッションで固有名詞 6 語を追加（下記の陳腐化の件） |
| `pnpm build-storybook` | 🟦 緑 | 🟦 緑 | 🟦 **緑** | story 26 本（素材 18 + 自作 7 + Foundations 1） |
