...
- 🆕 **手7 に着手した。**ブランチ `step/h7-design-agent-behavior`（`b7a97f3` から分岐）。
  **[手順書](手順/手7_ClaudeDesignに一覧を組ませる.md)は `status: in-progress`。**
  🟦 **§2 の D1〜D7 は全件決着**（推奨どおり・2026-08-01）／🟦 **H7-01 前半（型の確認）と H7-03（依頼文の確定）まで完了。**
  🟥 **次は H7-01 の後半＝人が claude.ai/design でデザインプロジェクトを作るところから。**
...
🟦 **§2 の D1〜D7 は決着済み**（`C / A / A→C / B+C / B / C / A`）。🟦 **依頼文も [H7-03](手順/手7_ClaudeDesignに一覧を組ませる.md) に確定済み。**

**残っているのは Claude 側から起動できない部分だけ**（手6 D6 と同じ構造）。

1. claude.ai/design で**デザインプロジェクトを 1 つ新規作成**する（D7=A）
2. 参照するデザインシステムに **`design — UI検証` を選ぶ**（🟥 **`Modernist` と取り違えない**）
3. その URL（projectId）を Claude に渡す → `list_files` が通るかで **D3=A の可否**が決まる
4. [H7-03 の依頼文](手順/手7_ClaudeDesignに一覧を組ませる.md)を**そのまま**打つ（🟥 **部品名を足さない。足すと Q1 が測れない**）
