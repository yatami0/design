...
最終更新: 2026-08-09（★★ 🆕 **部品3 の手順書を起こした**（[部品3](手順/部品3_DatePickerと射程の外の3件.md)・status `planned`・**コードは 1 行も書いていない**）。🟦 **PR #16 はマージ済み**（`1c5bc0b`）＝ **未マージの PR は 0 件**。★★ **問いは 6 本**——**Q1 = shadcn が配らない部品を自分で設計したときバーは通るか**（🟥 **`date-picker` はレジストリ 404**＝ **上流の実装が 1 行も無い**。**語彙を自分で決める初めての部品**）／ ★★★ **Q2 = `DatePicker` を出すと `PeriodSelect` は使う側に回るか**（🟥 **合成レシピは既に repo の中で 1 度実装されている**——`PeriodSelect` の `custom` 分岐が Popover ＋ Calendar を内蔵。**回らないなら [OBS-0013](OBS/OBS-0013_部品があるのにインラインスタイルで組んだ.md) が repo の内側で再発する**）／ **Q3 = story が無い 3 件に初めて目盛りを当てたら何が出るか** ／ 🟥 **Q4 = `react-day-picker` / `date-fns` は出荷物の依存として妥当か** ／ **Q5 = 面⑥（44px）** ／ **Q6 = `locale` はコアか題材か**。★★★ 🟥 **着手前実測でいちばん重いのは Q4**——`git log -S` で引いたら、**この 2 件は工程3（`5a9abde`・2026-08-07）で `calendar` と一緒に `dependencies` に入っており、判断の記録が無い。**🟥 **`calendar` は export されていない**（`src/index.ts` L83「PeriodSelect の内部実装なので出さない」）のに、**依存だけは出荷物として払っている。**★★ **[DR-0092](DR/DR-0092-the-core-holds-the-vessel-not-the-state.md) の一般則「出荷物に依存を 1 件足すことは、使い回し先全部がその依存を取ること」が立ったのは 2026-08-08＝ この 2 件が入った翌日**——**規則が一度も当たっていない依存が出荷物に居る**。★ **`DatePicker` を出荷することは、その 2 件を「使われる依存」として追認すること**（→ D4）。★ **着手前実測はほかに 3 件**——① 🟦 **素材 29 件のうち製品層の窓口を持たないのは `calendar` ただ 1 件**（全件走査） ② 🟦 **依存は 0 増で部品が 1 件増える**（`react-day-picker` / `date-fns` は既在＝ K3） ③ 🟨 **`calendar` のセルは `--cell-size: --spacing(7)` = 28px で 42 個並ぶ**＝ **[OBS-0008](OBS/OBS-0008_当たり判定44pxをどう扱うか.md)（44px・open）に repo 史上いちばん強い材料**。★★ **判断ポイントは D1〜D9。**🟥 **D4（依存の追認）と D7（`pnpm test-storybook` をゲート 7 本目にするか）だけユーザー確認待ち**——**D4 は出荷物そのものを決める**（工程4 D1 が同じ理由でユーザー判断に上がり、答えは「UI はできるだけ純粋に保つ」だった＝ **今回はその答えと逆向きに見える**）／ **D7 は CLAUDE.md の規約で全工程に効く**（🟦 **部品1 D7=C の条件「critical を 0 にしてから」は満たされている**——critical 0・**バー 106/106 緑**。🟨 **代償は所要時間 約 70 秒**）。★ **ゲートは `main` で内訳まで一致**（typecheck 緑 ／ error 50 / warning 1 ／ dist 84・`design.mjs` 91,322 B ／ format 緑 ／ **spell 332** ／ **バー 106/106 緑**）。🟨 **`pnpm spell` は非対話シェルで落ちる**（`mise` が効かず node 22.16.0）——**既知・回避策は handoff §環境の再現**。🟥 **次: D4・D7 の確認（ユーザー）→ C3-01 から実行**）
...
- ★★ 🆕 **部品3（DatePicker と射程の外の 3 件）の手順書を起こした**（2026-08-09・[手順書](手順/部品3_DatePickerと射程の外の3件.md)・status `planned`・ブランチ `da-nang`）。
  **コードは 1 行も書いていない。**🟦 **未マージの PR は 0 件**（#16 = `1c5bc0b` がマージ済み）。
  - ★★ **この回はこれまでの 2 本と性質が違う**——🟥 **`shadcn add` を 1 度も打たない初めての回。**
    **素材は全部すでに在庫にあり、足りないのは「合成の設計」だけ**（`date-picker` は**レジストリ 404**・実測）
  - **問いは 6 本**:

    | # | 問い | 芯 |
    | --- | --- | --- |
    | **Q1** | ★★★ **shadcn が配らない部品を自分で設計したとき、バーは通るか** | **語彙を自分で決める初めての部品。**通らないなら**バーは「上流の部品を検品する道具」でしかない** |
    | **Q2** | ★★★ **`DatePicker` を出すと `PeriodSelect` は使う側に回るか** | 🟥 **合成レシピは既に repo の中で 1 度実装されている**（`custom` 分岐が Popover ＋ Calendar を内蔵・工程3 D13=A）。**回らないなら [OBS-0013](OBS/OBS-0013_部品があるのにインラインスタイルで組んだ.md) が repo の内側で再発する** |
    | **Q3** | ★★ **story が無い 3 件に初めて目盛りを当てたら何が出るか** | **critical 0 は「story がある部品」だけの主張だった。**保たれるかで**バーの緑の射程**が決まる |
    | **Q4** | 🟥 ★★ **`react-day-picker` / `date-fns` は出荷物の依存として妥当か** | **事後審査**（下記） |
    | **Q5** | ★ **面⑥（44px）はこの部品で初めて正面から効くか** | **28px のセルが 42 個** |
    | **Q6** | 🟨 **`locale` はコアか題材か** | [DR-0088](DR/DR-0088-core-subject-boundary-is-decided-by-two-questions.md) の 2 問の 3 度目 |

  - ★★★ 🟥 **着手前実測でいちばん重いのは Q4 — 出荷物の依存が、判断の記録なしに入っていた。**
    ...
    | 事実 | 実測 |
    | --- | --- |
    | 出自 | 🟥 **工程3（2026-08-07）**。`calendar` を `shadcn add` した副産物 |
    | `dependencies` に居るか | 🟥 **居る**（＝ 使い回し先が全部取る） |
    | `calendar` は export されているか | 🟥 **されていない**（`src/index.ts` L83「**PeriodSelect の内部実装なので出さない**」） |
    | [DR-0092](DR/DR-0092-the-core-holds-the-vessel-not-the-state.md) の一般則が立った日 | **2026-08-08**＝ 🟥 **この 2 件が入った翌日** |

    ★★ **「器（依存 0）で済む形を先に探す」という規則は、この 2 件には一度も当たっていない。**
    🟥 **`DatePicker` を出荷することは、その 2 件を「使われる依存」として追認すること**（→ D4）
  - **着手前実測はほかに 3 件**——
    ① 🟦 **素材 29 件のうち製品層の窓口を持たないのは `calendar` ただ 1 件**（全件走査で確認）
    ② 🟦 **依存 0 増で部品が 1 件増える**（`react-day-picker` / `date-fns` は既在＝ K3 の期待値）
    ③ 🟨 **`calendar` のセルは `--cell-size: --spacing(7)` = 28px で、1 か月に 42 個並ぶ**
    ＝ **[OBS-0008](OBS/OBS-0008_当たり判定44pxをどう扱うか.md)（44px・open）に repo 史上いちばん強い材料**
  - **判断ポイントは D1〜D9。**🟥 **D4 と D7 だけユーザー確認待ち**——
    - 🟥 **D4（依存の追認）**: **出荷物そのものを決める**判断。**[工程4 D1](手順/工程4_一覧から詳細_編集あり.md) が同じ理由でユーザー判断に上がり、答えは「UI はできるだけ純粋に保つ」だった**——
      🟥 **今回はその答えと逆向きに見える**ので、**Claude が単独で倒してはいけない**（推奨は **A＝追認 ＋ DR 起票**。**C＝自作は「有限の語で言えない」ので却下**）
    - 🟥 **D7（`pnpm test-storybook` をゲート 7 本目にするか）**: **CLAUDE.md の規約で全工程・全セッションに効く。**
      🟦 **部品1 D7=C の条件「critical を 0 にしてから」は満たされている**（critical 0・**バー 106/106 緑**）。
      🟨 **代償は所要時間 約 70 秒**（既存 6 本の合計より重い）。**推奨は A＝足す。ただし K7 で赤の伝播を確かめてから**
  - 🟦 **ゲートは `main`（`1c5bc0b`）で内訳まで一致**: typecheck 緑 ／ **error 50 / warning 1** ／ dist **84**・`design.mjs` **91,322 B** ／
    format 緑 ／ spell **332** ／ story **54 ファイル / 106 件** ／ **バー 106/106 緑**（`vitest --project=storybook --run`・実測 11.54 秒）
  - 🟨 **`pnpm spell` は非対話シェルで落ちる**（`mise` が効かず `node -v` が 22.16.0）＝ **既知・新しい赤ではない**。
    回避策は [§環境の再現](#環境の再現) の `export PATH="$HOME/.local/share/mise/installs/node/24.18.1/bin:$PATH"`
  - 🟥 **次: D4・D7 の確認（ユーザー）→ C3-01 から実行**
...
> 🟥 **次の一手は 部品3 の D4・D7 の確認（ユーザー）**——決着したら Claude が C3-01 から実行する。
>
> | # | やること | 誰がやるか |
> | --- | --- | --- |
> | **1** | 🟥 ★★ **[部品3 §2 D4](手順/部品3_DatePickerと射程の外の3件.md) を決める**——**`react-day-picker` / `date-fns` を出荷物の依存として追認するか。**🟨 **推奨は A（追認 ＋ finding の DR を起票）。**★ **工程4 D1 で「UI はできるだけ純粋に保つ」と答えたのと逆向きに見える**ので上げている（**器では済まない**＝カレンダーは有限の語で言えない、というのが推奨の根拠） | 🟥 **ユーザー判断** |
> | **2** | 🟥 ★★ **[部品3 §2 D7](手順/部品3_DatePickerと射程の外の3件.md) を決める**——**`pnpm test-storybook` をゲート 7 本目にするか**（[未決 #14](#未決保留) の最終形）。🟦 **条件（critical 0）は満たされている。**🟨 **代償は約 70 秒。**推奨は **A＝足す**（🟥 **K7 で赤の伝播を確かめてから**） | 🟥 **ユーザー判断** |
> | **3** | **部品3 を C3-01 から実行する**（D4・D7 の決着後。他の D1〜D3・D5・D6・D8・D9 は推奨どおりの Claude 判断・事後承認待ち） | Claude |
