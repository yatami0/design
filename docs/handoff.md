...
最終更新: 2026-07-27（**手5 を 2 周とも通し、目視レビューまで終えた**。残るは Q6・Q8 と締め）

---

## 現在地

- **手0〜手4 が完了**（すべて `main` へ `--no-ff` マージ済み）。**手5 は進行中**——ブランチ `step/h5-token-swap` に **11 コミット未マージ**。
- ★ **手5 の 2 周（素直 / 無理）を通し、目視レビューまで終えた。**残るは **Q6・Q8 の締め**と、下記の宿題 3 件。
- **決定 19 件・発見 34 件を [docs/DR/](DR/index.md) に切り出し済み**（DR-0001〜0053）。**今セッションで 13 件増えた**（0041〜0053）。
- 🟥 **OBS が `open` 6 件**（[docs/OBS/](OBS/index.md)）。**閾値 3 件の倍。手5 完了時に `/obs review` を必ず回す。**
...
| 手5 | ★ トークン差し替え実験 | [手5](手順/手5_トークン差し替え実験.md) | 🟨 **進行中。**2 周 + 目視まで完了。**残るは Q6・Q8 と締め** |
...
| ゲート | 手3 完了時 | 手4 完了時 | **手5 進行中（2026-07-27）** | 備考 |
|---|---|---|---|---|
...
**ブランチ `step/h5-token-swap` に 11 コミット。作業ツリー clean。`main` へは未マージ。**

### 🟥 宿題 3 件（着手順）

| # | やること | どこで | なぜ |
| --- | --- | --- | --- |
| **1** | **フォーカスリングを実物の `Input` で測り直す** | `tools/visual-probe.mjs` に `<Input aria-invalid>` の検体を足す | ユーザーが「エラー時のフォーカスリングもブランド色に見える」と述べた。CSS の順序上は `aria-invalid:ring-destructive/20`(@44600) が `focus-visible:ring-ring/50`(@41235) に勝つ**はず**だが、**実物で測っていない**（[OBS-0009](OBS/OBS-0009_不透明度と状態面の概念を理解する.md) §1） |
| **2** | **`④ AppShell — CardSurfaces` の `Card` を `CardContent` 経由に直す** | `src/stories/Templates/AppShell.stories.tsx` | `card.tsx` の root は `py-` だけで `px-` を持たない。素の children を直接入れると**左右の余白がゼロ**（[DR-0053](DR/DR-0053-viewpoints-must-be-answerable-by-eye.md)） |
| **3** | **Q6・Q8 に答えて手5 を締める** | 手順書 [手5](手順/手5_トークン差し替え実験.md) §0 | **Q6** = 層タグで切り分けられたか → [DR-0051](DR/DR-0051-storybook-organized-by-layer-with-viewpoint-cards.md) が「タグだけでは足りず階層に出す必要があった」と答えているので**まとめるだけ**。**Q8** = ① 層への思想の指摘が出たか（[共通コンポーネント思想への指摘.md](共通コンポーネント思想への指摘.md) の 8 件目。0 件でもよい） |

### そのあと

1. 🟥 **`/obs review` を回す**（`open` **6 件**＝閾値の倍）。とくに [OBS-0007](OBS/OBS-0007_発見に推論を混ぜると後続が数え間違える.md) は
   **2 回目・3 回目が揃った**ので、`docs/DR/_template.md` の §影響 の書き方を変えるかの判断が要る
2. 実行記録 §手5 の締め（Q1〜Q8 の一覧）
3. 手順書 [手5](手順/手5_トークン差し替え実験.md) の `status` を `done` に、`next_action` を更新
4. **`main` へ `--no-ff` マージを提案する**（🟥 実行はしない）

### 🔺 ADR 昇格候補が 4 件になった（未決 #20）

[DR-0032](DR/DR-0032-layout-primitives-take-props-not-classname.md) / [DR-0033](DR/DR-0033-step5-criteria-differ-per-layer.md) / [DR-0034](DR/DR-0034-touch-target-visual-32-hit-44.md) / [DR-0052](DR/DR-0052-unreachable-spots-are-avoided-by-not-using-them.md)。
