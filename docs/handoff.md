...
- ★★ 🆕 **工程3（共通シェル）の手順書を起こした**（2026-08-08・[工程3](手順/工程3_共通シェル.md)・status `planned`）。**コードはまだ 1 行も書いていない。**
  ...
  - **赤テスト K1〜K6 を先に置いた。**★ **K1 が要**——フィルタは「UI は動くが取得に効いていない」形で**平然と緑になる**（絞り込まれた絵に見える）。→ 🟥 **件数だけでなく飛んだ URL を証拠に残す**
  - 🟨 **§2 は D1〜D10・全件推奨つき（事後承認待ち・すべて 🟦 戻せる）。**工場の思想に触るのは 3 件——
...
| **工程3** | ★ 共通シェル（工場の芯・4 画面に効く） | 🟨 **planned**（2026-08-08・[手順書](手順/工程3_共通シェル.md)）。**問い 6 本・判断 D1〜D10・赤テスト K1〜K6 まで確定。コードは 0 行。**★ 本体は **Q4＝コア/Redmine 固有の判定規則の起草**（段取り 未決 #6。候補＝**コアは語彙・題材は対応表**）。🟥 **次は P3-01 から実行** |
| 工程4 | 一覧 → 詳細（編集あり・最重） | ⬜ |
| 工程5 | 稼働表（ピボット） | ⬜ |
...
>
> 1. ★ **工程3 を実行する**（P3-01 → P3-10）。🟨 **§2 の D1〜D10 は推奨どおりの Claude 判断**——着手前に**異議があれば言ってほしい**。工場の思想に触るのは **D1**（画面は題材側 `src/redmine/screens/` に置く）・**D2**（フィルタ帯は器だけコアが持つ）・**D9**（story の棚の第 1 階層に `⑤ 題材（Redmine）` を新設）の 3 件。全件 🟦 git で戻せる
