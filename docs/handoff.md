...
最終更新: 2026-08-07（**✅ 手8c 完了。**DS 8 本調査 → 判定規則と部品設計 → 検算まで。**実装はしていない**——次はどの手で実装するかのユーザー判断）
...
| # | 論点 | 状態 |
| --- | --- | --- |
| 1 | **PR を出してマージする**（ブランチ `hand8-design-investigation` → `main`。[DR-0068](DR/DR-0068-merge-through-pull-requests.md) によりマージは人） | 🟥 提案済み・未実行 |
| 2 | **設計 4 件＋語彙 1＋散文 1 の実装をどの手に割り当てるか**——新しい手（手8d）か、手9 の一部か。[製品層の部品設計.md](製品層の部品設計.md) はそのまま実装に入れる形（props シグネチャまで確定） | 🟥 未決 |
