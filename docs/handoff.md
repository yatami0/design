...
- ★★ 🆕 **工程4（一覧 → 詳細・編集あり）の手順書を起こした**（2026-08-08・[工程4](手順/工程4_一覧から詳細_編集あり.md)・status `planned`）。**コードはまだ 1 行も書いていない。**
...
| 工程4 | 一覧 → 詳細（編集あり・最重） | 🆕 ⬜ **planned**（2026-08-08・[手順書](手順/工程4_一覧から詳細_編集あり.md)）。**問いは 6 本**（Q1 編集の状態管理＝未決 #4／Q2 バリデーション／Q3 DescriptionList・Timeline／Q4 見た目の選択／🆕 Q5 書き込みでも「3 点だけ」か／🆕 Q6 外部依存は出荷面に何を出すか）。🟥 **§2 D1 だけユーザー確認待ち**（推奨 B が段取りの推奨 A と食い違う） |
| 工程5 | 稼働表（ピボット） | ⬜ |
...
> ★★ 🆕 **2026-08-08 更新: 未マージの PR は 0 件。工程4 の手順書まで起きた。**🟥 **次の一手は下記 3b（D1 の確認）。**
>
...
> 3. ✅ ~~工程4 の手順書を起こす~~ **完了**（2026-08-08・[工程4](手順/工程4_一覧から詳細_編集あり.md)・status `planned`）。問い 6 本・判断 D1〜D12・赤テスト K1〜K8。
> 3b. 🟥 ★★ **次の一手は §2 D1 の確認**（[段取り 未決 #4](工場の段取り.md)＝編集の状態管理の置き場）——
>    **A: コアに shadcn `Form`**（`react-hook-form` / `zod` / `@hookform/resolvers` を出荷物の依存にする。**段取りの推奨**）／
>    **B: コアは `field` の器だけ（依存 0）＋ rhf ＋ zod は題材の画面が持つ**（**手順書の推奨**・[DR-0088](DR/DR-0088-core-subject-boundary-is-decided-by-two-questions.md) の 2 問の帰結）／ C: 完全自作。
>    🟦 **B → A は後から `shadcn add form` を足すだけ**（A → B は依存を抜く分だけ重い）＝ **軽いほうから始められる。**
>    **決まり次第 P4-01 から実行できる**（D2〜D12 は推奨どおりで進む）
> 4. 🟨 申し送り: ~~**fixtures の全廃は工程4 で再判定**~~ → **手順書 D10 で判定済み＝据え置き**（判定条件は工程3 が「既存 story を作り直す必要が出るか」と明記。🟦 実測: 使っているのは `DataGrid` / `ListDetail` / `AppShell` の **3 story**でいずれもコアの検証装置、工程4 は触らない）。**次の再判定は「コアの story が MSW を要求したとき」**
> 5. 🟨 申し送り: 工程3 の締めで[段取り 未決 #7](工場の段取り.md)（規約文書の起草時機）を問うた——**答えは「まだ」**（使い回し先 0・DR 参照の不便も未発生）。ただし **DR-0088 の検算表は「境界判定の 2 問」の規約 1 ページ目の材料**になる。工程4 の締め（P4-10）でもう一度問う
> 6. 🟨 申し送り: **[未決 #14](#未決保留)（`@storybook/addon-vitest`）は工程5 の着手前に問う**（手順書 D11=B の帰結——工程4 で入れると変数が 2 つ動く。🟥 **理由は回数ではなく「Playwright の手順が 3 工程連続で同じ形になったか」**）
>
