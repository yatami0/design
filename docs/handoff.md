...
- ★★ 🆕 **部品6 の手順書を起こした**（2026-08-09・[部品6](手順/部品6_緑の面積を数える.md)・status `planned`・**コードは 1 行も書いていない**）。
  **問いは Q1〜Q5・判断ポイントは D1〜D10。🟥 D10（ゲート 8 本目）だけユーザー判断**（**持ち越し 3 回目**）。
...
| 工程5 | 稼働表（ピボット） | ⬜ 🟨 **手順書は書き上がっている**（[工程5](手順/工程5_稼働表_ピボット.md)）。🟦 **D7（addon-vitest）は部品3 D7=A が先に決めた**ので、再開時は読み替える |
...
> ★★★ 🆕 **2026-08-09 更新（部品軸）: 部品1〜部品5 は done・PR も全部マージ済み**（部品5 = `380b896` / PR #19）。
> 🆕 **2026-08-09 追記: 部品6 の手順書を起こした**（[部品6](手順/部品6_緑の面積を数える.md)・status `planned`・**コードは 1 行も書いていない**）。
>
> | # | やること | 誰がやるか |
> | --- | --- | --- |
> | **A** | ✅ ~~**部品5 の PR をマージする**~~ **完了**（`380b896` = [PR #19](https://github.com/yatami0/design/pull/19)） | ✅ |
> | **B** | 🟥 ★★★ **`node tools/opened-overlay-check.mjs` をゲート 8 本目にするか（部品6 D10・持ち越し 3 回目）**——🟦 **材料はさらに強い**: ① 赤テスト両方向済み ② 所要 0.1 秒 ③ 現在 7/7 緑 ④ **部品5 で「コメントで通る」穴を塞いだ**（[DR-0101](DR/DR-0101-a-comment-satisfied-the-check-for-absence.md)） ⑤ 🆕 **部品6 D4=D の網（型 ＋ 固定した赤テスト）が同じ道具に掛かる**。🟥 **同時に「この検査が見ていないもの」も決める**——**`sidebar.tsx` は `Primitive.Portal` を書かないので一覧に載らない**（部品5 Q3 の ③） | 🟥 **ユーザー判断** |
> | **C** | 🟥 ★★ **部品6 を C6-01 から実行する**（[手順書](手順/部品6_緑の面積を数える.md)・**問いは Q1〜Q5・判断は D1〜D10**）。**候補 3 本のうち ①（[OBS-0022](OBS/OBS-0022_他の機械ゲートにも保留の口があるか.md)）と ②（[OBS-0021](OBS/OBS-0021_面1は部品が描かれたことを見ていない.md)）を 1 回に束ねた**——🟥 **③（面③ の借金 33 件）は D3=B で外した**（**部品1 D6=B「触るときに返す」と正面から食い違う**。代わりに**返し忘れが見える列を台帳に足す**） | Claude |
