...
D1=C の 2 周目は「A が 🟥 だったときの切り分け」用だったが、**1 周目で使われた。**

| 選択肢 | 中身 |
| --- | --- |
| **A（推奨）** | **事前登録どおり回す**（末尾に 1 文だけ足す）。価値が Q1 → **Q3（宣言語彙の外に出た 4 語が明示で締まるか）**へ移る |
| **B** | 回さず Q1 を確定させて閉じる。安いが **Q3 に片側の観測しか残らない** |
| **C** | 🟥 予定を変えて「宣言した語彙だけを使え」と明示する。**事前登録から外れる**ので、やるなら A の後の 3 周目 |
