...
最終更新: 2026-08-07（🆕 **✅ 手8e を 7 周目まで通した。予測 4 本のうち 3 本が的中し、宣言語彙の外に出たクラスは 0 件**。★★ **予測に無かった観測——デザイン側は `.prompt.md` を直接読んでいた**（[DR-0075](DR/DR-0075-design-side-reads-the-design-system-directly.md) が [DR-0064](DR/DR-0064-design-project-receives-runtime-only.md) を覆した）。🆕 **観測の様式を広げた**——8 周目から**過程も持ち帰る**（[DR-0076](DR/DR-0076-capture-the-run-not-just-the-output.md)））
...
✅ **手8c（設計）・手8d（実装）・手8e（再同期 → 7 周目）まで完了した**（2026-08-07）。
🆕 **次の一手は 8 周目。🟥 生成は人が打つので、着手はユーザー操作から始まる。**

### 🟥 0. 手8e の後始末 → 8 周目へ

| # | やること | 状態 |
| --- | --- | --- |
| 1 | **PR をマージする**——🟦 **PR は出した**（[#5](https://github.com/yatami0/design/pull/5)・ブランチ `commit-and-handoff-update`）。[DR-0068](DR/DR-0068-merge-through-pull-requests.md) により**マージは人** | 🟥 **マージ待ち** |
| 2 | 🆕 **8 周目は [DR-0076](DR/DR-0076-capture-the-run-not-just-the-output.md) の 3 点で観測する**——① **ツールトレースを人が撮って `artifacts/h8f/` へ** ② 成果物を 1 回通しで読む ③ 予測表に「予測していない箇所」の行を置く | 🟥 **8 周目の様式** |
| 3 | 🆕 **8 周目に測る宿題 2 件**: ① [OBS-0013](OBS/OBS-0013_部品があるのにインラインスタイルで組んだ.md)（インラインスタイルが **2 回目**を出すか。出たら DR-0070 の規則①が成立し打ち手の検討に入る）② **何をきっかけに `.prompt.md` を選ぶのか**（README 由来 / 要件由来 / 常に全部。**トレースを取れば切り分けられるはず**——[DR-0075](DR/DR-0075-design-side-reads-the-design-system-directly.md) の推論） | 🟥 **未実行** |
| 4 | 🟨 **`Link` はまだ 1 度も選ばれていない**（面④b は「リンクを作る場面が消えた」ので消えただけ）。**リンクが要る画面でないと測れない**ので、依頼文を変えるかどうかは未決（変えると 1 変数が壊れる） | 🟨 **未決** |
| 5 | 🟨 あわせて **手8b の動機を測り直す**（製品層で包む範囲が広がったので「やらない」の目が上がった） | 8 周目の後 |
| 6 | **D3=C・D8=B の事後承認**——手8c の調査範囲を推奨（B/A）より広げた判断は、ユーザーの「詳細な調査から設計が重要です」（2026-08-07）を根拠にした。**異議があれば言ってほしい**（結果は変わらないが、判断の形として記録に残す） | 🟨 事後報告 |
