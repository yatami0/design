...
| **工程3** | ★ 共通シェル（工場の芯・4 画面に効く） | 🟨 **planned**（2026-08-08・[手順書](手順/工程3_共通シェル.md)）。**問い 6 本・判断 D1〜D11・赤テスト K1〜K7 まで確定（D11・K7 はレビューで追加）。コードは 0 行。**★ 本体は **Q4＝コア/Redmine 固有の判定規則の起草**（段取り 未決 #6。候補＝**コアは語彙・題材は対応表**）。🟥 **次は P3-01 から実行** |
| 工程4 | 一覧 → 詳細（編集あり・最重） | ⬜ |
| 工程5 | 稼働表（ピボット） | ⬜ |
...
> ★★ 🆕 **2026-08-08: 工程3 の手順書ができた**（[工程3](手順/工程3_共通シェル.md)・status `planned`）。**次:**
>
> 1. ★ **工程3 を実行する**（P3-01 → P3-10）。🟨 **§2 の D1〜D11 は推奨どおりの Claude 判断**——着手前に**異議があれば言ってほしい**。工場の思想に触るのは **D1**（画面は題材側 `src/redmine/screens/` に置く）・**D2**（フィルタ帯は器だけコアが持つ）・**D9**（story の棚の第 1 階層に `⑤ 題材（Redmine）` を新設）・🆕 **D11**（lint の射程を画面層へ張り直す・レビューで追加）の 4 件。全件 🟦 git で戻せる
> 2. 🟥 **次回の `/design-sync` は人が打つ**——工程1 Q4（出荷経路の維持）の最終確認。🟦 **工程2 で dist は 1 バイトも動いていない**ので条件は変わっていない。🟨 **工程3 では条件が変わる**（コアの新部品を `componentSrcMap` に足す＝ D10。**題材の story がカードに湧かないか**も同時に見る）
> 3. 🟨 **工程2 の D1〜D13 は事後承認待ちのまま**（[手順書 §2](手順/工程2_データの器_MSWとデータモデル.md)。とくに **D4**（取得の層をコアに置かない）と **D12**（lint 2 本を戻した）は工場の思想に触る）。PR [#9](https://github.com/yatami0/design/pull/9) は**マージ済み** `5212ff0`
> 4. ✅ 申し送り消化: D3（`rsc: false`）の「shadcn 追加インストールで部品が変わって降ってくるか」は**工程3 Q3・P3-02 に組み込んだ**
> 5. ✅ 申し送り消化: **`dist/lib/fixtures/issues.d.ts` の漏れ**は**工程3 D6・P3-09 で塞ぐ**と決めた（[DR-0085](DR/DR-0085-three-independent-scopes-decide-what-ships.md)）
> 6. 🟨 **工程3 の締めで [段取り 未決 #7](工場の段取り.md)（工場の規約文書の起草時機）を 1 度問う**
>
