...
flowchart TD
  ...
  H6["手6: /design-sync で登録<br/>→ 役割カテゴリとフラグは渡るか観測"]
...
| 手 | 手順書 |
|---|---|
| 手0 | （無し。フォーマット確定前に実行した） |
| 手1 | [手1_shadcn導入と役割分類.md](手順/手1_shadcn導入と役割分類.md) |
...
- 手6 の観測点：**役割カテゴリとフラグが `/design-sync` で渡らないなら、辞書は Claude Code 側（skill / rules）で持たせるしかない。**思想の「AI が辞書引きできるようにする」の受け手が誰になるかがここで決まる。
