...
| `Badge` / `Empty` / `Skeleton` / `Table` / `Label` / `Separator` / `Card` / `Pagination` / `Dialog` / `DropdownMenu` / `Popover` / `Sheet` / `Tooltip` / `Checkbox` / `Input` | 再輸出 15 件 | 開（上流のまま） | **0** | **素通しのまま**（規則①。需要が 2 回証明された部品から個別にラッパーへ昇格させる——`Select` が最初の 1 件） |
...
- 🟨 先例（MUI `align:'right'`）と違い**寄せは自動で当てない**——6 周の逸脱に寄せは 1 件も無く、tmp-admin §4.4 にも規定が無い。足すなら実測が出てから（規則①）
- 🟨 MUI / TanStack は「ソート・フィルタ用の生値」と「表示」を分ける（`valueGetter` / `accessorFn`）。我々の `accessor` は表示専用の一本——**現行 DataGrid はソート UI を持たないので生値の口は作らない**。ソートの需要が出たら `accessorFn` 相当を足す（規則①）
...
- 🟨 `min-height: 100%`（r1 だけが書いた）は**足さない**——`SidebarProvider` が `min-h-svh` を持っており（`ui/sidebar.tsx` L140）viewport 基準なので親の高さに依存しない。**1/6 周＝規則①で不成立**
...
2. `kind`/`emphasis` の語彙で design agent の**表現意図を全部受けられるかは未知**——受けられない意図が出たら、それは §1.2 の規則①（新しい観測）として次の周に数える
...
3. **`asChild` / render prop は導入しない**——需要の実測 0 回（規則①）。必要が 2 回証明されたら §1.2 の手順で判定する
