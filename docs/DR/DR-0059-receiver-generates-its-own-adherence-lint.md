...
> 🟥 **[DR-0066](DR-0066-neither-side-lints-the-generated-output.md)（2026-08-02）が 2 点を訂正した。**本文は記録として残す。
> ① 🟥 **§推論「走っていなければただの飾り」より悪い。**`no-restricted-syntax` は **oxlint に存在しない**ので、
> **設定ファイル全体が parse エラーになり、実装されている 2 本まで道連れになる。**
> ② 🟥 **§影響 3「`.d.ts` の質が受け手の lint の質を決める」は原因の取り違え。**
> **型は正しく、抽出が浅かった**——継承した props（HTML 属性・Radix の props）が落ち、
> **走ったと仮定して当たる 5 件は全部偽陽性**だった。
> 🟦 **観測部（設定の存在・セレクタの形・`replaces` の内訳・トークン一覧の食い違い）は正しい。**
> 🟨 なお **§3 の「`<button>` 1 本だけ」は手7 で素材層 16 件を足した結果 6 本に増えた**（[実行記録 §手8 H8-02](../実行記録.md)）。
