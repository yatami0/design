...
| [DR-0105](DR-0105-split-a-component-on-readability-not-writability.md) | 🆕 ⭐ **部品を分ける基準は「既存 API で書けるか」ではなく「書けたものが読めるか」** — まず既存部品で組み、**破れた項目の一覧**を新設の理由にする。🟥 **`DataGrid` で 92 列のピボットは組めた**（`th` 94 本・合計も一致）**が 4 点破れた**（セル単位の符号化／行ヘッダの固定／合計行／列見出しの強弱）。★ **予測は 3 点で実際は 4 点**——**先に断定していたら 4 点目は出ていない。**🟥 **4 点のどれも tsc / lint / build を緑で通る** | 工程5 | decided |
...
| [DR-0095](DR-0095-shipped-dependencies-arrived-without-a-decision.md) | 🆕 ⭐★ **出荷物の依存 2 件は、誰の判断でもなく入っていた** — `react-day-picker` / `date-fns` は**工程3 で `shadcn add calendar` の副産物として `dependencies` に入り**、判断の記録が 1 行も無い。🟥 **`calendar` 自身は「出さない」と明示して出荷していない**のに、**依存だけは出荷物として払っていた**。[DR-0092](DR-0092-the-core-holds-the-vessel-not-the-state.md) の一般則が立ったのは**その翌日**＝ **規則が一度も当たっていない依存が 2 件**。★★ **一般則は「足す判断」に掛かるが、依存が増える経路は判断を伴わない**——`shadcn add` は部品と npm パッケージを同時に引き、**前者は見えて後者は `package.json` の 1 行**。🟦 **[部品3 D4=A（ユーザー判断）で追認**（器では済まない＝カレンダーは有限の語で言えない） | — | **部品を足すたび**・出荷・依存 |
| [DR-0096](DR-0096-overlays-that-no-story-opens-are-invisible-to-every-check.md) | 🆕 ⭐★ **開かれない overlay は、どの検査からも見えない** — `PopoverContent` は Radix の `role="dialog"` を出すが**名前を持たず**（`PopoverTitle` は `aria-labelledby` に繋がっていない）、**工程3 から 2 日間、名前の無い dialog を出荷していた**。🟥 **見えなかった理由は「Popover を開く story が 1 本も無かった」**——閉じた overlay は DOM を持たないので **axe も面① も面④ も「0 件で緑」**。★ [DR-0048](DR-0048-build-storybook-does-not-render.md) が「実行しないから見えない」なのに対し、本件は**「実行しても、開かないから見えない」**。塞ぎ方は**製品層で `PopoverContent` を昇格させ `aria-label` を型で必須に**（素材層 0 行・`tsc` が既存 2 箇所を落とした）。🟥 **`DropdownMenu` / `Sheet` / `Tooltip` / `Select` の中身はいまも開かれていない** | — | **overlay を触るたび**・面②・story の書き方 |
| [DR-0097](DR-0097-an-exception-that-covered-the-whole-set-hid-the-rule.md) | 🆕 ⭐ **例外が全量を覆っていて、規則の形が見えなかった** — [完成バー](../部品の完成バー.md) §3 は「**serious は落とさない**」と書いていたが、**実装（`a11y.test: 'error'`）は impact を 1 度も見ていない**（`color-contrast` と `region` を **rule 単位**で無効化しているだけ）。**食い違いに気づけなかったのは、serious が 77 → 113 → 142 件とも 100% `color-contrast`（無効化済み）だったから**＝ **規則が働く場面が 1 度も来なかった**。🟦 **実装のほうが正しかった**（今回まさに「落ちない側」の記述に従っていたら [DR-0096](DR-0096-overlays-that-no-story-opens-are-invisible-to-every-check.md) は台帳の数字になって誰も直さない）→ **定義を実装に合わせて訂正**。★ **impact で切る設計と rule で切る設計は別物**で、この repo が採っていたのは後者 | — | **バーを使うたび**・面② |
| [DR-0098](DR-0098-incomplete-was-counted-as-green.md) | 🆕 ⭐★ **機械が「分からない」と答えたものが、緑として記録されていた** — axe の `incomplete` を、**落とす側（`a11y.test: 'error'`）も数える側（`tools/a11y-scan.mjs` の `resultTypes: ['violations']`）も独立に捨てていた**。🟥 **`Sheet/Open` は「通った」のではなく、axe が `role="dialog"` を見て判定を放棄していた**（`violations: 0` ／ `incomplete: aria-hidden-focus/serious/3`）。★ **「対象 0 件で緑」とは別の型**——**対象は在り、検査も走り、それでも緑になる**。🟦 数える場所を作った（出荷物の棚 **23 件**）が**落としてはいない** | — | **バーを使うたび**・面② |
| [DR-0099](DR-0099-the-blind-spot-list-must-be-machine-derived.md) | 🆕 ⭐★ **射程の外の一覧は機械が引く** — **静的**（`tools/opened-overlay-check.mjs` が素材層の `Primitive.Portal` から引き、主張を要求）＋ **動的**（`src/stories/opened.ts` の `expectOpened` が主張の真偽を測る）の 2 段。🟥 **片方では閉じない**——静的だけなら「書いてあるが開いていない」を通し、動的だけなら「書かなければ 0 件で緑」。★★ **機械に引かせたら 7 件出た**——`dropdown-menu-sub-content` は目で数えた 4 件にも入っていなかった | — | **overlay を足すたび**・検査の作り方 |
| [DR-0100](DR-0100-pending-judgements-split-into-three-kinds.md) | 🆕 ⭐ **判定の保留は 3 種類に割れた** — (a) **文字が短くて判定を保留**（9 件・🟥 **実測すると本当に AA 未達**で、既知の色の組と同一） ／ (b) **重なっていて背景色を決められない**（1 件） ／ (c) **検査器が無条件に保留**（13 件）。🟥 **[OBS-0019](../OBS/OBS-0019_storyが一度も描いていない状態をどこまで機械で要求するか.md) §5 の見立ては外れた**——`aria-valid-attr-value` の**参照先 ID は 2 件とも実在し**、axe が `aria-haspopup` の有無だけで実在を確かめずに保留を立てていた（ソース実測）。🟦 **保留 23 件は再現する**（2 回打ってノード単位で一致） | — | **保留を数えるたび**・面② |
| [DR-0101](DR-0101-a-comment-satisfied-the-check-for-absence.md) | 🆕 ⭐★ **「無いことを落とす検査」がコメントで通っていた** — [DR-0099](DR-0099-the-blind-spot-list-must-be-machine-derived.md) の静的側は 1 本の正規表現で、**JSDoc に書いた `expectOpened('sheet-content')` を主張として数えた**。🟥 **本物の主張を消しても検査は緑のまま**（赤テストで確認）。★★ **「型は通るが作用しない」の逆向き**——**書いていないのに効いたことになる**。🟦 **コメントを落としてから走査するようにして塞いだ**（両方向の赤テスト済み）。🟥 **置かれた翌日に見つかった**（間に挟まったのは 1 PR） | — | **文字列で検査を書くたび** |
| [DR-0104](DR-0104-the-dom-never-overflowed.md) | 🆕 ⭐★ **検査は射程に在り毎回走っていたが、その条件を満たす DOM が 130 story に 1 つも無かった** — 92 列のピボットで**バーが初めて赤**（`scrollable-region-focusable`・serious＝**横スクロールにキーボードで到達できない**）。🟥 **規則は無効化されていない**（無効なのは `color-contrast` / `region` の 2 つだけ）。**器も欠陥も `shadcn add` の初日から在り、チケット一覧も同じ器を使っている。**★★ **足りなかったのは検査でも規則でもなく「溢れる表」**——**「対象 0 件」でも「保留」でも「主張 0 本」でもない 4 つ目の型（条件未到達で緑）**。🟥 **面② は 130/130 で 100% と表示されていた** | 工程5 | **極端な入力の story を足すたび**・面② |
...
| DR-0095 | 🆕 🟥 工場の規約（依存） | **依存の審査は「足す操作」ではなく `dependencies` の差分に掛ける。**副産物で入った依存は、その部品を出荷するまで誰にも見えない（`dist` にも `.d.ts` にも出ない）。★ **出荷入口 4 本はどれも「何が出るか」しか見ておらず、「何を引き連れて出るか」を見ていない** |
| DR-0096 | 🆕 🟥 工場の規約（story） | **開閉を持つ部品は「開いた story」を必須にする。**閉じた状態しか描かない story は、その部品を 1 度も検査していない。★ **名前の既定を持てない部品は、文書ではなく型で要求する** |
| DR-0097 | 🆕 🟥 工場の規約（検査） | **「落とさない」と書いた面は、実装がどう落としているかを別に確かめる。**例外が全量を覆っていると、規則は一度も観測されないまま正しく見える |
| DR-0098 | 🆕 🟥 工場の規約（検査） | **検査結果を「合格 / 不合格」の 2 値で読まない。**機械が判定を保留した件数を、合格と別に数える。★ **落とす側も数える側も独立に `violations` しか見ておらず、「分からない」が緑になっていた**（axe の `incomplete`・出荷物の棚に 23 件） |
| DR-0099 | 🆕 🟥 工場の規約（検査） | **「検査が届いていない対象」の一覧を文書に列挙しない。**機械が**性質**から引き（`Primitive.Portal`）、**検体側に主張を要求する 2 段**で閉じる。★ **目で数えた 4 件は 1 件が誤りで、1 件が漏れていた** |
| DR-0100 | 🆕 🟥 工場の規約（検査） | **「保留」を数えるだけでは足りない。**種類（誰の事情か）と理由（機械の `messageKey`）を一緒に保存し、**分類できないものは落とす**。★ **畳むときは必ず引き換え**（機械が測れなかったものを我々が測る手）**を書く** |
| DR-0101 | 🆕 🟥 工場の規約（検査） | **ソースを文字列で検査するものは、コメントを先に落とす。**落とさないと「検査を通すために書く 1 行」が実装と同じ重さを持つ。★ **「無いことを落とす検査」は、自分自身が最も偽の緑を作りやすい** |
| DR-0102 | 🆕 🟥 工場の規約（検査） | **「N/N 緑」を件数で読まない。面ごとに「掛かった / 対象」を機械で出す。**★ **130 中 103（79%）は主張を 1 本も持たず、掛かっていたのは「何かが描かれた」と「axe が黙った」の 2 つだけだった**——**対象は在り、検査も走り、結果も正しい。ただ何も要求していない** |
| DR-0103 | 🆕 🟥 工場の規約（測定器） | **測る側にも網を掛ける。**数字を作る `tools/*.mjs` 1,512 行に型検査も自動テストも無かった。🟥 **意図的な数え落としを入れてもゲート 7 本は 1 本も反応せず、台帳の数字だけが変わった**（保留 23 → 10）。★ **手で打って捨てていた両方向の赤テストを repo に固定する**（`pnpm self-check`） |
