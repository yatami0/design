...
id: DR-0069
type: finding
title: '規約ヘッダに禁止を足したら、禁止した箇所以外が壊れた — 2 周連続で骨格部品が消え、戻したら全部戻った'
status: observed
date: 2026-08-02
step: 手8
...
### 5. ★ 対照（6 周目）で全部戻った — 因果が確定した

ヘッダを手7 の状態へ戻しただけで、部品・語彙・依頼文は不変。

| | 3 周目 | 4 周目 | 5 周目 | **6 周目** |
| --- | --- | --- | --- | --- |
| `Container` / `Section` / `DataGrid` | 1 / 1 / 1 | 1 / 1 / 1 | 🟥 0 / 0 / 0 | 🟦 **1 / 1 / 1** |
| `Table` 系を素材層で手組み | 0 | 0 | 🟥 17 | 🟦 **0** |
| `x-import` に `class=` | 0 | 🟥 2 | 🟥 5 | 🟦 **0** |
| `class-name=` | 2 | 🟥 0 | 🟥 0 | 🟦 **2** |

★ **3 周目と完全に同じ形に戻った。**
🟥 **`class=` もヘッダ由来だった**——起票時の「n=1 なので断定できない」は**対照で解消した。**

## 根拠（実測）

2026-08-02。検体 **6 本**を markup の `x-import` 単位で数えた
（`artifacts/h7/*.dc.html` 3 本 ＋ `artifacts/h8/RedmineIssueList-r4/-r5/-r6.dc.html`）。
**`-r6` がロールバック後の対照。**
...
- 🟨 **DSL の綴りが「元から不安定」なのか「ヘッダで揺れた」のかは、まだ切り分いていない。**
  1 周目は `onClick=`（camelCase）を書いている。🟦 **ただし `class=`（＝実害が出る綴り）は 4・5 周目にしか出ず、
  戻したら消えた**ので、**実害のある方はヘッダに反応している。**
