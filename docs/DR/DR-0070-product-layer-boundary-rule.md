...
date: 2026-08-07
step: 手8c
related: [DR-0032, DR-0063, DR-0069, DR-0077]
poc_feedback: 'ui.md / architecture.md の材料（packages/ui の層境界の規則）'
---

# DR-0070: 素材層と製品層の境界は「見た目の管轄権」＋ 3 段の判定手順で決める

> 🟥 **superseded — [DR-0077](DR-0077-abolish-the-two-occurrence-rule.md)（2026-08-07・ユーザー判断）が置き換えた。**
> **境界の原理（見た目の管轄権）と判定手順の 2・3 段目はそのまま生きている。**落ちたのは
> **第 1 段「同じ場所から 2 回以上出たか」だけ**——2 回ルールの廃止に伴い、**回数は判断の条件ではなくなった**
> （記録する観測量としては数え続ける）。判定手順は **① 語彙で表せるか → ② 誰の責務か** の 2 段。
> 🟦 **手8c の分類結果は 1 件も変わらない**（規則①が単独で結論を決めた行は無い。DR-0077 §影響 2）。
> **本文は書き換えない**（[_template.md](_template.md) の規律）。
