...
| ②③④ をアプリの方針で差し替えられるようにする | 🟦 **成立する。**これが最も強い理由だが、**差し替えたい要求そのものがまだ出ていない**（🆕 回数ではなく**要求の不在**が理由・[DR-0077](DR/DR-0077-abolish-the-two-occurrence-rule.md)） |
...
5. **「状態は hook へ」は、状態と副作用を区別していない。**
   ...
   🆕 **軸の形（何を分類するか）がまだ定まっていない**ので、本書は指摘に留める（🟥 回数は理由にしない・[DR-0077](DR/DR-0077-abolish-the-two-occurrence-rule.md)）。
