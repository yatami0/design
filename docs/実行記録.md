...
| **Q2** | ★ 製品層に足すべき部品は何か | 🟦 **4 件＋語彙 1＋散文 1**（全件に実測回数）。0 回の部品は入っていない——素材層 15 件のラッパーは棄却 |
...
| **Q5** | 設計は 6 周の実測を説明できるか | 🟦 **全件対応づいた**（H8C-07）。🟥 机上の検算であり、実際に消えるかは 7 周目 |
...
| **Q8** | 「対象 0 件で緑」16 例目が出るか | 🟨 **ゲートでは出ていない**（この手は docs のみ）。🟥 **同型の穴は 2 つ踏んだ**——① 調査エージェント 3 本が落ちたまま「5 本で結論」を書きかけた（再走で全数到着させた）② 取得日の誤指示（08-02）にエージェント 7 本が従い、1 本だけが実日付で訂正した——**検証装置（エージェント指示）自体の誤りは、出力の形式が揃っているほど気づけない** |
