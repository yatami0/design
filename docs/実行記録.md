...
## `/design-sync` 再同期（2026-08-08・工程3 後・人が起動 → [PR #12](https://github.com/yatami0/design/pull/12) `cfeaff5`）

**土台が Vite になってから 1 回目の同期。**デザインプロジェクトは <https://claude.ai/design/p/3acbb737-85fe-4098-95f4-c99070168ba1>（**31 → 36 部品**）。
一次記録は `.design-sync/NOTES.md` §工程3 後の再同期。ここには**工程3 が託した観測 3 点の答え**と、同期が独立に見つけたものを転記する。
...
| 検証の内訳 | 🟥 **`unchanged` 0 ／ `changed` 31 ／ `added` 5。carry-forward が全滅** |
| 採点 | 🟦 **56 story すべて `match`**（`close` 0・`mismatch` 0・factual failure 0）。**36 シート全部を画像判定**（sibling-trusted 0） |
| render check | 🟦 **36/36 clean**（`bad` / `thin` / `variantsIdentical` / `gridOverflow` すべて 0） |
| validate | 🟦 exit 0。警告は `[CSS_FROM_STORYBOOK]` 1 本（既知） |
| アップロード | 193 ファイル ＋ sentinel ＋ `_ds_sync.json`・**削除 0** |
| conventions header の実在検証 | 🟦 **名前のドリフト 0 件**（クラス 27・custom property 5・部品/Provider/hook 41 を全件実測）。🟥 **ただし「効いていない」claim が 1 件**（下記 ★） |

### ★ 工程3 が託した観測 3 点の答え

| # | 託した観測 | 答え |
| --- | --- | --- |
| ① | 新しいコア部品 6 件がカードになるか | 🟨 **5/6。**`Breadcrumb` `Tabs` `PeriodSelect` `FilterBar` `PageHeader` が追加。**`FilterField` だけカードにならなかった**（story が無い） |
| ② | `FilterField`（2 キー目）を converter がどう扱うか | 🟥 **測れていない。**カードが作られなかったので**当該分岐が一度も走っていない**＝「対象 0 件で緑」の型 → [OBS-0014](OBS/OBS-0014_同一ファイルに2キーを向けたときconverterが何をするか.md) |
| ③ | 題材 story がカードに湧かないか | 🟦 **湧かなかった。**🟥 **ただし自動ではない**——`titleMap` に `null` を手で 2 件足して除外した → [DR-0091](DR/DR-0091-claude-design-is-a-fourth-shipping-entrance.md) |

🟥 **③ が新しい発見になった**——`/design-sync` は `dist` を経由しない**4 本目の出荷入口**で、
判定軸は story の `title`（ディレクトリでも export でもない）。
[DR-0087](DR/DR-0087-fetching-belongs-to-the-subject-layer.md) の lint 2 本は**この経路を 1 行も見ていない**。
除外しなければ `[TITLE_UNMAPPED]` として扱われる＝**既定で落ちるのではない。**

### 🟥 同期が見つけたもの ① — carry-forward 全滅の原因は「型 import の 1 行」

`sourceKey` の入力は `globalSlice` / `componentSlice` / **story ファイルの `srcSha`** / owned preview / story 一覧。
config は 1 文字も動いていない（`keyRecipe` 7・`scriptsSha` も同一）。動いたのは **story 45 ファイル全部の 1 行目**——
工程1 の `@storybook/nextjs-vite` → `@storybook/react-vite`。

★ **型だけの import で、描画には 1 ピクセルも効かない。**それでも指紋は story ファイル全体なので**全件が `changed`**。
🟦 仕様どおり（指紋を「描画に効く部分だけ」にはできない）。
🟥 **土台を入れ替える工程の後は、再同期に全件再採点の時間を見込むこと**（今回 56 story）。

### 🟥 同期が見つけたもの ② — `SelectTrigger` の `width` は出荷物でも効いていない

header は `width: sm|md|lg` を教えており、**名前は全部実在する**——`w-field-sm/md/lg` は
`_ds_bundle.css` に別々の規則で載っており（`--container-field-*` = spacing×32/48/80）、`FIELD_WIDTH` も生きている。
🟥 **それでも `Select` の `Widths` story は 3 つとも同じ幅で描かれる。**原因は `w-fit` と tailwind-merge
（[DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md) と同一）。

★ **新しい角度が 1 つ増えた**——**header の claim は「名前としては真、効果としては偽」だった。**
header 検証は毎回「名前のドリフト 0 件」を確認してきたが、**名前の実在は効果を保証しない**＝
**検証の方法そのものへの指摘**。🟦 storybook 側も preview 側も同じなので**採点は `match`**（同期の忠実性は保たれている）。

🟦 [PR #11](https://github.com/yatami0/design/pull/11)（`1425e30`）が同じ箇所を直しているが
🟥 **未マージなので今回の出荷物には入っていない。**→ NOTES の見張り **#17**（マージ後の再同期で消えたことを確認する）。
🟥 **conventions header は書き換えていない**（[DR-0069](DR/DR-0069-adding-prohibitions-to-the-header-degraded-the-output.md)）——直すのはコード側。

### 🟨 その他（NOTES の見張り #18〜#20）

- **#18**: 土台を触る工程の後は carry-forward が全滅する（上記 ①）
- **#19**: 🟥 **`cfg.entry` は `src/index.ts` のまま**にした。工程1 で `dist/design.mjs` が実在するようになり
  converter に「本物の dist」を渡す選択肢が生まれているが、**出荷物の由来が変わる判断**なのでこの同期では動かさない。**次の工程で決める**
- **#20**: 🟦 **pnpm の回避策はもう要らない**（工程3 D12 の `allowBuilds` 宣言で `pnpm i --frozen-lockfile` が素直に通る。
  プレースホルダ `pnpm-workspace.yaml` は生えなかった）
- `[GRID_OVERFLOW]` 0・`[SPOT_CHECK]` 0・`[REFERENCE_STALE?]` 0（参照 Storybook を同じセッションで建て直した——risk #14 の読みが 3 回連続で的中）
- `[TOKENS_MISSING]` は「1 missing, below threshold」で tag が出ない（risk #13 据え置き）

### 🟦 順番について（結果的に解決した）

`fix/select-overlay-and-field-width` を同期の前に入れると「4 つ目の変数（トリガ幅 107 → 192px）が混ざる」と
懸念していたが、**同期が先に打たれたので起きていない。**むしろ**塞ぐ前の状態が出荷物に記録された**ので、
**次回同期が before / after の対照になる**（見張り #17 がそれを見る）。

---

## 工程4 — 一覧 → 詳細（編集あり）（2026-08-08）

> 計画は [手順書](手順/工程4_一覧から詳細_編集あり.md)。**本節は実測のみ。**
> 実行環境: Conductor worktree `brasilia`・ブランチ `brasilia`（`main` = `22440c8` の上）。

### P4-01 — 現況を撮る ＋ 予測の登録

- 🟦 **「環境の再現」の穴 ① は塞がった。**新しい worktree で `pnpm install --frozen-lockfile` が
  **`ERR_PNPM_IGNORED_BUILDS` を出さず・`pnpm-workspace.yaml` のプレースホルダも生やさずに exit 0**（2.5 s）。
  工程3 D12=B（`allowBuilds: esbuild/msw = false` を commit）の効果で、`.design-sync/NOTES.md` 見張り #20 の確認になった。
  → 🟨 **本工程のゲートも `./node_modules/.bin/` 直叩きで揃える**（工程3 と同条件で比較するため。理由は「穴」ではなく「同条件」に変わった）
- ゲート 6 本（`main` = `22440c8`）: `tsc` 🟦 緑 ／ `eslint` 🟥 **error 40 / warning 1（内訳まで一致**:
  no-arbitrary-value 31 / restrict-template-expressions 4 / no-confusing-void-expression 4 / set-state-in-effect 1 /
  warning は incompatible-library**）** ／ `vite build` 🟦 緑（**dist 65 ファイル・`design.mjs` 74,419 B**）／
  `prettier` 🟦 緑 ／ `cspell` 🟦 緑（**283 ファイル**）／ `storybook build` 🟦 緑（**story 45 ファイル・index.json 71 件**・全部 `type: story`・docs 0）
- 🟨 **`cspell` だけ handoff の表（280）と食い違った。**差分 3 は工程外コミット（`a61cbb4` / `1425e30` / `22440c8`）で足した md で、
  **新しい赤ではない**（Issues found: 0）。→ P4-10 で表を直す
- `find dist -type f | sort` を before として保存（K5 用・65 行）。`git status` はクリーン。素材層は **21 件**

#### レジストリの実測（★ 手順書起こし時・D1/D6/D8 の根拠）

`https://ui.shadcn.com/r/styles/new-york-v4/<name>.json` を直接引いた（2026-08-08）。

| item | `registryDependencies` | `dependencies`（外部） |
| --- | --- | --- |
| `form` | button, label | 🟥 **react-hook-form / zod / @hookform/resolvers** ＋ radix-ui |
| **`field`** | label, separator（**両方在庫**） | 🟦 **0 件** |
| `textarea` | — | 🟦 0 件 |
| `alert` | — | 🟦 0 件 |
| `sonner` | — | 🟥 **sonner ＋ next-themes** |
| progress / radio-group / switch / avatar | — | radix-ui（在庫） |

- ★ **段取り §工程4 D1 の二択（`Form` か自作か）が不完全だった**ことがここで確定した。**第 3 の選択肢（器だけ・依存 0）が実在する。**
- ★ **`sonner` の `next-themes`**: 工程1 で外した Next が戻る形。→ D8 = インライン表示（Toast は作らない理由を残す）

#### 🆕 P4-01 で見えたもの（D13 の追記につながった）

- `vite.config.ts` の `external` は**手書きで `package.json` の `dependencies` と 1:1** と冒頭コメントが宣言している
  （「依存を増減したらここも追随する」）。→ 🟥 **rhf / zod を `dependencies` に置くと「利用側の node_modules にある前提」を名乗る**ことになり、
  **D1=B と食い違う**。→ **§2 D13=B（`devDependencies` に置く）を追記してから進んだ**

#### 予測の登録（★ 測る前に書く・DR-0076）

🟥 **予測が実行されうるかを検算してから登録した**（[OBS-0014](OBS/OBS-0014_同一ファイルに2キーを向けたときconverterが何をするか.md) の失敗を繰り返さない）。

| # | 予測 | 結果（実測後に記入） |
| --- | --- | --- |
| 予1 | `shadcn add textarea field alert` で**既存 21 件は 1 行も変わらない**（素材層 diff 0 行が 8 回連続になる） | 🟦 **的中**（diff 0 行・`label` / `separator` は "Skipped"） |
| 予2 | K6 の eslint は **error 40 → 45 前後（+5）**。内訳は `no-arbitrary-value` が全部で、**`field` が大半（4 前後）**・`alert` 1 前後・`textarea` 0 | 🟥 **外れ（件数も内訳も）**——**+2**（40 → 42）。**`alert` 1（`no-arbitrary-value`）／ `field` 1（🆕 `no-unnecessary-condition`）／ `textarea` 0**。★ **ベースラインに一度も出たことのない規則が出た**（下記 P4-03） |
| 予3 | 題材（画面・スキーマ・story）は dist に **1 バイトも出ない**（K5。JS・`.d.ts`・静的の 3 本とも） | 🟦 **的中**（3 本とも 0 件） |
| 予4 | ★ **`.d.ts` に `react-hook-form` / `zod` / `@hookform` の型名が 0 件**（D1=B の帰結・Q6） | 🟦 **的中**（`dist/` 全体を grep して 0 件） |
| 予5 | ★ **画面に書く `className` は 0 件のまま**（Q4 の合格線。工程3 の実績を編集でも保つ） | 🟨 **一度 1 件になり、直して 0 件**——`Timeline` の `children` に `className="text-label"` を書いていた（**合成方針 §8-4 を自分で破っていた**）。口を `details: string[]` に変えて書式をコアへ戻した |
| 予6 | **`package.json` の `dependencies` が 1 件も増えない**（D13=B の帰結） | 🟦 **的中**（12 件のまま。rhf / zod / resolvers は全部 `devDependencies`） |
| 予7 | **`convert.ts` に復路（`toIssuePatch`）を足すだけで「3 点だけ」は保たれる**——画面に snake_case が 1 つも漏れない（Q5） | 🟦 **的中**（画面の snake_case は 0 件。ただし **`convert.ts` は「書き換わる 3 点」の 1 つなので、3 点の中身が増えた**——下記 Q5） |
| 予8 | ★ **予測していない箇所が 1 件以上出る**（DR-0076 ③ の様式） | 🟦 **的中・5 件**——① `shadcn add` が**対話プロンプトで止まった** ② `field` が**新種の赤**を持ち込んだ ③ `satisfies` の**主張が半分偽**だった ④ ★★ **Radix `Select` が束ねた値を 0 に潰していた**（K1-b が赤） ⑤ `titleMap` の key は**空白を落とした形**だった |

### P4-03 — shadcn 追加（Q3・K6・D6）

- 🟥 **`shadcn add textarea field alert` が対話プロンプトで止まった**——`field` の `registryDependencies` は
  `label` / `separator` で**両方在庫にある**ため、CLI が「`label.tsx` already exists. Overwrite?」を出して待った。
  **`alert` / `textarea` は書かれたが `field` は書かれないまま exit 0**（🟥 **終了コードでは分からない**）。
  → `printf 'n\n…' |` で **N を明示して**打ち直し、**`Created 1 file: field.tsx` ／ `Skipped 2 files: label.tsx, separator.tsx`** を確認。
  🟨 **工程3 は同じ状況で自動的に "Skipped" になっていた**（`button`）ので、**依存が既存かどうかで挙動が変わる**わけではなく、
  **`shadcn` の版か対象ファイルの中身の差**で分かれている（追わない・記録だけ）。
- 🟦 **素材層 21 → 24 件。既存 21 件の `git diff` は 0 行**（Q3。素材層無変更は **8 回連続**）。
- 🟥 **K6（ignore を足す前のゲート 6 本）で新種の赤が出た**——`error 40 → 42`:

| ファイル | 規則 | 中身 |
| --- | --- | --- |
| `ui/alert.tsx` L7 | `tailwindcss/no-arbitrary-value` | `has-[>svg]:grid-cols-[auto_1fr]` |
| `ui/field.tsx` L195 | 🆕 **`@typescript-eslint/no-unnecessary-condition`** | `uniqueErrors?.length == 1`（配列リテラルへの不要な optional chain） |
| `ui/textarea.tsx` | — | 0 件 |

  ★★ **`no-unnecessary-condition` は 8 手＋工程0〜3 のベースラインに一度も出ていない規則**。
  [DR-0010](DR/DR-0010-shadcn-invents-arbitrary-values.md)（shadcn 自身が任意値を発明している）は**任意値の話**だったが、
  **今回は型の厳しさ側で初めて赤が出た**＝ **赤の「種類」が増えたのであって件数が増えたのではない**。🟥 **ignore していない。**

### P4-04 — フォームの器（★ D1=B）＋ D12 の lint

- ★ **`field.tsx` の `FieldError` は `errors?: Array<{ message?: string } | undefined>` を受けるだけで、
  `react-hook-form` を 1 行も import していない**（実測）。rhf の `FieldErrors` は構造的にそのまま渡せる。
  → **D1=B の「コアは rhf を知らないまま器を提供できる」は、上流の実装がすでにそう作られている**ことの裏づけ。
- 🟥 **D12（コア 3 層は状態ライブラリを import しない）の赤テスト——3 検体とも発火した**:

| 検体の置き場 | import | 結果 |
| --- | --- | --- |
| `src/patterns/__probe.tsx` | `react-hook-form` | 🟦 赤（`no-restricted-imports`） |
| `src/components/Layout/__probe.tsx` | `zod` | 🟦 赤 |
| `src/components/ui/__probe.tsx` | `@hookform/resolvers/zod` | 🟦 赤 |
| **正の対照** `src/redmine/screens/__probe.tsx` | `react-hook-form`（型のみ） | 🟦 **緑**（題材は持ってよい） |

  検体を消して緑に戻したことも確認（`git status` クリーン）。
- 🟥 **自分のコードで `no-deprecated` を踏んだ**——`React.FormEventHandler` は @types/react 19 で
  **"FormEventHandler doesn't actually exist"** として deprecated。正は `SubmitEventHandler`。**lint が実装中に捕まえた。**

### P4-05 — `DescriptionList` / `Timeline`（Q3・D7=A）

- **D7=A の根拠を一次情報で裏取りした**（`raw.githubusercontent.com/ant-design/ant-design/master/components/<name>/index.en-US.md`）:

| 部品 | Ant Design の `group` |
| --- | --- |
| `descriptions` | **Data Display** |
| `timeline` | **Data Display** |
| `table` / `list` | Data Display（対照） |

  思想が DataDisplay を「Ant / Carbon の Data Display 由来」と書いている以上、**同じ棚に置くのが由来に忠実**。
- 🟥 **自分で任意値と数値の段を書きかけた**（[DR-0074](DR/DR-0074-we-wrote-the-same-deviations-ourselves.md) の形）——
  `Timeline` の初版に **`grid-cols-[auto_1fr]`** と **`mt-1`** を書いた。**lint に掛ける前に自分で気づいて書き直した**が、
  🟨 **「気づいたから消えた」は再現性が無い**（lint は製品層に効くので、放置していれば赤にはなった）。

### P4-06 — 復路の翻訳とスキーマ（★ Q5・D9=B）

- **`convert.ts` に `toIssueEdit`（往路の続き）と `toIssuePatch`（復路）を足した。**
  🟥 **`toIssuePatch` は「変わった項目だけ」を返す**——全項目 PUT は他人の編集を踏み潰す（K1-b の観測点）。
- 🟥 **`satisfies z.ZodType<IssueEdit>` の赤テストで、自分の主張の半分が偽だと分かった**:

| 壊し方 | `tsc` |
| --- | --- |
| フィールドを**消す** | 🟦 赤（TS1360） |
| フィールドの**型を変える** | 🟦 赤（TS1360） |
| フィールドを**足す** | 🟥 **緑のまま**（構造的部分型） |

  ★ 最初のコメントは「**増減したら赤になる**」と書いていた。**赤テストを打ったから捕まった**——
  [DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md)（書いたつもりで作用が無い）と同じ形を、**今回は書いた直後に測って捕まえた**。
  コメントは実測どおりの表に書き直した。

### P4-07 — 詳細画面と編集（★ K1・K2・K3・D5=A・D15・D16）

- **D15=B の実装**: `data.ts` の `STATUSES` / `PRIORITIES` を export し、
  `/issue_statuses.json` と `/enumerations/issue_priorities.json`（**どちらも実 API に在る**）を MSW に足した。
  🟦 **工程2 が PUT ハンドラに持っていたインライン複製も消えた**（表の複製 **3 → 1**）。
- 🟨 **story の `id` を間違えた**——生成データの課題 id は **1001 から**（`data.ts` L245）。`id: 1` は 404 になり、
  画面は「読み込みに失敗した」を出した。🟦 **画面が正しく失敗を見せたので、原因はすぐ分かった**（D8=B のインライン表示が効いた最初の例）。

#### ★★ 🟥 K1-b が赤になった — Radix `Select` が束ねた値を 0 に潰していた

**件名と進捗だけ変えて保存したのに、body が 4 項目だった**:

```
keys = [done_ratio, priority_id, status_id, subject]（期待: done_ratio, subject）
...
- **原因**: 課題（`fetchIssue`）と選択肢（`fetchIssueStatuses`）は**別々に飛ぶ**。
  **選択肢が届く前に `reset` が走ると `SelectItem` が 1 つも無い**ので、
  **Radix `Select` は「`value` に一致する項目が無い」と判断して `onValueChange("")` を発火する。**
- 🟥 **そして誰も気づけない形で終わっていた**——`status_id: 0` は `applyNamed` の表に無いので
  **モックは黙って何もせず 204 を返し、画面は「保存した」と表示した。**
  **実 Redmine なら 422 か不正な書き換え。**
- ★★ **K1-b はこれを狙って書いた検体で、狙いどおり赤になった。**
  「対象 0 件で緑」ではなく、**赤テストが仕事をした**形（通算 17 例目にはならなかった）。
- **塞ぎ方（D16=A・2 段）**: ① 選択肢が揃うまでフォームを出さない ② `onValueChange` の空文字を無視する。
  → **再実行で 8/8 PASS**（`keys = [done_ratio, subject]`）。
- 🟨 **コア側の規定にするかは OBS へ**（空文字を飲むと「選択を消す」用途が塞がるので、需要を見てから）。

#### K1〜K3 の結果（`tools/edit-probe.mjs`・Playwright）

| 検体 | 結果 | 証拠 |
| --- | --- | --- |
| K1-a PUT が 1 回だけ | 🟦 PASS | 回数 1 / `/issues/1001.json` |
| K1-b body は変えた項目だけ | 🟦 PASS（**1 度赤 → 塞いで緑**） | `keys = [done_ratio, subject]` |
| K1-c 画面が落ちていない | 🟦 PASS | console error 0 件 |
| K2-a 不正値では PUT が 0 回 | 🟦 PASS | 回数 0 |
| K2-b エラー文が両方出る | 🟦 PASS | 「件名は必須。」1 / 「進捗は 100 以下。」1 |
| K3-a 保存の知らせ（インライン） | 🟦 PASS | 「保存した」1 件 |
| K3-b 取り直しで履歴がある | 🟦 PASS | Timeline 4 件 |
| K3-c 取り直した値が編集後 | 🟦 PASS | 件名に `★編集` |

- 🟥 **`storybook build` はこの間ずっと exit 0 だった**（K1-b が赤のときも）。[DR-0048](DR/DR-0048-build-storybook-does-not-render.md) の再演 **3 度目**。
- 🟨 **rhf の `watch()` は `react-hooks/incompatible-library` が名指しで警告する**ので `Controller` に替えた
  （rhf 自身が制御コンポーネントに勧めている形が、たまたま警告も出さない形だった）。
- 🟨 **`handleSubmit` は Promise を返す**ので `void` で捨てないと `no-misused-promises` が赤。

### P4-08 — Q1 の答え（未決 #4・[DR-0092](DR/DR-0092-the-core-holds-the-vessel-not-the-state.md)）

DR に書いた。**DR-0088 の 2 問を編集の固有物 12 件に当て、裁けなかったものを名指しした。**

### P4-09 — 出荷面（K5・K7・K8・D4・D10）とゲート 6 本

- **K5: dist 65 → 74 ファイル（+9）。全部コア。**

| 増えたもの | |
| --- | --- |
| `.d.ts` +9 | `Alert` / `DescriptionList` / `Timeline` / `Field` / `Textarea` / `FormLayout` ＋ 素材層 3 |
| 題材（JS） | 🟦 **0**（`design.mjs` を `redmine\|issueSchema\|useIssue\|IssueDetail` で grep して 0 件） |
| 題材（`.d.ts`） | 🟦 **0** |
| 静的 | 🟦 **0**（`mockServiceWorker.js` 無し） |

- ★ **Q6: `dist/` 全体を `react-hook-form` / `@hookform` / `zod` で grep して 0 件。**
  **`dependencies` も 12 件のまま**（3 件は全部 `devDependencies`）＝ **D1=B / D13=B は出荷面でも成立した。**
- 🆕 **K7 と D4=B**: `tools/title-map-check.mjs` を新設（**ゲートではない**——同期を打つ前に人が回すもの）。
  🟥 **書いた初版が誤判定を出した**——`titleMap` の key は**空白を落とした形**（`A状態面` ⇔ `★ Review/A 状態面`）で、
  **正規化せずに突き合わせると生きている 6 件を「死んでいる」と報告する**。正規化を入れたら
  **本当の 1 件（`チケット詳細`）だけが残った** → 足して緑。
  🟨 **K7 の「足し忘れると何が起きるか」は前回同期の一次記録が既に答えていた**（`[TITLE_UNMAPPED]` として**警告に出る**＝静かには湧かない）。
  **それでも検査を置いたのは「警告は人が見落とすから」**であって、壊れるからではない。
- 🟦 **K8: 対象 0 件。**この工程では**トークン語彙を 1 語も足していない**（`COLUMNS` / `FIELD_WIDTH` / `INLINE_GAP` / `STACK_GAP` の再利用だけ）。
  `tokens.css` と `src/lib/tw-merge.ts` の diff は 0 行。→ **[DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md) の宿題 ① は今回は発火しない**（**打たずに緑にしないため明記する**）。
- 🟨 **story のタグに Storybook 既定の自動ドキュメント用タグを書きかけた**——repo の story タグ語彙は
  `own` / `vendor`（手8d の層タグ）だけで、そのタグはどこにも使われていない。
  **cspell が拾ったので辞書に足さず `own` に揃えた**（「辞書に足すのは固有名詞だけ」の運用どおり）。
...
| --- | --- | --- |
| `tsc --noEmit` | 🟦 緑 | — |
| `eslint .` | 🟥 **error 42 / warning 1** | **+2 は全部素材層**（`alert` の任意値 1・`field` の `no-unnecessary-condition` 1）。🟦 **自作分の赤は 0**（コアも題材も） |
| `vite build` | 🟦 緑 | **dist 74 ファイル**（+9・全部コア）・`design.mjs` **83,784 B**（+9,365） |
| `prettier --check` | 🟦 緑 | — |
| `cspell` | 🟦 緑 | **303 ファイル**（+20）・**辞書 0 語追加** |
| `storybook build` | 🟦 緑 | **story 49 ファイル / index.json 80 件**（+4 ファイル・+9 story） |

🟥 **`storybook build` の緑は描画も保存も保証しない**（今工程で 3 度目の実証）——K1〜K3 は Playwright（`tools/edit-probe.mjs`）。

### 工程4 の答え（Q1〜Q6）

| # | 答え |
| --- | --- |
| **Q1** | ★★ **コアは器だけを持ち、編集の状態は題材が持つ**（ユーザー判断「UI はできるだけ純粋に保つ」）。**[DR-0092](DR/DR-0092-the-core-holds-the-vessel-not-the-state.md)。**🟦 **成立した**——器の props は `label` / `htmlFor` / `required` / `description` / `error` / `children` の 6 つで止まり、`value` も `onChange` も生えなかった。lint（D12）で機械化・赤テスト済み |
| **Q2** | **制約は題材（zod スキーマ）、見せ方はコア（`error` は文字列 1 本）。**🟥 **「選択肢」は 2 種類あった**——絞り込みの語（`open`/`closed`/`*`・端点から取れない）と**実体の一覧**（`/issue_statuses.json` から取る）。**同じ「選択肢」でも出どころが違う**（D15） |
| **Q3** | **両方とも部品になった**（`DescriptionList` = 語彙 2 つで閉じた／`Timeline` = 器）。🟥 **ただし役割 9 カテゴリに席が無い**——`Field` を含め **3 件**。Ant の `group: Data Display` を根拠に置いた（**指摘 14**） |
| **Q4** | 🟦 **画面の `className` は 0 件・任意値 0 件**（工程3 の実績を編集でも保った）。🟨 **ただし一度 1 件になった**——`Timeline` の ReactNode の口に書式クラスを書いていた。**口を文字列配列に変えて書式をコアへ戻した**＝ **「編集で見た目の選択が増える」は起きたが、部品側に引き取れた** |
| **Q5** | 🟦 **保たれた**（画面の snake_case 0 件）。🟥 **ただし「3 点」の中身は増えた**——`convert.ts` に**復路 2 本**（`toIssueEdit` / `toIssuePatch`）が加わった。**「書き換わるのは 3 点だけ」は場所の主張であって量の主張ではない**ことが編集で明らかになった |
| **Q6** | 🟦 **無風だった**（`.d.ts` の外部型 0 件・`dependencies` 増 0 件・題材の混入 0 件）。**Q1 の答えが B だったから無風**であって、A なら [DR-0072](DR/DR-0072-no-passthrough-of-dependency-types.md) の再演になっていた。🆕 **4 本目の入口は検査で守るようにした**（D4=B） |


---

## `/design-sync` 再同期（2026-08-09・工程4 後・人が起動 → `main` 直コミット `a9424b9`）

**打つものが無かった回。**デザインプロジェクトは <https://claude.ai/design/p/3acbb737-85fe-4098-95f4-c99070168ba1>（**39 部品のまま変化なし**）。
一次記録は `.design-sync/NOTES.md` §工程4 後の再同期。ここには**台帳側に効くもの**だけ転記する。
...
| 検証の内訳 | 🟦 **`unchanged` 39 ／ `changed` 0 ／ `added` 0 ／ `removed` 0。carry-forward が全件効いた** |
| 採点 | — **シートは 1 枚も撮っていない**（capture は `empty_worklist`） |
| render check | 🟨 **skipped**（`[RENDER_SKIPPED]`）。**アンカー一致の no-change 再同期での既定挙動**＝ドライバ由来の想定内 warn |
| validate | 🟦 exit 0。警告は `[CSS_FROM_STORYBOOK]` 1 本（既知） |
| アップロード | 🟦 **`upload.any: false`**（`bundle` / `styling` / `aux` すべて false・`deletePaths` []）。**`finalize_plan` を打っていない** |
| conventions header の実在検証 | 🟦 **名前のドリフト 0 件**（クラス 27・custom property 2・props と union 値・否定の主張を全件実測。`window.Design` は **164 export**） |

### 🟥 台帳と remote が食い違っていた

**アンカー（`_ds_sync.json`）を取ったら既に 39 部品**で、`DescriptionList` / `Timeline` / `FormLayout`（工程4 の新設）が載っていた。
**工程4 の後に既に同期が打たれ、記録されていない。**`git log -- .design-sync/` の最後は `5b96c77`（工程4）で、
動いたのは `config.json` の 10 行だけ。**handoff は「次: `/design-sync`（人）」のままだった。**

★ **教訓: 現在地の正本は NOTES ではなく remote のアンカー。**
再同期の最初に `get_file _ds_sync.json` を取れば、**台帳が何を言っていようと本当の現在地が 1 発で分かる**（今回それで確定した）。

### 🟦 見張り #17 の決着 — `SelectTrigger` の `width` は出荷物で効くようになった

[PR #11](https://github.com/yatami0/design/pull/11)（`1425e30`・[DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md) を塞いだもの）の効果を、
**出荷物そのもの**（`ds-bundle/components/selection/Select/Select.html?story=Widths`）を Playwright で開いて実測した。

| | 塞ぐ前（2026-08-08 の同期） | 今回（2026-08-09） |
| --- | --- | --- |
| `w-field-sm` | 112.31 px | 🟦 **128 px** |
| `w-field-md` | 113.59 px | 🟦 **192 px** |
| `w-field-lg` | 104.88 px（**sm > lg の逆転**） | 🟦 **320 px** |

`--container-field-*` の spacing×32/48/80 どおり。emit されるクラスから **`w-fit` も消えている**（tailwind-merge が畳むようになった）。

★★ **これは「検証の方法そのものへの指摘」への答え。**
2026-08-08 の同期は「**名前の実在は効果を保証しない**」を指摘して終わっていた（header の claim は名前としては真・効果としては偽）。
🟦 **今回は名前確認で止めず px を読んだ**——**効果まで測れる claim は測る**、が同期側の作法として 1 つ増えた。

### 🟥 検証の向きを逆にしたら 8 件落ちていた → [OBS-0016](OBS/OBS-0016_headerが名指ししていない出荷部品8件をどう埋めるか.md)

header 検証は毎回「**header → 出荷物**」（名前が実在するか）を全件見てきたが、
**「出荷物 → header」（実在するものが名指しされているか）は一度も数えていなかった。**数えたら **8 件**落ちていた:

`DescriptionList` `Timeline` `Breadcrumb` `Tabs` `FilterBar` `FormLayout` `PageHeader` `PeriodSelect`

header は「2 層とも実在する部品だから自分で markup を書くな」と言っているので、**列挙に無い部品は design agent が手組みに回る**
（[OBS-0013](OBS/OBS-0013_部品があるのにインラインスタイルで組んだ.md) と同型）。
🟥 **ただし単純に足せない**——[DR-0069](DR/DR-0069-adding-prohibitions-to-the-header-degraded-the-output.md) で
**842 バイト足したら「触っていない箇所」が壊れた**実績がある。**埋め方そのものが問い**なので OBS-0016 に起票し、**header は書き換えていない**。

### 🟦 見張り #19 の決着 — `cfg.entry` は据え置き

工程4 の判断ポイント **D3 = B** により、`cfg.entry` は `src/index.ts` のまま。再同期は `--entry src/index.ts` で回す。
倒すと ① [DR-0083](DR/DR-0083-lib-build-silently-strips-use-client.md)（dist は `'use client'` を警告 0 で失う）を出荷物に持ち込む
② `pnpm build` が同期の前提になる。🟥 **判定軸は `title` のままなので「dist を渡す」と「境界が守られる」は別**（[DR-0091](DR/DR-0091-claude-design-is-a-fourth-shipping-entrance.md)）。

### 🟨 環境の再現（3 回目の確認）

Conductor の worktree はフレッシュ clone と同じ（`.ds-sync` / `dist` / `sb-reference` / `.cache` は全部 gitignore で存在しない）。
NOTES の手順どおりで**回避策なしに通った**——`pnpm i --frozen-lockfile` は exit 0、**プレースホルダ `pnpm-workspace.yaml` は生えなかった**（見張り #20 の 3 回目の確認）。

---

## 部品1 — 完成バーを機械で閉じる（2026-08-09・[手順書](手順/部品1_完成バーを機械で閉じる.md)・ブランチ `design-ui-component-procedure`）

**部品軸の 1 本目。**同期軸を一旦置き、部品そのものを測る線に切り替えた（ユーザー指示）。
🟨 **ユーザー判断で B1-06（39 件全部にバーを通す）を部品2 と並行に回し、B1-05 までで区切った**——
「早く作っている状態に戻る」ことを優先。**バーは新部品にも最初から掛かるので、並行でも抜けない。**

### B1-02 — 検証エンジン（`@storybook/addon-vitest`）

| 観測 | 結果 |
| --- | --- |
| 導入 | `@storybook/addon-vitest@10.5.4` / `vitest@4.1.10` / `@vitest/browser@4.1.10` / `@vitest/browser-playwright@4.1.10`（厳密ピン・全部 `devDependencies`） |
| `dependencies` | 🟦 **0 増**（工程4 D13 のとおり出荷物に影響なし） |
| 結果 | 🟦 **49 ファイル / 80 テストが 10.6 秒**（`index.json` の 80 件と一致＝取りこぼし 0） |
| 未決 #14 | 🟦 **決着**（入れた）。工程4 D11 が指定した条件（Playwright 3 工程連続）は達成済みで、唯一残っていた論点「移送コスト」は**手9 廃止（DR-0078）＋ 工程4 D13** で消えていた |

- 🟥 **K1 発火**: わざと throw する story で exit 1・原因行まで出た。**[DR-0048](DR/DR-0048-build-storybook-does-not-render.md)（`storybook build` は story が落ちても exit 0）を落とす経路が初めてできた。**
- 🟥 **`vitest` は `vite.config.ts` を継承しない。**初回 49 中 46 が `@/` 解決に失敗（原因は 1 つ）。**写しを作らず本体から `resolve.alias` を引いた**（`'@'` を 2 箇所に書くと [DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md) の宿題① と同じ形を増やす）。
- 🟥 **`.storybook/vitest.setup.ts` は置かないのが正しい。**addon 10.3+ は preview の注釈を自動適用し、`setProjectAnnotations` を自前で置くと「競合回避のため自動適用をスキップ」して**逆に効かなくなる**（実測）。
- ★ **D9（実行中に §2 へ追記して実施）: 日本語ファイル名の story は addon-vitest が 1 件も変換できない。**中身そのまま・名前だけ ASCII の複製は通ると **1 変数で実測**（title・tags・`component` の有無・NFC/NFD は全部つぶした。名前は **NFC** で macOS の NFD 問題ではない）。`★ Review` の 7 ファイル / 8 story を ASCII 名へ `git mv`（**title は日本語のまま**）。
  🟥 **同じ形の 2 例目**——[§手5](実行記録.md) に「自作の静的サーバが percent-encoding をデコードせず日本語ファイル名の chunk が 404」がある。**日本語ファイル名は検証装置を壊す。**

### B1-03 — バーの定義（[`docs/部品の完成バー.md`](部品の完成バー.md)）

**7 面のうち機械が 4・人が 3。**「緑なら完成」ではなく「**機械が持てる面と人が見る面を割ったもの**」。

★★★ 🟥 **バーの形を決めたのは、書く前に踏んだ罠 3 ——観測範囲が描画範囲と一致していなかった。**
`Dialog/Open` は「描画されない story」として `cfg.overrides.Dialog.skip` に入っていたが、**Playwright で直接見たら描画されている**:

```
#storybook-root → 空（0 文字）
#radix-_r_0_    → [role=dialog]「開いた状態 / … / Close」
```

**`sb-error: no storybook root content` も `/design-sync` の比較ハーネスも `#storybook-root` だけを見ている**ので、
**portal に出る部品は丸ごと観測から消えていた。**🟨 `.design-sync/NOTES.md` は `Tooltip` で同じことに気づいていたが**固有の癖として処理していた。系統的な問題。**
→ NOTES の skip の根拠を訂正した（**skip 自体は当面そのままでよいが、偽の根拠を次の同期に持ち込まない**）。

🟦 **ただし範囲を広げても部品の欠陥件数は動かなかった**——`document` 走査で増えた 96 件は**全部 `region`**（iframe に `<main>` が無い harness 由来）で、`region` を除けば root 走査と**完全一致**。

### B1-04 — a11y の切り分け（★ この回の芯）

**出荷物の棚 65 story・違反 88 件**（critical 11 / serious 77）を**色の組**で畳んだ。

★★ **77 件の `color-contrast` は 7 種類しかなく、全部トークンの組だった。**

| 件数 | 色の組 | 比（要 4.5） | 部品数 |
| --- | --- | --- | --- |
| **44** | `--muted-foreground` `#8a8a8e` on 白 | **3.43** | **13** |
| 11 | warning `#ff9500` on その fill | **1.93** | 4 |
| 11 | neutral `#828287` on その fill | 3.23 | 3 |
| 5 | success `#34c759` on その fill | **1.94** | 3 |
| 3 / 2 / 1 | destructive `#ff3b30` on 白 / fill | 2.86〜3.54 | 1〜2 |

🟥 **部品ごとの欠陥ではなく、① 層の配色の決定が N 箇所に出ているだけ。部品をどう直しても消えない。**
＝ **[指摘 8](共通コンポーネント思想への指摘.md)（① 層は「値の作り方」を分類しない）の 2 例目**（手順書の予測どおり）。
→ **この回では直さない**（配色を動かすと 5 画面全部が動く）。**[OBS-0017](OBS/OBS-0017_意味色とfillの対比が全滅している.md) に積んだ。**

### B1-05 — critical 11 件を返した（🟦 **11 → 0**）

🟥 **原因は「名前が無い」ではなく「`role="combobox"` は内容から名前を取らない」だった。**
実測: `SelectTrigger` には**「ステータス」という文字が見えている**。それでも名前は空。
**推測で直さずに測ったから分かった**——見えている文字を名前だと思い込むと直し方を間違える。

| 直した場所 | 件数 | 中身 |
| --- | --- | --- |
| 🟥 `FilterField`（Pattern・**本物のバグ**） | 3 | **ラベルを描いていたのに紐づいていなかった。**`<label>` で包むだけで直る（`<button>` は labelable・暗黙の関連付けは最初の labelable 子孫を拾うので `Stack` を挟んでも届く）。**id も `cloneElement` も要らない** |
| `PeriodSelect`（製品層） | 2 | 既定の `aria-label="期間"` を持たせた。**単体で置いても壊れないのが自己完結した部品の責務**（[DR-0070](DR/DR-0070-product-layer-boundary-rule.md)）。`'aria-label'?: string` で上書き可 |
| story 側（`Select` / `Checkbox` / `Input`） | 6 | **素材の素の Select は使う側が名前を与えるもの。**story がそれを見せていなかった＝ **story の欠陥**。`Checkbox/Default` は裸で、同じファイルの `States` は `Label + htmlFor` で正しく組んでいた |

- 🟦 **バーを機械で効かせた**（`preview.tsx` の `a11y.test` を `'todo'` → `'error'`）。`color-contrast` と `region` は **rule 単位で無効化**（消したのではなく、**数える場所を `.context/a11y-detail.mjs` と OBS-0017 に移した**）。
- ★ **効かせた瞬間にもう 1 件捕まえた**——`★ Review/I 層の比較` の裸の `Checkbox`。🟦 **a11y の母数からは外している棚**（D4=B）だが、**バーの適用範囲は母数より広い**（意図どおり）。
- 🟦 **K5 成立（3 段）**: `PeriodSelect` の `aria-label` を外す → **赤**（`button-name`）→ 戻して**緑**。

### ゲート（`main` = `923004a` のベースラインと比較）

| ゲート | 結果 |
| --- | --- |
| typecheck | 🟦 緑 |
| lint | 🟦 **error 42 / warning 1・内訳まで一致** |
| build | 🟦 緑・dist **74**・`design.mjs` **83,875 B**（+91・`aria-label` と `<label>` のぶん） |
| format / spell / storybook | 🟦 緑・spell 312・story **49 / index 80** |
| 🆕 **a11y**（出荷物の棚） | 🟦 **critical 0**（11 → 0）／ 🟨 **serious 77**（全部 `color-contrast` = OBS-0017） |
| 🆕 **story test** | 🟦 **49 ファイル / 80 テスト緑** |

🟥 **一度 error 43 になった**——`.context/` の使い捨てスクリプトが `no-unused-vars` を持ち込んだ。
**prettier では塞いだのに eslint では塞いでいなかった**＝ **同じ穴を 2 度踏んだ**（[DR-0040](DR/DR-0040-frame-leaks-when-a-layer-is-added.md) の 5 例目の続き）。
🟦 **ゲートがあったから「新しい赤」として捕まえられた。**

---

## 部品2 — 役割 9 カテゴリの充足（2026-08-09・[手順書](手順/部品2_9カテゴリの充足.md)・ブランチ `semarang`）

**部品軸の 2 本目。**[部品1 D1](手順/部品1_完成バーを機械で閉じる.md) = **C → B** の **B**。
★ **この回の芯は Q1**——[完成バー](部品の完成バー.md)は**後から当てる検査**として使われたことしかない。
**最初から通す**のは本回が初めてで、**順序が逆になる。**

### C2-01 — 現況と、実行前に登録した予測

**ゲート（`main` = `4f65a46`＝ PR #14 マージ後）を実測した。**

| ゲート | 実測 | 手順書 §1.3（`923004a`）との差 |
| --- | --- | --- |
| typecheck | 🟦 緑 | — |
| lint | 🟥 **error 42 / warning 1** | 🟦 一致 |
| build | 🟦 緑・dist **74**・`design.mjs` **83,875 B** | 🟨 **+91 B**（部品1 B1-05 の `aria-label` と `<label>` のぶん・**新しい赤ではない**） |
| format | 🟦 緑 | — |
| spell | 🟦 **312 ファイル** | 🟨 **+4**（部品1 が足した md。**私の変更で動いた数ではない**） |
| storybook | 🟦 緑・**49 ファイル / 80 件** | 🟦 一致 |
| 🆕 story test | 🟦 **49 / 80 緑**（10.3 秒） | — |
| 🆕 a11y（出荷物の棚） | 🟦 critical **0** ／ 🟨 serious **77** | — |

**lint 42 件の内訳**（K4 の突き合わせ先）:

| 規則 | 件数 |
| --- | --- |
| `tailwindcss/no-arbitrary-value` | **32** |
| `@typescript-eslint/restrict-template-expressions` | 4 |
| `@typescript-eslint/no-confusing-void-expression` | 4 |
...
🟨 **`no-arbitrary-value` 32 件は素材層 24 ファイル中 12 ファイルから出ている**
（`sidebar` 11 / `tabs` 4 / `button` 4 / `calendar` 3 / `tooltip` 2 / `card` 2 / `select` `dropdown-menu` `dialog` `checkbox` `badge` `alert` 各 1）。
★ **残り 12 ファイルは 0 件**＝ **「素材を入れたら必ず任意値が出る」ではない。**

🟨 **環境で 1 件詰まった**——`pnpm spell` は `/usr/local/bin/node`（22.16.0）だと
`Unsupported NodeJS version` で exit 1 になる。`mise.toml` は `node = "24"` を指定しており、
`mise exec -- pnpm spell` なら通る。🟦 **repo の赤ではなく shell が mise を通していないだけ**（以後 `mise exec` で回す）。

#### 🟥 実行前に登録した予測（`shadcn add` を打つ前に書いた）

| # | 予測 | 根拠 | 検算（実行されうるか） |
| --- | --- | --- | --- |
| **予1** | 既存 24 件の diff は **0 行** | 8 手＋工程0〜4 の連続記録 | 🟦 **実行されうる**——工程4 で `field` の regDeps が `label.tsx` の上書きを聞いてきた前例があり、**5 件の regDeps は全部 `[]`** なので今回は聞かれないはず |
| **予2** | `dependencies` は **0 増** | レジストリ実測（5 件とも `dependencies: []`） | 🟦 実行されうる |
| **予3** | 🟥 **`no-arbitrary-value` が +3**（幅 +2〜+5） | 素材 24 件中 12 件が 0 件・単純 primitive は 1〜2 件が相場（`checkbox` `badge` `alert` `select` 各 1）。**最有力は `switch`**（thumb の `translate-x-[calc(...)]`）と `slider` | 🟥 **外れうる**——`radix-nova` は既定 style と中身が違う可能性がある |
| **予4** | ★ **バーは一発では通らない**（Q1） | **`switch` / `slider` / `radio-group` は素で置くと名前が無い**（`Select` と同じ `role` 由来の穴）。バーの面②は「critical 0」だが、**バーは「story にラベルを付けろ」とは書いていない** | 🟦 **実行されうる**（`a11y.test: 'error'` が機械で落とす） |
| **予5** | 製品層の窓口は **5 件とも素通し**（Q5） | 手3 D1=(c)。5 件とも「欠落品」ではなく「既定値の上書きが要らない素材」 | 🟥 **外れうる**——`Progress` は `value` の既定、`Avatar` は fallback の扱いで割れる可能性 |
| **予6** | Q3 の 2 問は **8 件中 6 件以上で「作れ」に倒れる** | `Statistic`（値＋ラベル＋増減）・`Stepper`（段の並び）・`Tag`（≒ Badge）は**有限の語で言える** | 🟦 実行されうる |
| **予7** | 🟥 **予測していない箇所が 1 件以上出る** | 工程4 で 5 件的中した枠 | — |

### C2-02 — `shadcn add` ×5（K1・K3・K4）

`radio-group` / `switch` / `slider` / `progress` / `avatar` を 1 コマンドで追加。**対話プロンプトは出ず、5 ファイルが書かれた**（工程4 の `field` と違い regDeps が全部 `[]` だったため）。

| 検体 | 結果 |
| --- | --- |
| **K1** 既存 24 件の diff | 🟦 **0 行**（`git status` は新規 5 件のみ）。`pnpm-lock.yaml` / `components.json` も 0 行 |
| **K3** `dependencies` | 🟦 **0 増**（12 件のまま）・`@base-ui/*` は **0 件** |
| **K4** `no-arbitrary-value` | 🟥 **予3 が外れた**——予測 **+3**（幅 +2〜+5）に対し実測 **+7** |

#### 🟥 予3 が外れた — 内訳が予測と違った

| ファイル | 増分 | 中身 |
| --- | --- | --- |
| 🟥 **`switch.tsx`** | **6** | `data-[size=default]:h-[18.4px]` / `w-[32px]` / `data-[size=sm]:h-[14px]` / `w-[24px]` ＋ `translate-x-[calc(100%-2px)]` ×2 |
| `slider.tsx` | 1 | `transition-[color,box-shadow]` |
| `radio-group` / `progress` / `avatar` | **0** | — |

★ **`switch` 1 件で予測の全量を使い切った。**しかも中身が予測と違う——
**当てにいったのは `translate-x-[calc(…)]`（2 件で的中）だが、残り 4 件は生のピクセル値**で、
🟥 **`h-[18.4px]` は非整数**。[DR-0010](DR/DR-0010-shadcn-invents-values.md)（shadcn 自身が任意値を発明する）の
**いちばん強い形**——`18.4px` はトークンにも段にも対応しない、由来の説明できない数。

🟥 **`progress.tsx` は別の規則を持ち込んだ**——`@typescript-eslint/restrict-template-expressions` **+1**
（`style={{ transform: \`translateX(-${100 - (value || 0)}%)\` }}` ＝ **インラインスタイル**）。
★ **工程4 で `field.tsx` が `no-unnecessary-condition` を持ち込んだのと同じ形**で、**2 回目**。
🟨 **合計は error 42 → 50（+8）。ignore していない。**

#### 🟥 副産物: `slider.tsx` の `bg-white` はどの検査にも掛からなかった（→ D9）

thumb が **`bg-white`**（テーマを持たない不透明色。`.dark` でも白のまま）。
**`no-arbitrary-value` は角括弧しか見ず**、`PRIMITIVE_COLOR` は `(slate|gray|…|rose)-[0-9]` なので
**数字を持たない `white` / `black` は 1 つも当たらない。**

**赤テスト（塞ぐ前）**: 製品層に `bg-white` `text-black` `bg-black/10` `bg-gray-500` を並べて lint →
🟥 **`bg-gray-500` だけが落ち、他 3 つは素通り。**
**塞いだ後**: 🟦 **4 つとも落ち、`bg-card` / `text-muted-foreground` は緑のまま。**
🟦 **新しい赤は 0**（`bg-black/10` は `dialog.tsx` / `sheet.tsx` にあるが、この規則は素材層を `ignores` している）。

### C2-03 — 製品層の窓口（D4=B）／ C2-04 — story（D5=B）

- **窓口 5 本** ＋ `src/index.ts` の export ＋ `.design-sync/config.json` の `componentSrcMap`（44 → 49）
- **story 5 ファイル / 15 件**（バーの面③ を最初から置き、**「対象外」には全部理由を書いた**）

### C2-05 ★ 完成バーに通した（Q1・K2）— **一発では通らなかった**

**15 story 中 13 が一発で通り、2 が落ちた。落ちた 2 つは同じ 1 つの原因。**

#### 落ちた 2 件 — `Slider` に名前を与える口が素材層に無い（→ D8）

```
Slider/Default, Slider/States → aria-input-field-name（axe・serious）
```

🟥 **`<Slider aria-label="進捗率">` は Root の `<span>` に載るだけで thumb に届かない。**
名前が要るのは `span[role="slider"]` のほうで、素材の `Slider` は
Root / Track / Range / Thumb を **1 関数の中で閉じており、thumb に渡す口が 1 つも無い。**

★★ **これは [DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md) と同じ形の 2 例目**——
**型は `aria-label` を受け、lint も story も緑で、作用だけが無い。**

🟦 **`Slider/Range` だけ通ったのは Radix が自動で名前を入れるから**——
`getLabel(index, total)` は **摘み 1 つなら `undefined`、2 つなら `Minimum` / `Maximum`、3 つ以上なら `Value n of m`**
（`@radix-ui/react-slider` の実装を読んで確認）。🟥 **自動の名前は英語**で、日本語の画面では通っても読めない。

#### 🟥 D8=B（製品層で組み直す）を実装したら、機械が拒んだ

**素材の className をそのまま製品層へ持ち上げると、同じ文字列が違法になる:**

| 持ち上げたクラス | 素材層 | 製品層 |
| --- | --- | --- |
| `size-3` | 🟦 合法（`ignores: ['src/components/ui/**']`） | 🟥 **`no-restricted-syntax`**（数値の段＝ primitive） |
| `transition-[color,box-shadow]` | 🟥 赤 1 件（ベースライン内） | 🟥 **同じ赤が製品層に移るだけ** |

★★ **手3 D1=(c)「欠落品は製品層で作る」と 手3 D4=B′「製品層は数値の段を使えない」が正面から衝突する。**
直すには **① 層に 12px の語彙を足す**ことになり、**この回が持てる判断ではない**
（部品1 D3 が ① 層の決定を明示的に後回しにしている）。
🟨 **工程4 の [指摘 12](共通コンポーネント思想への指摘.md)（分類どおりに置こうとすると機械に阻まれる）の 2 例目。**

→ **D8=A に切り替え、素材層 `slider.tsx` を 2 箇所だけ触った**（`aria-label` を thumb へ ／ `bg-white` → `bg-background`）。
🟨 **素材層を触ったのは 8 手＋工程0〜4 で初めて。**
🟥 **ただし K1 は「0 行」と報告する**——`slider.tsx` は**同じコミットで新規追加されたファイル**なので、
git には diff が出ない。**指標は字面どおり緑だが、実体（shadcn の出力そのまま）は破れている。**
→ **上流との差分を実測して残す**（`https://ui.shadcn.com/r/styles/radix-nova/slider.json` と比較して **3 箇所**。
うち 1 箇所は CLI が書き換えた import path なので、**我々の変更は 2 箇所**）。

#### ★★★ そして面④ を測ろうとしたら、測定器のほうが壊れていた（→ D10・[DR-0094](DR/DR-0094-the-bar-engine-ran-without-any-css.md)）

バー面④「語彙 prop の効果」を `play` 関数で実装した最初の実行で、3 件が落ちた。
...
★ **0 / 16px / 透明はいずれも「素の HTML の値」**（中身の無い `<button>` は幅 0、`<span>小</span>` は文字 1 つぶん、
背景指定が無ければ透明）＝ **CSS が 1 行も当たっていない状態と完全に一致する。**
🟥 **`waitFor`（1 秒ポーリング）でも直らなかった**＝ **描画待ちの競合ではない。**

**原因は 1 つ**: `vitest.config.ts` が `vite.config.ts` から **`resolve.alias` だけ**を引いており、
**`@tailwindcss/vite` を引いていなかった。**`preview.tsx` は `globals.css` を import しているが、
中身は `@import 'tailwindcss'` で、**それを展開するのがそのプラグイン。**

★★★ **`vitest.config.ts` の冒頭コメント自身が「jsdom は CSS を計算しないので実ブラウザにする」と書いている。
理由に挙げた当のものが、実ブラウザの中で動いていなかった。**

🟦 **部品1 の「critical 0」は無効にならない**——落とした 11 件は全部 `button-name` / `label` 系で、
**名前の有無しか見ず CSS に依存しない。**依存するのは `color-contrast`（rule 単位で無効化済み）と面④ だけ。
→ `plugins` に `tailwindcss()` を 1 行足して **95 テスト全部が緑。**

#### 🟨 面④ を `play` で書くと `no-floating-promises` が 12 件出る

`storybook/test` の `expect` は instrumenter を通るので **Promise を返す。**
🟦 **赤テストで確かめた**——`await` 無しで故意に偽の assertion を置いても**テストは落ちた**
（instrumenter が内部で await している）＝ **黙って素通りする形ではない。**
それでも **12 件は「新しい赤」**なので `await` を付けて 0 に戻した（**ignore していない**）。

#### 🟦 バーの赤テスト（K5 相当）

`slider.tsx` から `aria-label` の受け渡しを**外す → 赤 2 件**（`aria-input-field-name`）→ **戻して緑。**
**バーは赤くできることを確認してから運用している。**

### C2-06 — Q3: shadcn に無い部品に [DR-0088](DR/DR-0088-core-subject-boundary-is-decided-by-two-questions.md) の 2 問を当てた（D3=B）

**2 問**: ① Redmine を知らない repo でも意味が通るか ② 見た目・振る舞いの選択を有限の語で言えるか。

| 部品 | 問① | 問② | 2 問の結論 | 🟥 それでも作らない理由 |
| --- | --- | --- | --- | --- |
| **IconButton** | 通る | **言える**（アイコン＋サイズ＋tone） | **作れ** | 🟨 **`Button` の 1 語**（`size="icon"`）で足りるかを先に測っていない。**部品を足す前に既存 prop の余地を数える** |
| **FAB** | 通る | **言える**（位置＋サイズ） | **作れ** | 🟥 **画面の隅に固定する部品は、置き場（Template）と対で決まる**。`AppShell` に置き場が無い |
| **FileUpload** | 通る | 🟥 **言えない**（進捗・失敗・再試行・複数・D&D の合成） | **器だけ** | 🟥 **器だけ作っても中身が無い**——[DR-0092](DR/DR-0092-the-core-holds-the-vessel-not-the-state.md) の「器」は**中身を使う側が持てる**場合に成立する。アップロードは**通信の状態**を持つので、コアが持てるのは枠だけ＝ **今の形では意味を成さない** |
| **List** | 通る | **言える**（項目＋区切り＋密度） | **作れ** | 🟨 **`item` が上流に在る**（regDeps は `separator`＝在庫）。**自作する前に既製を測る**のがこの repo の順序 |
| **Tree** | 通る | 🟥 **言えない**（展開状態・遅延読込・選択モデル） | **器だけ** | 🟥 **状態を持つ**＝ [DR-0092](DR/DR-0092-the-core-holds-the-vessel-not-the-state.md) が「コアは状態を持たない」と決めた側。**器の形が定まらない** |
| **Tag** | 通る | **言える**（文言＋tone＋削除可否） | **作れ** | 🟥 **`Badge` と `StatusPill` が既に在る**。**3 つ目を足す前に「3 つの違い」を言えないといけない**——言えていない |
| **Statistic** | 通る | **言える**（値＋ラベル＋増減） | **作れ** | 🟨 **`DescriptionList` との差が「増減の矢印」だけ**。工程7（EVM）が数字の並べ方を要求するので、**そのとき形が決まる** |
| **Stepper** | 通る | **言える**（段の並び＋現在位置） | **作れ** | 🟥 **5 画面のどれにも段階が無い**（一覧・詳細・ガント・EVM・稼働表）。**置く場所が無い** |
| Text/Heading | 通る | **言える**（`--text-*` の語彙） | **作れ** | 🟥 **[指摘 8](共通コンポーネント思想への指摘.md) と正面衝突**——① 層の語彙をそのまま部品にすると、**同じ決定が 2 箇所に写る** |
| Icon | 通る | **言える**（名前＋サイズ） | **作れ** | 🟦 **`lucide-react` が既に `dependencies`** にあり、**素の `<IconName />` で足りている**（工程3・4 で不足 0 回） |
| Image | 通る | **言える**（比率＋fit＋fallback） | **作れ** | 🟥 **`aspect-ratio` も `next/image` も無い**。**比率の語彙は ① 層の決定**で、この回が持てない |

#### ★★ Q3 の答え — **予6 が的中し、そして 2 問の限界が見えた**

**11 件中 9 件が「作れ」に倒れた**（予測は「8 件中 6 件以上」）。
🟥 **2 問だけでは「作るな」を一度も言えない。**
★★★ **2 問には「需要」の項も「既存で足りるか」の項も無い。**
**上の「作らない理由」11 件は、どれも 2 問の外から来ている**——
既存 prop の余地（IconButton）／置き場（FAB・Stepper）／既製品（List・Icon）／
重複の説明責任（Tag）／① 層との二重管理（Text・Image）／状態（FileUpload・Tree）。

→ 🟥 **これは [DR-0088](DR/DR-0088-core-subject-boundary-is-decided-by-two-questions.md) への差分要求。**
2 問は **「コアか題材か」の境界**は裁けるが、**「作るか作らないか」は裁けない**——
**別の問いであることが、需要が無い状態で当てて初めて分かった。**
（これまでの判定は全部**画面が要求したから**始まっていたので、需要の項が要ることに気づけなかった。）

### C2-08 — ゲートと台帳

| ゲート | ベースライン（`4f65a46`） | 実測 | 判定 |
| --- | --- | --- | --- |
| typecheck | 緑 | 緑 | 🟦 |
| lint | error **42** / warning 1 | 🟥 error **50** / warning 1 | 🟥 **+8**（`no-arbitrary-value` +7 ＝ switch 6・slider 1 ／ `restrict-template-expressions` +1 ＝ progress）**全部素材層・自作分 0** |
| build | dist **74**・`design.mjs` **83,875 B** | dist **84**・**91,322 B** | 🟨 +10 ファイル（`.d.ts`）・**+7,447 B** |
| format | 緑 | 緑 | 🟦 |
| spell | 312 | **329** | 🟨 +17（本回の md・**新しい赤ではない**）。辞書 **+1 語**（`instrumenter`） |
| storybook | 49 ファイル / 80 件 | **54 / 95** | 🟨 +5 / +15 |
| **story test（バー）** | 80/80 緑 | 🟦 **95/95 緑** | 🟦 |
| **a11y critical**（出荷物の棚） | **0** | 🟦 **0** | 🟦 **バーを保った** |
| **a11y serious**（同） | 77 | 🟨 **83** | 🟨 **+6・全部 `Avatar` の 1 色組**（→ OBS-0017 の 8 組目） |
| K5（出荷入口 4 本） | — | 🟦 `title-map-check` OK（story 54 題）・`dependencies` 12・題材の混入 0 | 🟦 |

🟨 **a11y の母数の注記**: 本回は `document` 走査（部品1 は `#storybook-root`）にしたため、
`landmark-one-main` 74 件 / `page-has-heading-one` 77 件が新たに見えた。
🟦 **どちらも moderate で、`region` と同じ harness 由来**（storybook の iframe に `<main>` も `<h1>` も無い）＝
**バーは moderate を対象外にしている**ので判定は動かない。**除外の理由を明記しておく**（黙って引かない）。

### 締め — §0 の問い Q1〜Q5 への答え

| # | 問い | 答え |
| --- | --- | --- |
| **Q1** | ★★ 完成バーは、新しく作る部品に対して機能するか | ★★★ **機能した。ただし一発では通らなかった**（15 中 13）。🟦 **落ちた 2 件は本物の欠陥**（`Slider` に名前を与えられない）で、**バーが無ければ確実に出荷していた**。🟥 **さらに大きいのは副産物**——面④ を実装した瞬間に**バーの実行エンジン自体が CSS 無しで走っていた**ことが出た（[DR-0094](DR/DR-0094-the-bar-engine-ran-without-any-css.md)）。★ **答え: バーは後追い検査ではなく設計指針として働いたが、「バーが緑」と「バーが測れている」は別だった** |
| **Q2** | ★ 依存 0 の 5 件は「入れるだけ」で済むか | 🟥 **済まなかった。**依存は 0 増（K3 🟦）だが、**lint が +8**・**`Slider` は素材層を触らないと直せない欠陥つき**・**`switch` は非整数の生ピクセル `18.4px`**。★ **「依存 0」と「無変更で入る」は別**が実測で確定 |
| **Q3** | ★★ shadcn に無い 8 件は部品になるか | **11 件中 9 件が「作れ」に倒れた**（予6 的中）。★★★ **2 問だけでは「作るな」を一度も言えない**——**2 問には「需要」の項も「既存で足りるか」の項も無い**。→ [DR-0088](DR/DR-0088-core-subject-boundary-is-decided-by-two-questions.md) への差分要求 |
| **Q4** | 🆕 🟥 `combobox` が引く `@base-ui/react` をどう扱うか | **引かない（D2=B）**＋ **[DR-0093](DR/DR-0093-shadcn-radix-nova-is-not-a-single-primitive-source.md) を起票。**🟦 本回の 5 件は誰も引かないので実害 0（`@base-ui/*` は 0 件）。🟥 **機械では守っていない**——`@base-ui/*` が 0 件のうちは検査が「対象 0 件で緑」になり、**発火を確かめる検体が無い** |
| **Q5** | 製品層の窓口はどこまで要るか | 🟥 **予5 が外れた。**5 件のうち **4 件が素通し・1 件（`Slider`）がラッパー**。★ **割れ方が規則の解像度を測った**——ラッパーになった理由は「見た目の既定値」ではなく **「素材が名前を渡せない」**で、**手3 D1=(c) が想定していない種類の欠落**だった |

#### 予測の答え合わせ（7 本中 **5 的中 / 2 外れ**）

| # | 結果 |
| --- | --- |
| 予1 既存 24 件 0 行 | 🟦 **的中**（🟥 ただし指標の限界つき——新規ファイルは分母に入らない） |
| 予2 `dependencies` 0 増 | 🟦 **的中** |
| 予3 `no-arbitrary-value` +3（+2〜+5） | 🟥 **外れ**（実測 **+7**・内訳も違う。`switch` が 1 件で全量を使った） |
| 予4 バーは一発では通らない | 🟦 **的中**（ただし**落ちたのは `Slider` だけ**——`switch` / `radio-group` は story にラベルを書いたので通った） |
| 予5 5 件とも素通し | 🟥 **外れ**（`Slider` がラッパーに昇格） |
| 予6 2 問は 6 件以上で「作れ」に倒れる | 🟦 **的中**（11 件中 **9 件**） |
| 予7 予測していない箇所が 1 件以上 | 🟦 **的中・4 件**——① **`bg-white` はどの検査にも掛からない**（D9） ② **製品層へ持ち上げると同じクラスが違法になる**（D8=B を機械が拒んだ） ③ ★★★ **バーのエンジンに CSS が当たっていなかった**（D10・DR-0094） ④ **`rsc: false` が `use client` を 5 件中 3 件だけ剥がした** |

#### 🟨 予7 ④ の詳細 — `rsc: false` の剥がし方が部品ごとに違う

| 部品 | 上流レジストリ | 導入後 |
| --- | --- | --- |
| `radio-group` / `slider` / `avatar` | `"use client"` **あり** | 🟥 **無し** |
| `switch` / `progress` | `"use client"` **あり** | 🟦 あり |

**先頭 5 行は 5 件とも同じ形**（`"use client"` → 空行 → `import * as React`）なのに割れる。
🟦 **単独 `add` でも同じ結果**（`radio-group` を消して 1 件だけ入れ直して確認）＝ **決定的で、同時追加の順序とは無関係。**
🟨 **工程1 D3 の申し送り「`rsc: false` 後の `shadcn add` で部品が変わって降ってくるか」に対する答え**——
**変わって降ってくる。しかも部品ごとに違う。**
🟦 **実害は無い**（[DR-0083](DR/DR-0083-lib-build-silently-strips-use-client.md)：lib build はどのみち `'use client'` を落とす）が、
★ **「素材層は shadcn が吐いたそのもの」という前提は、CLI の時点で既に真ではない。**

#### 🟨 台帳のずれを 1 件直した

[部品カタログ](部品カタログ.md) の「実測」列は **18 件のまま**だった。
手順書は「24 → 29」と書いていたが、**表は工程3・工程4 の 6 件**
（`Alert` / `Breadcrumb` / `Calendar` / `Field` / `Tabs` / `Textarea`）**を反映していなかった。**
→ **18 → 29 に更新**。🟦 **`src/components/ui/*.tsx` を数えれば 1 秒で分かる**ので、以後はそれと突き合わせる
（L549 の教訓「台帳はゲートを回すたびに突き合わせる」の 4 例目）。

---

## 部品1 B1-06 — バーを既存 44 件に通した（2026-08-09・[手順書](手順/部品1_完成バーを機械で閉じる.md)・ブランチ `lisbon`）

> **B1-02〜B1-05 は PR #14 で完了済み。**本節は**残っていた B1-06 / B1-07**。
> 台帳（44 部品 × 面の「いまの状態」）は [部品の完成バー_台帳.md](部品の完成バー_台帳.md) が持つ。

### B1-06 の着手前実測 — ★★★ 通す前に、通す道具が無いと分かった

`main` = `2284b8a`（部品2 マージ後）。**バーの各面が本当に実装されているかを数えた。**

| 面 | バー文書の宣言 | 実装 | 実測 |
| --- | --- | --- | --- |
| **① 描画された** | 🟥 **落とす** | ❌ **0 行** | 🟥 **`() => null` の空 story を足したら 96/96 緑で通った** |
| **② a11y** | 🟥 critical 0 | ✅ `a11y.test: 'error'` | 🟦 critical 0 |
| **③ 状態面** | 🟨 台帳 | ❌ 台帳が無い | 🟥 44 件中 **10 件** |
| **④ 語彙の効果** | 🟥 **落とす** | 🟨 **3 部品だけ**（部品2 が手で書いた `play`） | 🟥 union prop 21 件中 **2 件** |
| **⑤ 型の閉じ** | 🟥 落とす | ✅ `tsc` ＋ lint | 🟦 運用中 |

★★★ 🟥 **これは「対象 0 件で緑」ではない**（通算 16 例は据え置き）——
**「目盛りを書いて、針を付けなかった」**形。[DR-0094](DR/DR-0094-the-bar-engine-ran-without-any-css.md)（エンジンが CSS 無しで走っていた）と同じ
**「バー自身が測れていない」系**の **2 例目**。
★ **面④ が「書けば掛かる」形だったのが効いた**——**書かなければ黙って通る**ので、
**部品2 が 5 件で書いたのはバーが要求したからではなく、作った人が覚えていたから。**

#### 🟥 story を 1 本も持たない出荷物が 3 件あった

`Alert` / `Field` / `Textarea` は **`src/index.ts` で export しているのに story が無い**（工程4 で入れた素材）。
**バーの全面が「対象 0 件で緑」**で、`/design-sync` のカードも立たない。
★ **「出荷部品 44 件」という数え方そのものが story を数えている**（[DR-0091](DR/DR-0091-claude-design-is-a-fourth-shipping-entrance.md)）——
**この 3 件はどの数え方からも落ちる。**🟨 `calendar` は在庫のみで既知（D8）。

### B1-06a — 面① を機械化した（D10=A・D11=B）＋ K6 / K7

`.storybook/preview.tsx` に **`export const afterEach`**。**全 story に自動で掛かる。**

**プローブで測った DOM の実体**（判定式はこの実測から書いた）:

| story | `canvasElement` の子孫 | `document.body` 直下 |
| --- | --- | --- |
| `Separator/Horizontal` ／ `Spacer/Default` | 4 | harness 5 件のみ |
| **`Dialog/Open`** | 🟥 **0** | 🟦 overlay ＋ `#radix-_r_3_`（`[role=dialog]`）＋ focus guard `<span>` 2 件 |
| `Tooltip/AlwaysOpen` | 1 | portal の器 1 件 |
| **空 story** | 🟥 **0** | harness 5 件のみ |

- 🟥 **`canvasElement` は無名の `<div>`**（`#storybook-root` ではない）——**id では掴めない。参照で比べる。**
- 🟦 **harness は毎回同じ 5 件**（`.sb-preparing-story` / `.sb-preparing-docs` / `.sb-nopreview` / `.sb-errordisplay` ＋ `svg#storybook-a11y-vision-filters`）
- 🟥 **`textContent` では判定できない**（`Spacer` / `Separator` は文字を持たない）→ **大きさで判定**

**K6（赤テスト）**: 空 story を混ぜる → 🟦 **その 1 件だけが落ちた**（他 95 件は緑）。消して 95/95 緑。
**K7（反対側）**: 🟦 **`Dialog/Open`・`Spacer`・`Separator`・`Tooltip/AlwaysOpen` は通った**——
★ **「赤くできる」だけでは足りない。「赤くすべきでないものを赤くしない」も同じ回で測る。**

### B1-06b — 面④ の借金を返した（D12=B）＋ K8

**repo が定義した語彙を持つ 13 部品**に `play` を足した
（`Box` / `Container` / `Grid` / `Inline` / `Section` / `Spacer` / `Stack` / `Select` / `PeriodSelect` / `DataGrid` / `DescriptionList` / `StatusPill` / `Link`）。
**story は 80 → 91**（出荷物の棚）。

#### ★★★ 🟥 生値で書いたら 3 件外れた —— tokens.css の宣言は実効値ではない

最初、期待値を `tokens.css` の宣言から計算して書いた。**3 件が赤になった:**
...
🟦 **部品は正しく、期待値のほうが誤り**——`src/styles/tmp-admin.css` が `:root:root` の詳細度で
上書きしており、**[DR-0005](DR/DR-0005-token-ownership-and-two-stage.md)「値は tmp-admin を引き継ぐ」のとおり。**
🟨 **ずれているのは `tokens.css` のコメント**（「6px アイコンとラベル」等）。

★★★ **これが面④ の書き方を決めた**——**主張を「このクラスは**このトークン**を指している」に絞る。**
**生値を書くと ① 二重管理を 1 つ増やし ② トークン差し替えのたびに story が壊れる**——
**差し替えられることこそがこの repo の目的**（[DR-0078](DR/DR-0078-repo-becomes-a-ui-factory-for-a-core-design-system.md)）なのに。
→ 共通の測り方を `src/stories/measure.ts` に置いた（`resolveLength` / `resolveColor` は
**custom property を probe に載せて計算後の値を読む**）。
🟥 **`?.` を禁じた**——`querySelector(…)?.getBoundingClientRect().width` は**要素が無いと `undefined` で、
`expect(undefined).not.toBe(...)` が通る**＝ **「対象 0 件で緑」の play 版。**`el()` は**無ければ throw する。**

#### K8（赤テスト）— 🟥 DR-0090 は 1 語消すだけで完全に再発する

`src/lib/tw-merge.ts` の `container` から **`'field-md'` の 1 語だけ**を消した:

| story | 期待 | 実測 |
| --- | --- | --- |
| `Select/Widths` | 192px | 🟥 **113.59375px** |
| `PeriodSelect/Vocabulary` | 192px | 🟥 **67.859375px** |

★★ **`113.59` は [DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md) が記録した数値そのもの**——**同じ壊れ方が寸分違わず戻った。**
🟦 **`DescriptionList` は `field-sm` を使うので緑のまま**＝ **消した語だけが落ちる**（検査の分解能も確認できた）。
🟦 **塞いだのは 2026-08-08（PR #11）だが、再発を止める検査はこの回まで無かった。**

#### 🟥 面④ が裁けないものが 1 件出た → [OBS-0018](OBS/OBS-0018_リンクが本文と見分けられない.md)

`Link` の `play` に「リンクの色は親の色と違うはず」と書いたら赤になった:

```
AssertionError: expected 'rgb(0, 0, 0)' not to be 'rgb(0, 0, 0)'
```

🟦 **`--primary` も `--foreground` も `rgba(0,0,0,1)`**（tmp-admin V2「accent は塗りに使わない」＝ **仕様**）。
静止状態の下線も無い（`hover:underline focus-visible:underline` だけ）ので、
🟥 **リンクは本文と見分けが付かない。**🟦 **axe では検出できない**——`color-contrast` は
前景と背景の比しか見ず、**リンクと本文の対比は誰も測っていない**（WCAG 1.4.1 の領域）。
★ **一般則: 「語が効いているか」と「効いた結果が使えるか」は別の検査で、後者は機械に載っていない**（面④ と面⑦ の境目）。

### B1-06c — 台帳（D13=B）

[部品の完成バー_台帳.md](部品の完成バー_台帳.md) を新設。**44 部品 × 面。**

| 面 | B1-06 の前 | 後 |
| --- | --- | --- |
| ① 描画された | 🟥 実装 0 行 | 🟦 **44/44** |
| ② critical | 🟦 0 | 🟦 **0** |
| ③ 状態面 | 🟥 10/44 | 🟥 **10/44**（据え置き・D6=B） |
| ④ 語彙の効果 | 🟥 2/21 | 🟨 **15/21** |

★★★ **Q5 の答え: 借金の主体は部品ではなく、バーそのものだった。**
🟦 **バーの妥当性の検算としては成立している**——面① は 44/44 通り（**緩すぎない**：空 story は実際に落ちる）、
面③ は 10/44 しか無い（**厳しすぎない**：返せる量の借金が見えている）。

### 面② の再計測（`storybook-static` ＋ axe-core 4.12.1・`document` 走査）

| impact / rule | 出荷物の棚（**91 story**） | 除外棚（15 story） |
| --- | --- | --- |
| **critical** | 🟦 **0** | 🟦 **0** |
| serious / `color-contrast` | 🟨 **113** | 195 |
| moderate / `region` ・ `landmark-one-main` ・ `page-has-heading-one` | 94 / 85 / 88 | 25 / 14 / 14 |

★★ 🟥 **serious の「件数」は指標として使えない。**83 → **113**（+30）だが、
**増えたのは全部、既知の色の組を描く story が増えたぶん**（本回の `Vocabulary` story 11 本が `StatusPill` 等を再描画）。
🟦 **色の組で畳むと 8 種類のまま**で、**全部 [OBS-0017](OBS/OBS-0017_意味色とfillの対比が全滅している.md)**（最多 `#8a8a8e on #ffffff` が 65 件）。
→ 🟥 **台帳に載せるのは件数ではなく「色の組の種類」**（件数は story 数に比例して増える）。

### ゲート（`main` = `2284b8a` のベースラインと比較）

| ゲート | ベースライン | 本回 | 判定 |
| --- | --- | --- | --- |
| `typecheck` | 緑 | 緑 | 🟦 |
| `lint` | error 50 / warning 1 | **error 50 / warning 1** | 🟦 **内訳一致・自作分の赤 0** |
| `build` | dist 84 / `design.mjs` 91,322 B | dist **84** / **91,322 B** | 🟦 **不変**（story は出荷物に入らない） |
| `format:check` | 緑 | 緑 | 🟦 |
| `spell` | 327 | **332** | 🟨 **辞書に 2 語追加**（`nopreview` / `errordisplay` ＝ storybook harness のクラス名） |
| `build-storybook` | 緑 | 緑 | 🟦 |
| **バー** | 95/95 | **106/106** | 🟦 **+11**（`Vocabulary` story） |

🟨 **`lint` の途中経過**: 自作の `play` が `restrict-template-expressions` を **7 件**持ち込んだ（数値の
テンプレート埋め込み）。**`String()` で明示変換して 0 に戻した**——**自作分の赤は残さない。**
🟨 **spell のベースライン訂正**: 表の **327 は部品2 の測定時点**で、その後 **DR-0093・DR-0094 の 2 ファイル**が
増えている（**新しい赤ではない**）。本回の +1 は `src/stories/measure.ts`。

#### 🟨 環境の注記（この作業環境に固有・repo の欠陥ではない）

既定の `node` が **v22.16.0** で `cspell@10` の要求（>=22.18.0）を満たさず `pnpm spell` が落ちる。
🟦 **repo は `mise.toml` で `node = "24"` を固定している**ので、**mise を活性化していないシェルの問題。**
mise の node（**v24.18.1**）で実行して緑を確認した。

---

## 部品3 — DatePicker（在庫の合成）と、バーが見ていない 3 件（2026-08-09・[手順書](手順/部品3_DatePickerと射程の外の3件.md)・ブランチ `da-nang`）

**構成**: `main` = `1c5bc0b` から。🟥 **`shadcn add` を 1 度も打たない初めての回**——
素材は全部在庫にあり、足りないのは**合成の設計**だけ（`date-picker` は**レジストリ 404**）。
**D4（依存の追認）と D7（ゲート 7 本目）はユーザー判断**（2026-08-09・逐語「推奨で」）＝ **どちらも A**。

### C3-01 — 現況と予測

ベースライン（`1c5bc0b`）: typecheck 緑 ／ error **50** / warning 1 ／ dist **84**・`design.mjs` **91,322 B** ／
format 緑 ／ spell **332** ／ story **54 ファイル / 106 件** ／ **バー 106/106 緑** ／ a11y critical **0**・serious **113**（色の組 8 種類）。

**登録した予測**（手順書 §0.1 に着手前に書いたもの）:

| # | 予測 | 結果 |
| --- | --- | --- |
| K1 | 素材層 29 件の diff **0 行** | 🟦 **的中**（`git diff origin/main -- src/components/ui/` が空） |
| K3 | `dependencies` **0 増** | 🟦 **的中**（12 件のまま・`package.json` の diff は空） |
| K4 | `Field` / `Textarea` は入力系なので **`label` 系の critical が出る** | 🟥 **外れ**（critical **0**。理由は下記 C3-05） |
| K2 | バーに**一発では通らない見込み** | 🟦 **的中**（124 中 **1 件**が落ちた。ただし**落ちた場所が予測と違った**） |

### C3-02 — `DatePicker` を設計・実装（D2=B / D3=B / D9=A）

`src/components/Selection/DatePicker.tsx`（**製品層・自作の合成**）。
語彙は **`mode`（`'single' | 'range'`）** と **`width`（`FieldWidth` の既存語彙を再利用）**。

★★ 🟥 **手3 D1=(c)「欠落品 ＋ 既定値ラッパー」のどちらでもない第 3 の形が出た**（Q1）——
**素材は全部在庫にあり、上流の実装が 1 行も無い。**「合成」は規則が名前を持っていない形。

🟨 **`multiple` は採らなかった**——上流 `react-day-picker` は持つが、5 画面のどこにも需要が無く、
**面③ に「対象外」と書く理由すら書けない**（**使われないまま語彙を定義する**のは、語彙が実在することの証明を放棄する形）。

🟥 **Q6（`locale` はコアか題材か）の答え: 器（prop）に落ちた。ただし穴が残った。**
2 問を当てると 問①「他所でも意味が通る」= 通る、問②「有限の語で言えるか」= 🟥 **言えない**（言語は無限集合）
→ **器で受ける**（D9=A）。**しかし既定の文言（`placeholder = '日付を選ぶ'`）はコアが日本語で持っている**——
[DR-0070](DR/DR-0070-product-layer-boundary-rule.md)「単体で置いても壊れない」を満たすには既定が要り、**既定には言語が要る。**
🟦 **`PeriodSelect` の `aria-label = '期間'` に続く 2 例目**＝ **「言語はコアに入っていない」は既に偽だった。**

### C3-03 — story をバーの面つきで書く（D5=B）／ ★★ K2 が発火した

**`DatePicker` に 7 story**（Default / Empty / Disabled / Range / **Open** / Vocabulary / Controlled）。

★★★ 🟥 **`DatePicker/Open`（Popover を開く初めての story）が初回の実行で落ちた。**
...
🟥 **落ちたのは `DatePicker` ではなく `Popover`**——`PopoverContent` は Radix の `role="dialog"` を出すが
**名前を 1 つも持たない**（`PopoverTitle` は `aria-labelledby` に繋がっていない・実測）。
**`Popover` は工程3（2026-08-07）から出荷していて、2 日間、名前の無い dialog を出していた。**

★★ **見えなかった理由: `Popover` を開く story が 1 本も無かった**——
`Popover/Default` も `PeriodSelect/Custom` も**トリガを描くだけ**で、
**閉じた overlay は DOM を持たないので axe の対象が 0 件。**
→ 🆕 **[DR-0096](DR/DR-0096-overlays-that-no-story-opens-are-invisible-to-every-check.md)**（「対象 0 件で緑」の **17 例目**）。

**塞ぎ方は D10=B**（§2 に追記してから決めた）——**製品層で `PopoverContent` を昇格させ、`aria-label` を型で必須にした。**
🟦 **素材層は 1 行も触っていない**（`Select.tsx` の先例）。
🟦 **`tsc` が既存の使用箇所 2 つを両方落とした**（`DatePicker` と `Popover/Default`）＝ **型で要求すると書き忘れは即座に落ちる。**

#### 🆕 ★★★ 面④ の限界が出た —— **バーは「トークンに裏打ちされた語彙」しか測れない**（Q1 の本体）

| 語彙 | 面④ で測れるか | 測り方 |
| --- | --- | --- |
| `width`（`sm` / `md` / `lg`） | 🟦 **測れる** | `--container-field-*` の実効値と突き合わせ ＋ **単調性** |
| 🟥 **`mode`（`single` / `range`）** | 🟥 **測れない** | **実効値を持たない**（振る舞いの分岐）。**DOM の差（トリガの文言）でしか主張できない** |

★★ **これは「バーが上流の部品向けに作られていた」ことの表れ。**
**shadcn の部品の語彙は cva ＝ 見た目の語彙**なので、`getComputedStyle` で測れる。
🟥 **自分で設計した部品の語彙には、振る舞いの分岐が混じる**——**面④ の定義がそれを想定していない。**

### C3-04 — `PeriodSelect` を置き換えた（Q2・D6=A・K6）

★★★ **Q2 の答え: 回った。**`PeriodSelect` の `custom` 分岐（Popover ＋ Calendar の手組み・約 20 行）が
**`<DatePicker mode="range" …>` の 6 行になった。**

🟦 **K6 は緑**——`PeriodSelect` の 3 story ／ `FilterBar` ／ 題材の `IssueList` すべて通り、
**`PeriodSelect/Vocabulary` の面④（`width` の 3 語）も生きている**（[DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md) の再発なし）。

★ 🟥 **工程3 D13=A の判断は変えていない**——「範囲の入力はこの部品が持つ」は据え置きで、
**持ち方が手組みから部品の利用に変わっただけ。**
🟨 **D13=A が想定していた選択肢は「部品が内蔵する / 画面が組む」の 2 つ**で、
**「部品が別の部品を使う」は選択肢に無かった**（**外に出す先が画面しかなかったから**）＝
**部品を作ると、既存の判断の選択肢が 1 つ増える。**

### C3-05 — story が無い 3 件を拾った（Q3・K4）

| 部品 | story | a11y critical |
| --- | --- | --- |
| `Alert` | 3（Default / Variants / Overflow） | 🟦 **0** |
| `Field` | 5（Default / Orientation / Invalid / Disabled / Overflow） | 🟦 **0** |
| `Textarea` | 3（Default / States / Overflow） | 🟦 **0** |

🟥 **K4 は外れた**（予測: `label` 系の critical が出る）。
★★ **ただしこれは「射程外に借金が無かった」の証明にならない**——
**story を書いたのがバーを知っている側**で、D5=B に従って面つきに書き、`aria-label` を全部付けた。
🟥 **K4 が測ったのは「借金の量」ではなく「書く人がバーを知っているか」だった**＝ **検体の設計の限界。**

🟨 **serious は 1 種類増えた**——`Alert` の destructive な description が **`#ff4f45 on #ffffff`**（`--destructive/90`）。
★ **これまでの 8 組はすべて「トークンの組」だったが、9 組目は「トークン × 不透明度」**
＝ **① 層の語彙に無い色が部品の className から生まれている**（→ [OBS-0017](OBS/OBS-0017_意味色とfillの対比が全滅している.md) に追記・**OBS-0009 と接続する初めての実例**）。

### C3-06 — 面⑥（44px）を 42 セルで測った（Q5・D8=A）

`tools/hit-area-probe.mjs` を書き、**`hasTouch: true`** で実測（[DR-0049](DR/DR-0049-hit-area-reaches-44px-only-at-default-size.md) と同じ手法）。

| 群 | 個数 | 見た目 | 当たり判定 | 44px 未達 |
| --- | --- | --- | --- | --- |
| 日付セル | **42** | 28×28 | 🟥 **28×28** | 🟥 **42 / 42** |
| 月送り | 2 | 28×28 | 🟥 **28×28** | 🟥 **2 / 2** |
| トリガ（製品層 `Button`） | 1 | 129.73×32 | 🟦 **141.73×44** | 🟦 **0 / 1** |

★★★ 🟥 **予測していなかったほうが重い —— 44px の規定には迂回路がある。**
**当たり判定の拡張（`HIT_AREA`）は製品層の `Button` にしか無く**、
...
🟨 **28px は WCAG 2.5.8（AA・24px）を満たす**ので **axe は緑のまま**——
**「a11y が緑だから当たり判定も大丈夫」は成り立たない**ことの 2 例目。
→ **この回では直さない**（`--cell-size` は ① 層の決定）。[OBS-0008](OBS/OBS-0008_当たり判定44pxをどう扱うか.md) に追記した。

### C3-07 — 依存の事後審査（Q4・D4=A・ユーザー判断）

🟥 **`react-day-picker` / `date-fns` は工程3（`5a9abde`）で `shadcn add calendar` の副産物として入り、判断の記録が無かった。**
**`calendar` 自身は「出さない」と明示して出荷していない**のに、**依存だけは出荷物として払っていた。**
[DR-0092](DR/DR-0092-the-core-holds-the-vessel-not-the-state.md) の一般則が立ったのは**その翌日**。

**D4=A（追認）＋ finding の起票** → 🆕 **[DR-0095](DR/DR-0095-shipped-dependencies-arrived-without-a-decision.md)**。
🟨 **`dependencies` の増加を落とす検査は置いていない**（検体は作れるが、本回の範囲外と判断した）。

### C3-08 — ゲート 7 本目（D7=A・ユーザー判断）と K7

🟥 **K7: わざと `DatePicker/Vocabulary` の単調性の主張を反転させて実行 → `exit 1`。戻して `exit 0`。**
★ **`vite build` は型エラーを赤い字で出しながら exit 0 になる**（[DR-0082](DR/DR-0082-vite-build-prints-type-errors-but-exits-zero.md)）ので、
**「赤く見えること」と「ゲートとして機能すること」は別**——**確かめてから足した。**

`CLAUDE.md` の機械ゲートを **6 本 → 7 本**に更新。🟨 **所要は約 70 秒**（既存 6 本の合計より重い）。

### 締め — ゲートと §0 の問い

| ゲート | ベースライン（`1c5bc0b`） | 本回 | 判定 |
| --- | --- | --- | --- |
| typecheck | 緑 | 緑 | 🟦 |
| lint | error **50** / warning 1 | **error 50 / warning 1** | 🟦 **内訳一致・自作分の赤 0** |
| build | dist 84 / `design.mjs` 91,322 B | dist **85** / **92,605 B** | 🟨 **+1 ファイル**（`DatePicker.d.ts`）・**+1,283 B** |
| format:check | 緑 | 緑 | 🟦 |
| spell | 332 | **340** | 🟨 **+8（本回の md）・辞書 0 語追加** |
| build-storybook | 緑 | 緑 | 🟦 |
| 🆕 **test-storybook（7 本目）** | 106/106 | **124/124 緑** | 🟦 **+18**（`DatePicker` 7 ／ `Alert` 3 ／ `Field` 5 ／ `Textarea` 3） |
| **a11y critical**（出荷物の棚） | 0 | 🟦 **0** | 🟦 |
| **a11y serious の色の組** | 8 種類 | 🟨 **9 種類** | 🟨 **9 組目は「トークン × 不透明度」** |
| K5（出荷入口 4 本） | — | 🟦 `title-map-check` OK（story 58 題）・題材の混入 0 | 🟦 |

| # | 問い | 答え |
| --- | --- | --- |
| **Q1** | ★★★ shadcn が配らない部品を自分で設計したとき、バーは通るか | 🟨 **通った。ただし一発ではなく、落ちたのは新部品ではなく既存の `Popover` だった。**★★★ **本体の発見は「面④ が測れない語彙がある」**——`width` は測れるが **`mode` は実効値を持たない**。**バーは上流の部品（cva ＝ 見た目の語彙）向けに作られていた** |
| **Q2** | ★★★ `DatePicker` を出すと `PeriodSelect` は使う側に回るか | 🟦 **回った**（手組み約 20 行 → 6 行・K6 緑）。★ **工程3 D13=A の判断は変えていない**——**持ち方が変わっただけ**。🟨 **D13=A の選択肢に「部品が別の部品を使う」が無かった**＝ **部品を作ると既存の判断の選択肢が 1 つ増える** |
| **Q3** | ★★ story が無い 3 件に目盛りを当てたら何が出るか | 🟦 **critical 0**（K4 は外れ）。🟥 **ただし「借金が無かった」の証明にはならない**——**書いたのがバーを知っている側**で、K4 が測ったのは**書く人がバーを知っているか**だった。🟨 **serious の色の組は 1 種類増えた** |
| **Q4** | 🟥 ★★ 依存 2 件は出荷物の依存として妥当か | 🟦 **妥当**（D4=A・ユーザー判断）。🟥 **だが「気づいたら入っていた」ことのほうが発見**——[DR-0095](DR/DR-0095-shipped-dependencies-arrived-without-a-decision.md)。**一般則は「足す判断」に掛かるが、依存が増える経路は判断を伴わない** |
| **Q5** | ★ 面⑥（44px）はこの部品で初めて正面から効くか | 🟥 **効いた。42 / 42 が未達。**★★★ **予測していなかった発見: 44px の規定には迂回路がある**（`buttonVariants` は見た目だけ配り、当たり判定を配らない）。**論点が「量」から「規定がどこまで届くか」に変わった** |
| **Q6** | 🟨 `locale` はコアか題材か | 🟦 **器（prop）**。🟥 **ただし既定の文言はコアが日本語で持っている**（`PeriodSelect` の `aria-label='期間'` に続く **2 例目**）＝ **「言語はコアに入っていない」は既に偽だった** |

#### 🆕 起票した DR 3 本

- **[DR-0095](DR/DR-0095-shipped-dependencies-arrived-without-a-decision.md)** — 出荷物の依存 2 件は、誰の判断でもなく入っていた
- **[DR-0096](DR/DR-0096-overlays-that-no-story-opens-are-invisible-to-every-check.md)** — 開かれない overlay は、どの検査からも見えない
- **[DR-0097](DR/DR-0097-an-exception-that-covered-the-whole-set-hid-the-rule.md)** — 例外が全量を覆っていて、規則の形が見えなかった（**バーの面② は定義より実装のほうが厳しかった**）

#### 🟨 環境の注記（据え置き・repo の欠陥ではない）

既定の `node` が **v22.16.0** で `cspell@10` の要求（>=22.18.0）を満たさず `pnpm spell` が落ちる。
🟦 `mise` の node（**v24.18.1**）で実行して緑を確認した（[handoff §環境の再現](handoff.md) の既知 ②）。

---

## 部品4 — 開かれない overlay を開く（2026-08-09・[手順書](手順/部品4_開かれないoverlayを開く.md)・ブランチ `bangkok`）

**前提**: `main` = `1897a4e`（部品3 の PR #17 マージ済み）。**部品を 1 件も足していない回**——
story を 4 本と検査を 1 本足し、**バーが見ていなかった状態を開けに行った。**

### C4-01 現況（ゲート 7 本・`1897a4e`）

typecheck 緑 ／ **error 50 / warning 1** ／ dist **85**・`design.mjs` **92,605 B** ／ format 緑 ／
spell **340** ／ story **58 ファイル / 124 件** ／ **バー 124/124 緑**（11.87 秒） ／ a11y critical 0・serious 142。

### C4-01b 🟥 着手前実測で [DR-0096](DR/DR-0096-overlays-that-no-story-opens-are-invisible-to-every-check.md) の名指しが誤っていると分かった（Q3）

**手法**: ① `src/components/ui/*.tsx` から `Primitive.Portal` を全件走査 → **6 件**
② story 58 ファイルから `open` / `defaultOpen` / `play` を全件走査 ③ **一時 story で DOM を数えた**

| DR-0096 の記述 | 実測 |
| --- | --- |
| 開く story は **2 件** | 🟥 **3 件**（`Dialog/Open` ／ **`Tooltip/AlwaysOpen`** ／ `DatePicker/Open`） |
| **`Tooltip` の中身はいまも開かれていない** | 🟥 **誤り。**`<Tooltip open>` で **2026-07-27（手5）から開いている**（`[data-slot="tooltip-content"]` **1 件**・`role="tooltip"`） |

★★ **原因は数え方**——**story 名に `Open` が付くかで数えた**ので `AlwaysOpen` が漏れた。

### C4-02 開いた story を書いた（D2=B・`play` でトリガを操作）

**`userEvent.click` を使った。**🟥 **Radix `Select` は `pointerdown` で開く**ので生の `element.click()` では開かない
（部品3 の `DatePicker/Open` は `.click()` で通ったが、あれは `Popover`）。

### C4-03 ★★★ K2 の結果 —— 🟥 **予測は 2 本とも外れた**

| 部品 | 🟥 予測（登録済み） | 実測 |
| --- | --- | --- |
| `DropdownMenu` | **落ちる**（`aria-required-children`） | 🟨 **落ちたが rule が違う**——**`aria-hidden-focus`（serious）** |
| `Select` | 🟦 **通る** | 🟥 **外れ。落ちた**（同じ `aria-hidden-focus`） |
| `Sheet` | 🟦 **通る**（`SheetTitle` が紐づく） | 🟨 **通ったが理由が違う**（下記） |

#### 🟥 発見 1 —— `aria-hidden-focus` は部品の欠陥ではなく検査器の限界

Radix は modal overlay を開くと `hideOthers()` で **document の残り全部に `aria-hidden="true"` を付ける**。
その中に残るトリガが focusable なので axe が落とす。**`axe-core@4.12.1` のソースを読んだ**:
...
🟥 **`role="menu"`（`DropdownMenu`）／ `role="listbox"`（`Select`）は modal と認識されない。**
🟦 **実際にはフォーカスは閉じ込められている**——`userEvent.tab()` ×3 で **4 部品とも 1 度も外へ出ない**（実測）。

#### ★★★ 🟥 発見 2 —— `Sheet` は「通った」のではなく、axe が判定を放棄していた（→ [DR-0098](DR/DR-0098-incomplete-was-counted-as-green.md)）

story 内で `axe.run(document)` を直接実行した実測:
...
→ **§2 に D7・D8・D9 を追記してから決めた**（規律どおり・その場で決めない）。

- **D7=C**: 該当 story だけ `aria-hidden-focus` を外し、**引き換えに `expectFocusTrapped` で直接測る**
  （🟥 **外すだけなら「消した」**。部品1 D3 が `color-contrast` でやった「数える場所を移す」形）
- **D8=B**: `a11y-scan.mjs` から `resultTypes` を外し、**保留を台帳に載せる**（落とさない）
- **D9**: 当初 B（`Dialog` には不要）→ 🟥 **D8=B の計測で `Dialog` にも保留 2 件が出たので A に訂正**
  （Radix 自身の `data-radix-focus-guard` の `<span>`）。★ **「対象 0 件」の判定を violations だけ見て下していた**

🟦 **パラメータの深いマージを赤テストで確認した**——`Open` の parameters を「別の rule を外す」に差し替えると
**`aria-hidden-focus` で落ちた**＝ **story 単位の無効化で a11y 検査ごと消えてはいない。**

### C4-04 Q2 の機械化（D4=C の 2 段）—— 🟥 **誰も名指ししていない 7 件目が出た**

```
$ node tools/opened-overlay-check.mjs
開閉を持つ素材 7 件（Primitive.Portal で判定）
  🟥 dropdown-menu-sub-content   dropdown-menu.tsx   **開いた story が無い**
```

🟥 **`dropdown-menu-sub-content`（入れ子メニュー）は DR-0096 の 4 件にも、
本回 §1.1 の 3 件にも入っていない**——**機械に引かせて初めて出た**（→ [DR-0099](DR/DR-0099-the-blind-spot-list-must-be-machine-derived.md)）。
→ `DropdownMenu/SubMenuOpen` を足して **7/7 緑**。

### C4-05 K3 —— 両方向 × 2 段で発火を確認した

| 検体 | 結果 |
| --- | --- |
| 静的: `Sheet/Open` の主張を消す → 戻す | 🟦 **exit 1 → exit 0** |
| 動的: 主張を残して `<Tooltip open>` の `open` を外す → 戻す | 🟦 **1 failed → 2 passed** |

★ **③（動的側）を必ずやる**——①② だけだと「静的側しか測っていない」。
...
| --- | --- | --- |
| typecheck | 緑 | 🟦 |
| lint | **error 50 / warning 1** | 🟦 **内訳一致・自作分 0** |
| build | dist **85**・`design.mjs` **92,605 B** | 🟦 **完全に不変**（story と tools だけ触ったので当然） |
| format:check | 緑 | 🟦 |
| spell | **346 ファイル / 0 件**（辞書 +1 = `pointerdown`） | 🟦 |
| storybook | **58 ファイル / 128 件**（+4） | 🟦 |
| **バー** | 🟦 **128 / 128 緑**（11.74 秒） | 🟦 **K4 = 回帰なし** |
| a11y（出荷物の棚） | **critical 0** ／ serious **148**（`color-contrast` 145 ＋ 🆕 **`aria-hidden-focus` 3**） ／ 🆕 **判定の保留 23** | 🟨 |

**赤テスト**: K1 **素材層 diff 0 行** ／ K3 **両方向 ×2 段** ／ K4 **128/128** ／
K5 `title-map-check` **OK**・dist 3 本不変 ／ K6 **`dependencies` 12 件のまま** ／
🟥 **K7（予測していない箇所）は 4 件**——① `aria-hidden-focus` という rule 自体 ② **`Sheet` の緑が incomplete だった**
③ **`dropdown-menu-sub-content` の存在** ④ **`Dialog` にも保留があった**（D9 を訂正した）。

### 締め —— 問いへの答え

| # | 問い | 答え |
| --- | --- | --- |
| **Q1** | 開いたら何が落ちるか | 🟨 **`aria-dialog-name` は 1 件も出なかった**（`Popover` は固有の穴だった）。代わりに **`aria-hidden-focus` が 2 部品**——🟦 **部品の欠陥ではなく axe の限界**（`isModalOpen()` は `[role=dialog]` しか見ない） |
| **Q2** | 開いた story を機械で要求できるか | 🟦 **できる。2 段で閉じる**（静的＝ `Primitive.Portal` から一覧を引く ／ 動的＝ 主張が真か）。**片方では閉じない**（[DR-0099](DR/DR-0099-the-blind-spot-list-must-be-machine-derived.md)） |
| **Q3** | なぜ数え間違えたか | 🟥 **数え方が目視（story 名に `Open` が付くか）だった。**機械に引かせたら **1 件が誤りで 1 件が漏れていた** |
| **Q4** | 他にも「描いていない状態」があるか | 🟥 **ある**（`Sidebar` の `collapsed` / mobile）。🟨 **`Primitive.Portal` では引けない**ので [OBS-0019](OBS/OBS-0019_storyが一度も描いていない状態をどこまで機械で要求するか.md) に積んだ |
| **Q5** | 面② 以外は何を言うか | 🟦 **面① は portal を見るので素通し、面④ は開いた中身に語彙が無いので掛からない。**🟥 **代わりに「面② が何も言っていない」ことが出た**（保留 23 件・[DR-0098](DR/DR-0098-incomplete-was-counted-as-green.md)） |

## 部品5 — 判定の保留と、描かれていない状態（2026-08-09・[手順書](手順/部品5_判定の保留と描かれていない状態.md)・ブランチ `perth`）

**前提**: `main` = `a1f3fe2`（部品4 の [PR #18](https://github.com/yatami0/design/pull/18) マージ済み）。
**この回も部品を 1 件も足していない**——**部品4 が作った 2 つの目盛り**
（[DR-0098](DR/DR-0098-incomplete-was-counted-as-green.md) の「保留を数える場所」と
[DR-0099](DR/DR-0099-the-blind-spot-list-must-be-machine-derived.md) の「射程の外を機械で引く」）**を初めて読む回。**

### C5-01 現況（ゲート 7 本・`a1f3fe2`）

typecheck 緑 ／ **error 50 / warning 1** ／ dist **85**・`design.mjs` **92,605 B** ／ format 緑 ／
spell **349**（🟨 表は 346・差分 3 は部品4 が最後に足した md） ／ story **58 ファイル / 128 件** ／
**バー 128/128 緑**（12.16 秒・setup 63 秒） ／ a11y critical 0・serious 148・**判定の保留 23** ／
（ゲート外）`opened-overlay-check` **7/7 緑**。

### C5-01b ★★★ 🟥 着手前実測 —— 保留 12 件の中身が全部割れた（Q1）

**手法**: `storybook-static` に axe を直接注入し、**`incomplete` のノードごとに `message` / `html` / `data` を出した**
（🟥 **台帳は `rule` / `impact` / `target` しか保存しておらず、理由を捨てていた**）。

| rule | 件数 | axe の言い分（`messageKey`） | 🟥 実測 |
| --- | --- | --- | --- |
| `color-contrast` | **5** | `shortTextContent`（`DatePicker` の日付セル） | **`#8a8a8e on #ffffff`・比 3.43** ＝ **既知の色の組の 1 番目（86 件）。AA 未達** |
| `color-contrast` | **4** | 同上（`Avatar` の「小」「中」「大」＋ `States` の「空」） | **`#85858b on #f2f2f7`・比 3.28** ＝ **既知の 6 番目（6 件）。AA 未達** |
| `color-contrast` | **1** | `bgOverlap`（`.rdp-caption_label`） | 🟥 **`nav.absolute.inset-x-0.top-0` が月ラベルを覆っている**（比自体は **21.00**） |
| 🟥 `aria-valid-attr-value` | **2** | `controlsWithinPopup` | 🟦 **参照先 ID は 2 件とも実在**（`document.getElementById`）。**axe が確かめていなかった** |

**`axe-core@4.12.1` のソース実測**（`ariaValidAttrValueEvaluate` の `preChecks`）:

```js
'aria-controls': function ariaControls() {
  var hasPopup = ['false', null].includes(virtualNode.attr('aria-haspopup')) === false;
  if (hasPopup) { needsReview = …; messageKey = 'controlsWithinPopup'; }   // ← ID の実在を見ていない
  return … && hasPopup === false;
}
```

🟥 **[OBS-0019](OBS/OBS-0019_storyが一度も描いていない状態をどこまで機械で要求するか.md) §5 の「本物の可能性がある」は外れた**（→ [DR-0100](DR/DR-0100-pending-judgements-split-into-three-kinds.md)）。

### C5-01c 🟥 着手前実測 —— `Sidebar` の mobile は道具無しで描ける（Q3）

| 実測 | 結果 |
| --- | --- |
| viewport は効くか | 🟦 **効く**（`innerWidth` desktop **1200** / mobile **320**・一時 story で実測） |
| どこで効くか | 🟦 `@storybook/addon-vitest` の `test-utils.js` が **`await setViewport(…)` を `composedStory.run()` の前に呼ぶ**（ソース実測）。`MINIMAL_VIEWPORTS.mobile1` = **320×568** |
| 🟥 mobile の初期状態 | 🟥 **`[data-slot="sidebar"]` が 0 件**（`openMobile` の初期値 `false`・**外から開く prop が無い**） |
| 🟥 そのときの面① | 🟥 **緑**（`SidebarTrigger` が描かれているから） |
| 開いたあと | 🟦 portal に出る（`DIV:sheet-overlay` → `DIV:sidebar`）。🟥 **名乗るのは `data-slot="sidebar"`**（`sheet-content` ではない） |

★★ **部品4 D1 が mobile を外した理由（「viewport の道具を新設することになる」）は誤りだった。**

### C5-02 保留に種類と理由を持たせた（D2=C・D3=B・D4=B）

`tools/a11y-scan.mjs` に ① **`messageKey` / `pair` / `contrastRatio` の保存** ② **`TOOL_LIMIT`**（検査器の限界・**根拠と引き換えを必ず書く**）
③ **`classify()`**（`tool-limit` / `short-text` / `bg-overlap` / **`unclassified`**） ④ **既知の色の組との突き合わせ** ⑤ **未分類 or 新しい色の組があれば `exit 1`** を足した。

```
[判定の保留・出荷物の棚] 23 件
  13  (c) 検査器が無条件に保留（引き換えを我々が測っている）
   9  (a) 文字が短くて本文か判定できない（色は測れている）
   1  (b) 重なっていて背景色を決められない
  → 色の組: 既知 9 件 / 新しい組 0 種類
```

### C5-03 K2 — 両方向で発火を確かめた

| 検体 | 結果 |
| --- | --- |
| `classify()` から `'bgOverlap'` の 1 語を消す | 🟦 **`exit 1`**（`未分類  color-contrast / bgOverlap / DatePicker / .rdp-caption_label` と理由つきで出た） |
| 戻す | 🟦 **`exit 0`**（23 件・内訳一致） |

### C5-04 Q2 の実測 → D5=A（測るだけ）

| 要素 | 矩形 (x, y, w, h) | `pointer-events` |
| --- | --- | --- |
| `nav.absolute.inset-x-0.top-0` | **34, 70, 196×28** | `auto` |
| `.rdp-caption_label`（「August 2026」） | **88, 74, 87×20** | `auto` |
| 前月 / 次月ボタン | 34 / **202**, 70, 28×28 | — |

- 🟥 `elementsFromPoint`（ラベル中心）の先頭は **`NAV`**（ラベルは 2 番目）
- 🟦 **日付セル 42 件のうち覆われているものは 0 件**＝ **操作は壊れていない**
- 🟥 **潜在するのは `captionLayout="dropdown"`**（この位置に対話要素を置いた瞬間、nav がクリックを奪う）
→ **直さず [OBS-0020](OBS/OBS-0020_カレンダーのnavが月ラベルを覆っている.md) に積んだ。**

### C5-05 `Sidebar` の `collapsed` / mobile（D6=B）

- `Sidebar/Collapsed` — `SidebarProvider defaultOpen={false}` ＋ `data-state="collapsed"` / `data-collapsible="offcanvas"` を主張
- `Sidebar/Mobile` — `globals: { viewport: { value: 'mobile1' } }` ＋ **開く前の 0 件を主張してから** `userEvent.click`、
  portal の `[data-slot="sidebar"][data-mobile="true"]` が **1 件・大きさ > 0** を主張
- 🟦 **K3 = 予測どおり通った**（3/3 緑）／ 🟦 **K1 = 素材層 diff 0 行**（連続記録を維持）

### C5-05b ★★★ 🟥 K8（予測していない箇所）—— 静的検査がコメントで通っていた（→ [DR-0101](DR/DR-0101-a-comment-satisfied-the-check-for-absence.md)）

**`Sidebar/Mobile` の JSDoc に `expectOpened('sheet-content')` と引用して書いた瞬間、検査の出力が変わった:**
...
| **コメントを落としてから走査する**（D11=A）＋ 同じことをする | 🟦 **赤**（`exit 1`・`sheet-content` が「開いた story が無い」） |
| 主張を戻す | 🟦 **緑**（担当も `Sheet.stories.tsx` に戻った） |

★★★ **「無いことを落とす検査」が、"書けば通る" 形だった。しかも書くのはコードでなくてよかった。**
🟨 **部品4 C4-04 の「詰まったら」欄が名指ししていた危うさ**（「grep が当たるかどうかに落ちると脆い」）**が 1 回で現実になった。**

### C5-05c D10=A — `aria-controls` の引き換えを `expectOpened` に畳み込んだ

**opt-in（該当 2 story だけで呼ぶ）にすると「書かなければ 0 件で緑」を作る**ので、
**`expectOpened` の中で毎回測る**（`aria-hidden` の中は見ない＝ axe と同じ理由）。

### C5-06 ゲート（`perth`・2026-08-09）

| ゲート | 実測 | 前回（`a1f3fe2`） |
| --- | --- | --- |
| typecheck | 🟦 緑 | 緑 |
| lint | 🟦 **error 50 / warning 1**（内訳一致・自作分 0） | 同 |
| build | 🟦 dist **85**・`design.mjs` **92,605 B**（**完全に不変**） | 同 |
| format:check | 🟦 緑 | 緑 |
| spell | 🟦 **356 ファイル / 0 件**（辞書 +3 = `viewports` / `datepicker` / `haspopup`） | 349 |
| storybook（story） | 🟦 **58 ファイル / 130 件**（+2） | 128 |
| **バー** | 🟦 **130 / 130 緑**（12.10 秒） | 128/128 |
| a11y（出荷物の棚） | 🟦 **critical 0** ／ **serious 148・色の組 9 種類**（**完全に不変**） ／ **保留 23**（種類つき） | 同 |
| （ゲート外）`opened-overlay-check` | 🟦 **7/7 緑** | 7/7 |
| （K5）`title-map-check` | 🟦 OK（story 58 題） | OK |
| （K6）`dependencies` | 🟦 **12 件のまま**（`package.json` の diff 0 行） | 同 |

🟥 **story を 2 本足したのに a11y の数字が 1 つも動かなかった。**
**これ自体が [OBS-0023](OBS/OBS-0023_台帳とバーが別のviewportを見ている.md) の実例**——
台帳は **1200×900 固定**なので、`Sidebar/Mobile` の `play` は**デスクトップの折り畳み**として作用し、
`Sidebar/Collapsed` ともども**中身が畳まれた姿**しか測っていない。
**バーが見た絵（320px のモバイル Sheet）は、台帳には 1 件も入っていない。**

### 締め —— 問いの答え

| # | 問い | 答え |
| --- | --- | --- |
| **Q1** | 保留の分類をどこに持つか | 🟦 **機械（`a11y-scan.mjs`）に持たせ、未分類は落とす**（D2=C）。**3 種類に割れた**（13 / 9 / 1）→ [DR-0100](DR/DR-0100-pending-judgements-split-into-three-kinds.md) |
| **Q2** | `nav` の重なりは本物の欠陥か | 🟨 **いまは違う**（日付セル 0 件が覆われている＝ 操作は壊れていない）。🟥 **`captionLayout="dropdown"` を出す日に踏む** → [OBS-0020](OBS/OBS-0020_カレンダーのnavが月ラベルを覆っている.md) |
| **Q3** | `Sidebar` の mobile を描くと何が出るか | 🟦 **道具の新設 0 で描けた**。🟥 **① 開く前は部品が 0 件 ② 面① は緑 ③ portal に出ても `sheet-content` を名乗らない**＝ **機械で引いた一覧にも穴が残る** |
| **Q4** | 面① は「部品が描かれた」を見ているか | 🟥 **見ていない**。**別のものが描かれていれば緑**＝ **3 つ目の緑の形** → [OBS-0021](OBS/OBS-0021_面1は部品が描かれたことを見ていない.md) |
| **Q5** | `collapsed` は面③ を広げる話か | 🟨 **広げない**（D8=B）。**1 部品固有の状態を面③ に入れると、残り 47 部品で毎回「対象 0 件で緑」を作る**。**代わりに story が持つ** |
| **Q6** | 台帳とバーは同じ絵を見ているか | 🟥 **見ていない**（実測で確定・上表）→ [OBS-0023](OBS/OBS-0023_台帳とバーが別のviewportを見ている.md) |
| **Q7** | 他のゲートにも保留の口があるか | 🟨 **範囲外にした**（D1=C）。候補を 7 本挙げて [OBS-0022](OBS/OBS-0022_他の機械ゲートにも保留の口があるか.md) に積んだ |

### 赤テストの結果（予測との突き合わせ）

| 検体 | 予測 | 実測 | 判定 |
| --- | --- | --- | --- |
| K1 | 素材層 diff **0 行** | 0 行 | 🟦 的中 |
| K2 | 未分類で赤・戻して緑 | そのとおり | 🟦 的中 |
| K3 | `Sidebar/Mobile` は通る | 通った | 🟦 的中 |
| K4 | `opened-overlay-check` は何も言わない | 🟥 **言った**（担当が奪われた）——**予測は「一覧に載らない」という意味では当たったが、副作用を見落としていた** | 🟨 半分 |
| K5 | 題材 0 件 | OK | 🟦 的中 |
| K6 | `dependencies` 0 増 | 0 増 | 🟦 的中 |
| K7 | 保留は **再現しない** | 🟥 **再現した**（ノード単位で完全一致） | 🟥 **外れ**（予測の根拠だった「1 件の食い違い」は、probe で `Avatar/States` を開いていなかっただけ） |
| K8 | 予測していない箇所が出る | 🟥 **3 件**——① **静的検査がコメントで通る**（DR-0101） ② **story を 2 本足しても a11y が 1 つも動かない**（OBS-0023 の実例） ③ **`aria-valid-attr-value` の impact は `critical`**（保留なので critical 0 の集計には出ない） | 🟦 的中 |

---

## 部品6 — 緑の面積を数える（2026-08-09・[手順書](手順/部品6_緑の面積を数える.md)・ブランチ `geneva`）

> 前提: 部品5 が done・[PR #19](https://github.com/yatami0/design/pull/19) マージ済み（`380b896`）。
> **部品は 1 件も増えていない**（48 のまま・**story 130 のまま**・**素材層 0 行**）。
> **足したのは道具 2 本と型の口 1 本**（`tools/coverage-scan.mjs` / `tools/self-check.mjs` / `tsconfig.tools.json`）。

### 締めの表 — 問いの答え

| # | 問い | 答え |
| --- | --- | --- |
| **Q1** | 「バー 130/130 緑」は何を主張しているのか | 🟥 **130 中 103（79%）は主張を 1 本も持たない**（出荷物の棚 115 中 88）。**掛かっているのは面①（何かが描かれた）と面②（axe が黙った）だけ**で、**面① は部品を特定せず、面② は `color-contrast` / `region` を無効化している**。→ [DR-0102](DR/DR-0102-green-must-be-read-as-an-area.md)・`pnpm coverage` |
| **Q2** | 測る側は誰が検査するのか | 🟥 **誰も検査していなかった**。`tools/*.mjs` **7 本 1,512 行が typecheck の射程外**（`checkJs` も無い）・**lint は型情報を使わない**・**自動テスト 0 ファイル**。🟥 **意図的な数え落としを入れてもゲート 7 本は 1 本も反応せず、台帳の数字だけが変わった**（保留 23 → 10）。→ [DR-0103](DR/DR-0103-the-measuring-side-had-no-net.md) |
| **Q3** | 他の 6 ゲートに保留の口はいくつあるか | 🟦 **7 本すべて数えた**（[OBS-0022](OBS/OBS-0022_他の機械ゲートにも保留の口があるか.md) §5）。🟥 **`format:check` は 155/378（41%）**・**`spell` の辞書は 18 → 90 語と単調増加**・**`tsc` の借金は 6 件 / 7 件**。★ **OBS-0022 が挙げていなかった Q2 がいちばん大きかった** |
| **Q4** | 面① を部品に紐づけられるか | 🟥 **いまの材料では機械で要求できない**。**厳密一致だと出荷物 115 のうち 65 件が落ち、うち 21 件は `data-slot` を出さない自作部品**。**前置一致を許すと `Dialog/Default`（`dialog-trigger` のみ）が通り、穴がそのまま残る**。→ [OBS-0021](OBS/OBS-0021_面1は部品が描かれたことを見ていない.md) を closed |
| **Q5** | 射程の数字をどう載せるか | 🟦 **完成バー §1 に「被覆」列**・**台帳 §5.5 / §5.6**・**handoff のベースライン表に 2 行**。🟥 **閾値は置かない**（D2=B）——**置く条件は「被覆が 1 度でも下がったら」**（D9） |

### C6-01 現況（`main` = `380b896`）

| ゲート | 実測 | 前回（部品5）比 |
| --- | --- | --- |
| typecheck | 🟦 緑 | 同 |
| lint | 🟥 error **50** / warning **1** | 内訳一致 |
| build | 🟦 dist **85**・`design.mjs` **92,605 B** | **完全に不変** |
| format:check | 🟦 緑 | 同 |
| spell | 🟦 **356 ファイル / 0** | 同（表と一致） |
| storybook | 🟦 **58 ファイル / 130 件** | 同 |
| バー | 🟦 **130 / 130 緑**（12.56 秒・setup 61.9 秒） | 同 |
| a11y | 🟦 critical 0 ／ 🟨 serious **148**・色の組 **9** ／ 保留 **23**（未分類 0・新しい組 0） | **完全に一致**（部品5 K7 の「ノード単位で再現する」を追認） |
| （ゲート外）`opened-overlay-check` | 🟦 **7 / 7** | 同 |

### C6-02 緑の面積（Q1・D2=B）

**`tools/coverage-scan.mjs` を作った。**コメントを落としてから走査する（[DR-0101](DR/DR-0101-a-comment-satisfied-the-check-for-absence.md)）。

| 面 | 掛かった / 対象 |
| --- | --- |
| ① 描画された | 130 / 130（🟥 部品を特定しない） |
| ② a11y | 130 / 130（🟥 serious は落ちない） |
| ③ 状態面 | 🟥 **0 / ?**（実装が無い） |
| ④ 語彙の効果 | 🟥 **10 / 22（45%）** |

**主張**: `await expect` 77 ／ `expectOpened` 8 ／ `expectFocusTrapped` 4 ＝ **89 本**。
**`play` を持つ story 27（21%）／ 主張が 1 本も無い story 103（79%）／ 1 本も主張を持たない部品 40 / 58 ファイル。**

★★ 🟥 **数え方で 110 と 103 に割れた**——**初版は `await expect(` だけを数えて 7 story を落としていた**
（`expectOpened` / `expectFocusTrapped` は自身が `expect` を内蔵し、story 側には `await expectOpened('…')` としか書かれない）。
🟦 **自己検査（「主張が 0 本なら走査が壊れている」）が先に発火して気づいた。**

★★★ 🟥 **面④ の母数 22 は、この回で初めて機械が引いた。**
**語彙の書かれ方は 3 通り**（文字列リテラル union **8** ／ `keyof typeof` **10** ／ `cva`+`VariantProps` **7**）で、
🟥 **1 通りだけ見ると 12 件しか出ず、`Button` も `Box` も落ちる。**
→ **台帳 §3 の「面④ の借金 6 部品」は目で数えたもので、機械の答えは 12 部品だった**（§3 に訂正を追記）。

★★ 🟥 **もう 1 つ、自分の道具で誤りを踏んだ**——**棚の判定を `EXCLUDED.includes(shelf)` と完全一致で書き、
`⑤ 題材（Redmine）` の 2 story を出荷物の棚に数えていた**（`a11y-scan.mjs` は `startsWith` で見ている）。
**出荷物の棚 117 → 115 に訂正。**

### C6-03 K2 — 被覆の目盛りは動く（両方向）

| 検体 | 結果 |
| --- | --- |
| `Box.stories.tsx` の `await expect` を潰す | 🟦 **主張を持つ story 27 → 26** ／ 戻すと 27 |
| `Box` の測り方（`styleOf` / `resolveLength`）を全部潰す | 🟦 **面④ 10/22 → 9/22**・`box` が 🟦 → 🟥 ／ 戻すと復帰 |
| 語彙の抽出経路（`cva`）を壊す | 🟦 **exit 1**（「語彙の経路 cva が 0 件」）／ 戻すと **exit 0** |

### C6-03b K4 — 測る側を壊しても誰も気づかない（Q2）

**検体**: `tools/a11y-scan.mjs` の `axe.run(document)` を `axe.run(document, { resultTypes: ['violations'] })` に戻す
（＝ 部品4 D8=B より前の形・**保留を数えなくする**）。
...
🟨 **副産物**: `resultTypes: ['violations']` は incomplete を **0 にはしない**——**ノードを 1 件に切り詰める**だけ
（23 → 10）。**旧実装が「捨てていた」のは件数ではなくノードの内訳だった。**

### C6-04 測る側に網を掛ける（Q2・D4=D）

**① 型（K3）**: `tsconfig.tools.json`（`checkJs: true`）＋ `pnpm typecheck:tools` を新設。
...
| 🟦 **本回の新規 2 本** | 🟦 **0**（＝ eslint の「自作分の赤は 0」を型にも当てた） |
| **合計** | 🟥 **56**（🟥 **予測 10〜40 は外れた**） |

規則別: `TS7006` 23 ／ `TS7053` 10 ／ `TS2345` 6 ／ `TS7034` 4 ／ `TS7005` 4 ／ `TS2339` 4 ／ `TS7031` 3 ／ `TS7023` 1 ／ `TS7015` 1。

🟥 **本体の `tsconfig.json` には混ぜない**——**eslint のベースライン 50 件と別種の 56 件が同じ場所に積まれると「新しい赤」が見えなくなる。**

**② 赤テストの固定**: `tools/self-check.mjs` ＋ `pnpm self-check`（**8 / 8**）。
**検体は `tmp/self-check/` に書き、走査先を環境変数で向ける**（`src/` は 1 バイトも触らない）。

**③ 赤テストの赤テスト（両方向）**: `opened-overlay-check.mjs` から `stripComments` を外して
[DR-0101](DR/DR-0101-a-comment-satisfied-the-check-for-absence.md) の穴を戻す → **`pnpm self-check` が 7/8・exit 1**
（落ちたのは「コメントに書いただけでは通らない」の 1 本）→ 戻すと **8/8・exit 0**。
🟦 **本番の `opened-overlay-check` は 7/7 のまま。**

### C6-05 面① の紐づけ（Q4・K7）

**手法**: `storybook-static` を立てて 130 story を開き、`[data-slot]` を全部拾って部品名（`title` の末尾）と突き合わせた（一時 probe・測定後に削除）。
...
🟥 **K7 の予測「10〜30 件落ちる」は外れた**——**厳密一致だと出荷物 115 のうち 65 件が落ちる。**
🟥 **前置一致を許すのは解にならない**——**`Dialog/Default` が持つのは `dialog-trigger` だけで、本体は描かれていない**
＝ **[OBS-0021](OBS/OBS-0021_面1は部品が描かれたことを見ていない.md) が指摘した穴がそのまま残る。**
→ **D5 は B（数えるだけ）に確定。**理由は 2 つとも実測——① 前置一致は要求として無意味 ② 厳密一致は出荷物の DOM を変える判断。

### C6-06 他ゲートの保留（Q3・D6=A・D7=B）

**7 本ぶんの実測表は [OBS-0022](OBS/OBS-0022_他の機械ゲートにも保留の口があるか.md) §5。**主な数字——

- 🟥 **`format:check` は 155 / 378（41%）**。ignore 219 のうち **`docs/**` の md が 176**
- 🟥 **`spell` の辞書は 18 → 90 語**（28 コミットで**単調増加・1 度も減っていない**）。
  ★ **部品6 は 0 語追加**——**2 回赤くなったが、どちらも辞書ではなくコードの綴りを直した**（初めて「足さない」を選んだ回）
- 🟥 **`exactOptionalPropertyTypes: true` → 6 件**（素材層 3 = `slider` / `dropdown-menu` / `calendar` ／ 自作 3 = `templates/AppShell` / `mocks/data` / `vitest.config`）
  ＝ **手3 の「`dropdown-menu.tsx:94` の 1 件」から 6 件に増えている**
- 🟦 **`skipLibCheck: false` → 7 件で全部 `node_modules`**（storybook 4 / ast-types 3）＝ **我々の型ではない**
- `eslint` の warning 1 は **`react-hooks/incompatible-library`**（TanStack Table の `useReactTable`）。`--fix` 可の error は **4 件**

### K1〜K8 の答え合わせ

| # | 予測 | 実測 |
| --- | --- | --- |
| K1 | 素材層 0 行 | 🟦 **0 行**（連続記録を継続） |
| K2 | 被覆の目盛りが両方向で動く | 🟦 **動いた**（3 通りとも） |
| K3 | 型エラー 10〜40 件 | 🟥 **外れ。56 件** |
| K4 | 誰も気づかない | 🟦 **当たり。ゲート 7 本は 1 本も反応しない** |
| K5 | 出荷入口 4 本は動かない | 🟦 **動かない**（`title-map-check` 緑・dist 不変） |
| K6 | `dependencies` 0 増 | 🟦 **12 件のまま**（`devDependencies` も 30 のまま＝ **型定義も足していない**） |
| K7 | 面① の紐づけで 10〜30 件落ちる | 🟥 **外れ。65 件**（うち 21 件は原理的に不可能） |
| K8 | 予測していない箇所が 1 件以上 | 🟦 **4 件**（下記） |

### 🟥 K8 — 予測していなかったもの 4 件

1. ★★★ **主張の数え方で 110 と 103 に割れた**——**`await expect(` だけでは 7 story を落とす。**
   🟦 **自分で書いた自己検査が先に発火して気づいた**（＝ **測る側に自己検査を入れる価値の実例**）。
2. ★★ **棚の判定を完全一致で書いて 2 story を出荷物の棚に数えた**——**`a11y-scan` は `startsWith` で見ている。**
   🟥 **同じ集合を 2 か所に書いた瞬間に食い違った**（[DR-0090](DR/DR-0090-token-classes-were-silently-dropped-by-tailwind-merge.md) の宿題 ①「語彙の写しが 2 箇所」と同じ形）。
3. ★★ 🟥 **`cspell` が「綴りについての説明文」まで検査する**——
   **正規表現の単語境界 `\b` が直後の識別子と繋がって未知語になり、
   それを説明したコメント自身がまた同じ未知語を作って赤くなった**（**本回 4 回踏んだ**：
   コード 2 回 ＋ **そのコードについて書いた本文 2 回**）。
   ★ **辞書に足せば消えるが、足すこと自体が Q3 で数えている「保留」**なので、
   **綴りを作らない形に書き換え続けた**（結果、**辞書 +0 語**）。
4. ★★ 🟥 **自分のシェルの数え方を 3 回間違えた**——
   `^tools/[a-z-]+\.mjs` が **`a11y-scan.mjs`（数字を含む）を落とし**（48 と 73 に食い違った）、
   `node ... | tail` の `$?` が **tail の終了コード**を読んだ（2 回）。
   ★ **本回の主題（測る側が測られていない）を、記録している側で 3 回再演した。**
...
| --- | --- | --- |
| typecheck | 🟦 緑 | 同 |
| lint | 🟦 **error 50 / warning 1** | 🟦 **内訳一致・自作分 0**（`tools/` は 9 本とも 0/0） |
| build | 🟦 dist **85**・`design.mjs` **92,605 B** | 🟦 **完全に不変** |
| format:check | 🟦 緑 | 同 |
| spell | 🟦 **362 ファイル / 0** | 🟦 **辞書 +0 語**（90 のまま） |
| storybook | 🟦 **58 ファイル / 130 件** | 🟦 **±0**（story を 1 本も足していない） |
| バー | 🟦 **130 / 130 緑** | 同 |
| a11y | 🟦 critical 0 ／ 🟨 serious **148**・色の組 **9** ／ 保留 **23**（未分類 0・新しい組 0） | 🟦 **完全に不変** |
| （ゲート外）`opened-overlay-check` | 🟦 **7 / 7** | 同 |
| 🆕 （ゲート外）`typecheck:tools` | 🟥 **56 件**（新設・ベースライン） | 🆕 |
| 🆕 （ゲート外）`self-check` | 🟦 **8 / 8** | 🆕 |
| 🆕 （ゲート外）`coverage` | 🟥 **面④ 10/22 ／ 主張 0 本 103/130** | 🆕 |

## 工程5 — 稼働表（ピボット）（2026-08-10・[手順書](手順/工程5_稼働表_ピボット.md)・ブランチ `bangkok-v1`）

> 前提: 部品6 が done（[PR #20](https://github.com/yatami0/design/pull/20)・`4b41feb`）。
> ★★★ **ユーザー指示で部品軸を中断し、工程軸へ戻った回**（[段取り §3b](工場の段取り.md)）。
> 🟦 **確認待ちは 0 件**（D7 は部品3 D7=A で決着済み）。

### P5-01 現況（`main` = `4b41feb`）＋ 予測の登録

🟦 **環境の穴 ① は 4 回連続で塞がったまま**——新しい worktree で `node_modules` が無い状態から
`pnpm i --frozen-lockfile` が **exit 0**・`git status` は clean（`pnpm-workspace.yaml` が生えない）。

| ゲート | 実測 | 手順書 §1 の現行表との差 |
| --- | --- | --- |
| `pnpm typecheck` | 🟦 緑 | 一致 |
| `pnpm lint` | 🟥 error **50** / warning **1** | 🟦 **内訳まで一致**（`no-arbitrary-value` **39** / `restrict-template-expressions` **5** / `no-confusing-void-expression` **4** / `no-unnecessary-condition` 1 / `set-state-in-effect` 1 / warning は `incompatible-library`） |
| `pnpm build` | 🟦 緑・dist **85**・`design.mjs` **92,605 B** | 一致 |
| `pnpm format:check` | 🟦 緑 | 一致 |
| `pnpm spell` | 🟦 緑・**362 ファイル** | 一致 |
| `pnpm build-storybook` | 🟦 緑・story **58 ファイル / 130 件** | 一致 |
| `pnpm test-storybook`（バー） | 🟦 **130 / 130 緑**（12.88 秒・setup 62.8 秒） | 一致 |
| （ゲート外）`opened-overlay-check` | 🟦 **7 / 7** | 一致 |
| （ゲート外）`self-check` | 🟦 **8 / 8** | 一致 |
| （ゲート外）`typecheck:tools` | 🟥 **56 件** | 一致 |
| （ゲート外）`coverage` | 🟥 面④ **10 / 22（45%）**・主張 0 本 **103 / 130（79%）** | 一致 |

🟦 **11 本すべてベースラインと完全一致。**この工程の差分は、ここからの差分だけ。

#### 予測（🟥 測る前に書く・[DR-0076](DR/DR-0076-capture-the-run-not-just-the-output.md)）

🟥 **登録前に「実行されうるか」を検算した**（工程3 の失敗）——
**予2〜予4 は P5-04（`DataGrid` で組む）が走らないと測れない。**
P5-04 は D1=B が明示的に要求している経路なので、**走る**。
予7 は `GET /time_entries.json` が既定 limit **25**・`MAX_LIMIT` **100** で返すことを実測済み（`src/mocks/handlers.ts` L23-36）なので、**総件数を数えれば必ず判定できる。**

| # | 予測 | 判定の材料 |
| --- | --- | --- |
| **予1** | `DataGrid` で 90 日ぶんのピボットは**書ける**（`columns` はただの配列で、`accessor` に日付をクロージャで閉じ込めればよい） | P5-04 |
| **予2** | 🟥 **破れるのは 3 点**——① **セル単位の強弱**（`kind` / `emphasis` は列単位） ② **行ヘッダ（人名）の固定**（素材層 `table` は `overflow-x-auto` のみ） ③ **合計行**（`DataGrid` に footer の概念が無い） | P5-04・P5-05 |
| **予3** | 濃淡は **① 層の語彙では表せない**（`tokens.css` の色は semantic 2 系統・段階を持つ色 **0**）→ **D2=B（部品の内部実装）**に倒れる | P5-06 |
| **予4** | 対応表は **2 本になる**（D4=A）。🟥 **共通で残るのは `resolvePeriod` まで**で、割れるのは「範囲 → クエリ」の 1 段だけ | P5-03 |
| **予5** | 畳み込みは**題材に収まる**（D3=B）。コアは `TimeEntry` を知らないまま済む | P5-03・P5-04 |
| **予6** | Q5 = 役割 9 カテゴリに**席が無い**（DataDisplay に pivot は無い）→ **指摘 15**（4 件目） | P5-05 |
| **予7** | 🟥 **K1 はページングを踏む**——`time_entries` の総件数は既定 limit **25** を超える。**総和を合わせるには limit を上げるか全ページ取るかが要る** | P5-04（K1） |
| **予8** | 🟥 **予測していない箇所が 1 件以上出る**（工程2〜4・部品1〜6 で連続的中） | 全体 |

### P5-03 取得と畳み込み（D3=B・D4=A）＋ K2

**足したもの**（すべて題材の層）:

| ファイル | 中身 |
| --- | --- |
| `src/redmine/period.ts`（追記） | `toSpentOnParams` / `periodToSpentOnParams`＝ **2 本目の対応表** |
| `src/redmine/workload.ts`（新規） | `foldWorkload`（`TimeEntry[]` → 人 × 日）／ `toIntensity` ／ `isNonWorkday` |
| `src/redmine/useWorkload.ts`（新規） | 取得 hook（🟥 **全ページ取る**） |
| `src/mocks/handlers.ts`（1 行） | `console.info('[msw] GET /time_entries.json…')`＝ K2 の証拠の口 |

★★ **Q3 の答え（予4 的中）**——**対応表は 2 本になり、共通で残ったのは `resolvePeriod`（語 → 範囲）まで。**
**割れたのは「範囲 → クエリ」の 1 段だけ**で、`PeriodSelect`（コア）は 1 つのまま。

🟥 **一覧の変換を流用していないことを機械で確かめた**（K2-b）——飛んだ URL に `updated_on` は 1 度も載らない。

★ 🆕 **予測に無かった発見: `all`（全期間）ではピボットが組めない。**
一覧は「絞らない」で成立するが、**ピボットは列の軸を範囲から引く**ので範囲が無いと表が無い。
🟥 **`PeriodSelect` のコメントは `all` を「EVM / 稼働表では『全期間』として意味を持つ」という理由で足したと書いている**が、
**日ごとの列を持つ表では意味を持たなかった**（→ story `AllPeriodCannotPivot` で固定）。

★ **ページングは踏んだ（予7 的中）。**既定 limit **25** に対し、今週 **34 件** / 四半期 **139 件**。
🟥 **`limit=100`（`MAX_LIMIT`）でも四半期は 2 ページ。**
→ **hook は全ページ取り、`entries.length !== total_count` なら throw する**（黙って足りない表を出さない）。

### P5-04 `DataGrid` で組んでみる（D1=B）＋ K1・K6

🟥 **組めた。**`th` **94 本**（担当 ＋ 92 日 ＋ 合計）。合計も API と一致（下表）。
**そのうえで 4 点破れた** → [DR-0105](DR/DR-0105-split-a-component-on-readability-not-writability.md)。

| # | 破れた項目 | 予測 | 実測 |
| --- | --- | --- | --- |
| ① | セル単位の符号化ができない（`<td>` の面に手が届かない） | 🟦 予2 ① | 構造から確定 |
| ② | 行ヘッダ（人名）を固定できない | 🟦 予2 ② | 🟥 **K6-f FAIL**: 右端までスクロールすると `佐藤 花子` が器の外 |
| ③ | 合計行が出せない（footer の概念が無い） | 🟦 予2 ③ | `data` に混ぜると `WorkloadRow.user` に実在しない人が要る（D10=B） |
| ④ | 🆕 **列見出しの強弱が出せない**（土日を落とす等） | 🟥 **予測に無かった** | `<th>` の書式は部品が固定・`DataGridColumn` に列見出しの語彙が 0 |

🟥 **4 点のどれも tsc / lint / build を緑で通る。**

### ★★★ P5-04b 完成バーが初めて赤になった（→ [DR-0104](DR/DR-0104-the-dom-never-overflowed.md)・D9）

**92 列の story を足したら `pnpm test-storybook` が落ちた。**落ちたのは自分で書いた主張ではなく、
**全 story に自動で掛かる面②（axe）**——**`scrollable-region-focusable`（serious）**。
...
★★ **この規則は 1 度も無効化されていない**（無効なのは `color-contrast` と `region` の 2 つだけ）。
**器も欠陥も `shadcn add` の初日から在り、チケット一覧（6 列）も同じ器を使っている。**
🟥 **足りなかったのは検査でも規則でもなく「溢れる表」**——`Overflow` という名の story は 4 本あるが、**どれも表ではない。**

🟦 **塞ぎ方は D9=B（製品層ラッパー）。**
🟥 **包み直しでは解けない**ことが実測で分かった——外にもう 1 枚 `overflow-x-auto` を足しても、
内側の `table-container` が `w-full` で先に溢れるので**赤くなるのは内側のまま**。
→ **器のマークアップ 4 行を写した**（🟥 **素材層 16 ファイルは `shadcn add` の 1 コミット以来、1 度も書き換えられていない**——
全ファイル履歴 1 件。ここで折ると「上流を差し替えたら消える修正」になる）。

**赤 → 緑の対**: `132/133`（exit 1） → **`137/137`（exit 0）**。

### P5-05 Q1 の答え ＋ D5（→ 指摘 15）

**新設した**（`src/components/DataDisplay/PivotTable.tsx`・**48 → 49 部品**）。**破れた 4 点がそのまま設計になった。**

🟥 **D5 の一次情報確認は「置き場が無い」で終わった**（2026-08-10 実測）——

| 一次情報 | ピボットの扱い |
| --- | --- |
| Ant Design | 🟥 **部品として存在しない。**`pivotMode` を `Table` に足す**未実装の feature request**（issue #45238）があるだけ |
| Carbon Design System | 🟥 **部品として存在しない。**`Data table` があるのみ |

→ **DataDisplay に置いた根拠は「由来の一次情報」ではなく「先例」**（`Table` / `DataGrid` が既にそこに居る）＝ **指摘 14 より 1 段弱い**。
★ **「席が無い」の意味が 1 つ増えた**——指摘 14 の 3 件は「思想が挙げ忘れた部品」（Ant には在る）だったが、
**今回は「業界の標準部品にもなっていない領域」**。→ **[指摘 15](共通コンポーネント思想への指摘.md)**（予6 的中・**4 件目**）。

### P5-06 濃淡（D2=B）＋ K3・K8

★★ **Q2 の答え: ① 層では表せない。**しかも理由は「段階が無い」ではなかった——

- `tokens.css` の色は **semantic 2 系統**（線 `--color-success` / `--color-warning` ＋ 面 `--color-fill-*` 4 つ）。**段階を持つ色は 0。**
- 🟥 **面色は tint 1 段として設計されている**（tmp-admin §4.5「状態 = tint pill + 色ドット」）ので、
  **段階の素材にならない**（amber-50 を 30% / 60% にしても白と見分けが付かない）。

🟦 **D2=B（部品の内部実装）。段階は「色」ではなく「不透明度」で作った**——
色は ① 層の `--color-warning` **1 つだけ**を使い、刻み（15 / 30 / 50 / 70）は部品の中。
→ **① 層の色語彙は 1 語も増えていない**（**K8 = 対象 0 件**・`tw-merge.ts` は無変更）。

**K3（実効色を読む）は story `Intensity` で固定した**（面④）:
`none` は `rgba(0,0,0,0)` ／ **`low`〜`peak` は 4 つとも違う実効色** ／ **不透明度が単調に増える**。

🟥 **K3 の初版は throw した**——**実効色は `rgba()` では返ってこない**。
Tailwind v4 は色を oklab で出すので `getComputedStyle` は `oklab(0.765 0.081 0.156 / 0.15)` を返す。
★ **投げてくれたから気づけた**（`measure.ts` の「見つからなければ throw」と同じ作法が効いた）。

### P5-07 画面と story（K4・K7）

- **K4-a（画面層の任意値）**: `w-[220px]` / `p-4` / `text-gray-600` を書いたら **3 件とも赤**
  （`tailwindcss/no-arbitrary-value` ＋ 工程3 D11 の画面層ブロック 2 件）。戻して 50/1 に復帰。
- **K4-b（語彙外の値）**: `intensity: 'ultra'` → `tsc` が
  `Type '"ultra"' is not assignable to type '"none" | "low" | "mid" | "high" | "peak" | undefined'`。
- **K7（`title-map-check` の初運用）**: 🟦 **落ちた**——
  `🟥 出荷しない棚の story が titleMap に無い: "⑤ 題材（Redmine）/稼働表"（key = "稼働表"）`。
  `.design-sync/config.json` に `"稼働表": null` を足して緑（story 60 題）。
  ★ **工程4 D4=B で置いた検査が、初めての実利用で仕事をした。**
- **Q4（画面の `className`）**: 🟦 **0 件を保った**（`Workload.tsx` の `className` は 0。
  `IssueDetail.tsx` の 1 件は**コメント本文**であってコードではない）。

### ★★ P5-08a 計測器が 3 本つづけて赤くなった（D11・D12・D13・D14）

**ゲート 7 本は通ったのに、ゲート外の計測器が順に落ちた。**この回でいちばん枝が伸びた場所。

#### ① `coverage` の母数が新部品を数えていなかった（D11=A）

**面④ の母数は 22 のまま**で、`PivotTable.intensity`（5 語）が入らなかった。
🟥 **部品6 は「語彙の書かれ方は 3 通り」と結論したが、4 通り目があった**——
`export type X = (typeof CONST)[number]`（`as const` 配列からの導出）。

★★ **落ちていたのは新部品だけではない。**`PERIOD_PRESETS`（工程3）と `DATE_PICKER_MODES`（部品3）も同じ書き方で、
**ずっと母数の外に居た**（`PeriodSelect` が母数に居たのは別 prop `width` のおかげ）。

🟥 **4 通り目を足したら 5 通り目も出た**——`export type X = 'a' | 'b'` に**名前を付けた**もの
（`DataGridColumnKind`（手8d）・`StatusTone`（工程2））。**経路 (i) は prop 宣言への直書きしか見ていない。**
★ **「何通りあるか」を数え切った証拠は無い。**

#### ② `coverage` の分子が「道具の使い方」で数えていた（D12=A）

`PivotTable` の `Intensity` story は**実効色を 5 段ぶん読んで主張している**のに **0 と数えられた**——
判定が `measure.ts` の 4 関数の呼び出しでしか立たなかったため。
🟥 **`measure.ts` は「安全な書き方」であって「面④ の定義」ではない**（存在理由は「見つからなければ throw」）。
→ `getComputedStyle(` を判定に足した。**副作用で `DataGrid` / `DescriptionList` / `Link` / `StatusPill` も
「書いてあったのに数えられていなかった」ことが判明した。**

| 面④ | 前 | 後 |
| --- | --- | --- |
| 掛かった / 対象 | 10 / 22（45%） | 🟦 **15 / 27（56%）** |
| 経路 | literal 8 / keyof 10 / cva 7 | literal 8 / **literalAlias 4** / keyof 10 / **constUnion 3** / cva 7 |

🟨 **被覆は下がらず上がった**が、🟥 **数字の意味が変わった**（部品6 D9 の「下がったら止める」は
「実装が退化したとき」の閾値であって、「測り方が正確になったとき」ではない）。

#### ③ `self-check` が 8/8 → 5/8 に落ちた（🟦 網が仕事をした）

**coverage を書き換えたら、固定してある赤テストが 3 本落ちた**——
検体の部品ファイルに経路が 1 つしか無く、「5 経路のどれかが 0 件なら抽出が壊れている」で exit 1 になった。
🟦 **検体に 5 経路を全部持たせて 8/8 に戻した**（＝ **赤テストが 5 経路すべての抽出を毎回確かめる**ようになった）。
★ **部品6 が置いた網が、置いた翌工程で実際に発火した。**

#### ④ `a11y-scan` が exit 1（D13=A・D14=A）

- **未分類 105 件**（全部 `color-contrast / elmPartiallyObscured` / `⑤ 題材/稼働表`）。
  🟥 **これも「横に溢れる表が 1 つも無かった」ために 1 度も出ていない**（[DR-0104](DR/DR-0104-the-dom-never-overflowed.md) と同じ根）。
  → **既知の `bg-overlap` と同じ種類**として `classify()` に足した（axe の messageKey が違うだけで意味は同一）。
- **新しい色の組 2 種類**——🟥 **どちらも「薄い文字を面の上に置いた」もの**:
  `#8a7d70 on #ffdfb3`（`muted` 列 × 濃い `intensity`・約 3.2） ／
  `#88888d on #f9f9fb`（`muted` 列 × 合計行の地・約 3.4）。
  ★ **`muted`（この列は重要でない）と面（値が大きい／合計行）は独立な軸で、重ねると
  「重要でない列の、値が大きいセル」が最も読みにくくなる**——**意味と読みやすさが逆向き。**
  → **D14=A: `muted` の文字落としは「面が無いとき」だけ。**新しい色の組は **2 → 0** になった。

### P5-08b ゲート 7 本 ＋ K5 ＋ 計測器

| ゲート | 実測 | ベースライン（`4b41feb`）比 |
| --- | --- | --- |
| `pnpm typecheck` | 🟦 緑 | 同 |
| `pnpm lint` | 🟥 error **50** / warning **1** | 🟦 **内訳まで一致・自作分の赤は 0** |
| `pnpm build` | 🟦 dist **86**・`design.mjs` **94,578 B** | **+1 ファイル / +1,973 B** |
| `pnpm format:check` | 🟦 緑 | 同 |
| `pnpm spell` | 🟦 **369 ファイル / 0** | 🟦 **辞書 +0 語**（+7 ファイル） |
| `pnpm build-storybook` | 🟦 緑・story **60 ファイル / 137 件** | **+2 ファイル / +7 story** |
| `pnpm test-storybook`（バー） | 🟦 **137 / 137 緑** | 🟥 **途中 1 度赤**（P5-04b） |
| （ゲート外）`pivot-probe` | 🟦 **13 / 13**（新設） | 🆕 |
| （ゲート外）`opened-overlay-check` | 🟦 **7 / 7** | 同 |
| （ゲート外）`self-check` | 🟦 **8 / 8** | 🟥 **途中 5/8**（P5-08a ③） |
| （ゲート外）`typecheck:tools` | 🟥 **78** | **+22**（全部 `pivot-probe.mjs`・**既存 probe と同じ 6 種類の implicit any**。新種は 0） |
| （ゲート外）`coverage` | 🟦 面④ **15 / 27（56%）** ／ 🟥 主張 0 本 **105 / 137（77%）** | 面④ **+11pt**（母数の直しを含む） |
| （ゲート外）`a11y-scan` | 🟦 exit 0・critical **0** / serious **150**・色の組 **9** ／ 保留 **23**（未分類 0・新しい組 0） | serious **+2**・**色の組は不変**・**保留は同数** |
| （ゲート外）`title-map-check` | 🟦 OK（story 60 題） | 🟥 **途中 1 度赤**（K7・初運用） |

**K5（出荷面の diff・3 本別々に）**

| 本 | 前 | 後 | 差 |
| --- | --- | --- | --- |
| JS | `design.mjs` 92,605 B | **94,578 B** | **+1,973 B**（`PivotTable` のみ） |
| `.d.ts` | 85 ファイル | **86** | 🟦 **`dist/components/DataDisplay/PivotTable.d.ts` の 1 件だけ** |
| 静的（`publicDir`） | — | — | 🟦 **増減 0** |

🟦 **題材の混入 0**（`dist` を `redmine` / `Workload` / `foldWorkload` で grep して 0 件）。
🟦 **`dependencies` 増 0**（`package.json` / `pnpm-lock.yaml` は無変更）。
...
| # | 予測 | 結果 |
| --- | --- | --- |
| 予1 | `DataGrid` で 92 列は書ける | 🟦 **的中**（`th` 94 本・合計も一致） |
| 予2 | 破れるのは 3 点 | 🟨 **半分**——3 点は的中したが**実際は 4 点**（列見出しの強弱） |
| 予3 | 濃淡は ① 層で表せない → D2=B | 🟦 **的中**。🟥 **理由は予測より深い**——「段階が無い」ではなく「面色が tint 1 段として設計されている」 |
| 予4 | 対応表は 2 本・共通は `resolvePeriod` まで | 🟦 **的中** |
| 予5 | 畳み込みは題材に収まる | 🟦 **的中**（コアは `TimeEntry` を知らないまま済んだ） |
| 予6 | Q5 = 席が無い → 指摘 15 | 🟦 **的中**。★ **一次情報にも席が無かった**のは予測外 |
| 予7 | K1 はページングを踏む | 🟦 **的中**（今週 34 / 四半期 139・`limit=100` でも 2 ページ） |
| 予8 | 予測していない箇所が 1 件以上出る | 🟦 **的中・6 件**（下記） |

**予8 の内訳（予測していなかったもの 6 件）**

1. ★★★ **バーが初めて赤になった**（`scrollable-region-focusable`）→ [DR-0104](DR/DR-0104-the-dom-never-overflowed.md)
2. **`all` ではピボットが組めない**（一覧には無かった状態）
3. **`coverage` の語彙経路が 3 通りではなく 5 通りだった**（D11）
4. **`coverage` の分子が「道具の使い方」で数えていた**（D12）
5. **`muted` × 面 で新しい色の組が 2 種類出た**（D14）
6. 🟥 **probe の初版が Storybook 自身の引数表を測っていた**（`最終列 = Control` / `1 列目 = propertyName`）
   ＝ **測る側が壊れる**の 3 度目（[DR-0103](DR/DR-0103-the-measuring-side-had-no-net.md) の主題を、また記録する側で再演した）

#### 問いの答え

| # | 問い | 答え |
| --- | --- | --- |
| **Q1** | `DataGrid` を拡張するか `PivotTable` を新設するか | 🟥 **新設した。理由は「書けなかったから」ではなく「書けたものが読めなかったから」**（4 点）→ [DR-0105](DR/DR-0105-split-a-component-on-readability-not-writability.md) |
| **Q2** | 濃淡はトークン語彙で表せるか | 🟥 **表せない。**色は semantic 2 系統・段階 0 で、**面色は tint 1 段として設計されている**＝ 段階の素材にならない。→ **D2=B（部品の内部実装）／段階は不透明度で作り、① 層の色語彙は 1 語も増やさなかった** |
| **Q3** | 「書き換わるのは 3 点だけ」は別のクエリ形でも保たれるか | 🟦 **保たれた。**対応表が 2 本になり、**共通で残ったのは `resolvePeriod`（語 → 範囲）まで**。割れたのは「範囲 → クエリ」の 1 段だけ。**`updated_on` は 1 度も飛んでいない**（機械で確認） |
| **Q4** | 畳み込みはどの層が持つか | 🟦 **題材。**[DR-0092](DR/DR-0092-the-core-holds-the-vessel-not-the-state.md) の射程は狭まらなかった——**コアは畳んだ結果だけを受け取り、`TimeEntry` も「稼働」も知らない** |
| **Q5** | 席が無い部品が 4 件目になるか | 🟥 **なった**（→ [指摘 15](共通コンポーネント思想への指摘.md)）。★ **今回は一次情報（Ant / Carbon）にも席が無く、置き場の根拠が「先例」しか無い** |

#### [段取り 未決 #7](工場の段取り.md)（規約文書の起草時機）— 3 度目の問い

🟨 **答えは「まだ」**（工程3・工程4 に続き 3 度目）。**使い回し先は依然 0。**
🟦 **ただし材料は 2 本増えて 4 本になった**——
[DR-0104](DR/DR-0104-the-dom-never-overflowed.md)（**条件未到達で緑**という 4 つ目の型）と
[DR-0105](DR/DR-0105-split-a-component-on-readability-not-writability.md)（部品を分ける基準）。
🟥 **起草しない理由は変わっていない**（規約は使う人が居て初めて検算できる）。工程6 の締めでもう一度問う。

#### D8=B の記録

**[OBS-0016](OBS/OBS-0016_headerが名指ししていない出荷部品8件をどう埋めるか.md) の検体が 8 → 9 件になった**（`PivotTable` が 9 件目）。
🟥 **header は書き換えていない**——この工程は design agent に組ませないので、書き換えても効果が観測できない。
