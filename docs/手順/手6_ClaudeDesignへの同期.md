...
### 🆕 D8（実行中に出た論点・H6-06 で追記）

**本体と Storybook でフォントが食い違っている。**

`src/app/layout.tsx` は `next/font/google` の **Geist** を読み込んで `--font-sans` を `<html>` に挿すが、
**Storybook は `layout.tsx` を実行しない**ので `--font-sans` が未定義になり、
`globals.css` の `@theme inline { --font-sans: var(--font-sans) }` が**自己参照**で解決不能になって
**ブラウザ既定（セリフ体）に落ちる。**

🟥 **compare は両側とも同じフォールバックに落ちるので「一致」に見える。**
skill は `[FONT_MISSING]` の行で **「両側が同じ落ち方をすることを合格として扱うな」**と名指ししている。
このまま上げると **design agent が作る UI は全部セリフ体**になる。

| #      | 論点                                                | 選択肢                                                                                                                                                | 決定       | 根拠      | 戻せるか  |
| ------ | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | --------- | --------- |
| **D8** | 本体と Storybook のフォント差をどう扱うか           | **A** `.storybook/preview.tsx` に Geist を入れて基準器を本体に合わせ、`cfg.extraFonts` で同梱する／ **B** `--font-sans` の自己参照を直して本体・Storybook 共通のフォントスタックにする／ **C** 今回はセリフのまま上げ、記録だけ残す／ **D** A ＋ B | 🟦 **D**（ユーザー判断 2026-08-01） | [DR-0058](../DR/DR-0058-app-only-font-never-reached-the-design-system.md) | 🟦 戻せる |

> 🟨 **D の実装で「合わせる先」が逆だった。**調査すると **`tmp-admin` は `--font-mono` は定義するが `--font-sans` を定義していない**——
> Geist はデザインシステムの語彙ではなく `layout.tsx` 1 枚の選択だった。
> → **基準器を本体に寄せるのではなく、本体の上書きを外して ① Tokens 層の既定へ戻した。**詳細は [DR-0058](../DR/DR-0058-app-only-font-never-reached-the-design-system.md)。

**推奨: D（A ＋ B）。**

- **B は本体側のバグ。**`--font-sans: var(--font-sans)` は自己参照で、
  **本体では `layout.tsx` が同名変数を上書きするので偶然動いているだけ。**
  移送先（PoC）に `layout.tsx` 相当が無ければ同じ症状が出る。→ **手9 に必ず出る借金。**
- **A は基準器を正す。**[DR-0026](../DR/DR-0026-two-css-pipelines-differ.md) は「判定は Storybook 側に固定する」と決めたが、
  🟥 **その Storybook が本体と違うフォントで描いていた。**
  → **手5 の目視レビュー（観点 D タイポ）は、本体と違うフォントで行われていた**ことになる。
- **C を採るなら「セリフで上げた」ことを実行記録に明記する**（成果物は「通った／通らなかった」なので、C も結論にはなる）。

🟨 **これは「実行中に §2 に無い選択肢が出たら §2 へ追記してから進む」規律の 4 例目**
（1 例目は手1 D9/D10、2 例目は手2 D9、3 例目は手5 D7）。
