...
next_action: '完了。Q1〜Q7 すべて回答済み。次は手3（② Components 層）'
---
...
### 追記（2026-07-26・H2B-02 の直後に判明）

**`storybook init` は「描画のみ」を選ばせない。**`--type nextjs --builder vite --yes --no-dev` で実行したところ、
**13 個の devDependency**（`@storybook/addon-vitest` / `vitest` / `playwright` / `@vitest/browser-playwright` /
`@vitest/coverage-v8` / `@chromatic-com/storybook` / `@storybook/addon-mcp` を含む）を入れ、
**Playwright のブラウザバイナリまでダウンロードした**。さらに `eslint.config.mjs` と `.gitignore` を書き換え、
`vitest.config.ts` / `vitest.shims.d.ts` / `src/stories/`（Example 一式）を生成した。
→ D3（描画のみ）が**実行の時点で覆されていた**ので、その場で決めずに本節へ追記する。

| #       | 論点                                       | 選択肢                                                                    | 決定                            | 根拠                                                                                                                                                                                                                                                     | 戻せるか  |
| ------- | ------------------------------------------ | ---------------------------------------------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| **D10** | init が既定で入れた 13 依存の扱い          | **A** 全部残す（D3 を覆す）／ **B** D3 のとおり描画のみに削る ／ **C** 一部残す | **C（描画 + a11y + docs）**     | D3 の根拠（Playwright の移送コスト・ADR-0009 が `proposed`・2 回ルール）は**実測後も成立している**ので test 系は外す。ただし **`@storybook/addon-a11y` は残す**——[DR-0023](../DR/DR-0023-real-conflict-is-touch-target.md) で **touch-min 44px が未決（未決 #11）**になっており、**それを機械で測る手段がこれで手に入る**。axe-core のみでブラウザバイナリを引かない。`addon-docs` は Storybook の表示機能そのもの | 🟦 戻せる |
| **D11** | `@storybook/addon-mcp` を残すか            | 残す ／ **外す**                                                              | **外す（手7 の候補として記録）** | Storybook を MCP 経由で AI に露出する addon。**手7（Claude Design が部品を使うか）の別経路になりうる**が、手6・手7 は `/design-sync` + プレビュー HTML と決まっている（[DR-0018](../DR/DR-0018-design-sync-takes-preview-html.md)）。手2b の目的（判定装置）とは無関係なので外し、**手7 の選択肢として未決に残す** | 🟦 戻せる |
| **D12** | init が壊した `eslint.config.mjs` の扱い   | import を消す ／ **正しく配線する**                                            | **正しく配線する**              | init は `import storybook from 'eslint-plugin-storybook'` を足したが、**config 配列に追加していない**（＝未使用の import が残るだけで、プラグインは効いていない）。story は**自分で書くコード**なので検査対象に入れる。PoC も将来 Storybook を入れる方針（`architecture.md` §3.6）なので、**移送時にそのまま持っていける** | 🟦 戻せる |
| **D13** | init が生成した `src/stories/` の Example  | 残す ／ **消す**                                                              | **消す**                        | `Button.tsx` / `button.css` 等の**独自コンポーネントと生の CSS**が入っており、本 repo の規約（トークン以外の値を書かない）と正面から反する。D5 の置き場（`src/stories/<役割カテゴリ>/`）と**構造も衝突する** | 🟦 戻せる |

### 追記2（2026-07-26・H2B-03 の赤テストで判明）

**`.storybook/**` がどのゲートにも検査されていなかった。**`tsconfig.json` の `include` が
`src/**/*` と `*.config.ts` しか持たないため、`.storybook/main.ts` と `preview.tsx` は
**typecheck の対象外**であり、**lint は `Parsing error: … was not found by the project service` で 2 件落ちる**
（＝検査されずにエラーだけ出る、最も悪い状態）。

| #       | 論点                                | 選択肢                                                                                        | 決定                                    | 根拠                                                                                                                                                                                                                                                | 戻せるか  |
| ------- | ----------------------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| **D14** | `.storybook/**` をゲートに載せるか  | **A** `tsconfig.json` の `include` に足す ／ **B** eslint 側で型情報ルールだけ外す ／ **C** ignore する | **A**                                   | B は既存の「設定ファイルは `disableTypeChecked`」と同じ扱いだが、**`.storybook/*.ts(x)` は設定であると同時に React を書く場所**（`preview.tsx` の decorator）なので型検査を捨てたくない。C は**「対象 0 件で緑」の事故そのもの**（PoC の前例）。A なら typecheck と lint の両方が同時に載る | 🟦 戻せる |
