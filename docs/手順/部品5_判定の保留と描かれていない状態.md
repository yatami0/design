...
next_action: '完了（2026-08-09・ブランチ perth）。マージは人。🟥 D9（ゲート 8 本目）はユーザー判断のまま'
---
...
| **K7** | 🟥 **保留の件数は再現するか**（同じコミットで `a11y-scan` を 2 回打つ） | 🟨 **再現しない**に賭ける——🟨 **着手前実測で 1 件ぶん食い違って見えた**（🟥 **これは実測の誤り**——4 件目は `Avatar/States` の「空」で、**probe で `States` を開いていなかっただけ**。台帳は `story.title` で畳むので story 名が出ない。**予測の根拠が最初から無かった**） | 🟥 **再現しないなら、23 という数字はベースラインとして使えない**——**[handoff](../handoff.md) のベースライン表に「揺れる」と書く必要がある。**★ **再現したら、食い違いの原因は待ち時間（`waitForTimeout(120)`）** |
...
| 2 | `color-contrast` | **4** | 同上（`Avatar` の fallback「小」「中」「大」＋ `States` の「空」） | 🟥 **実効色は `#85858b on #f2f2f7`・比 3.29**＝ **既知の色の組の 6 番目（6 件）そのもの**。**AA 未達** | **(a)** |
...
| **D5** | ★★ 🟥 **(b) の重なり（`nav` が月ラベルを覆う）を直すか**（Q2） | A: **測るだけ**（覆っている範囲を出して OBS/DR に積む） ／ B: 製品層で塞ぐ ／ C: 素材層（`calendar.tsx`）を直す ／ D: 何もしない | 🟦 **A**（実測後に確定・→ [OBS-0020](../OBS/OBS-0020_カレンダーのnavが月ラベルを覆っている.md)） | ★★ **まだ「欠陥か」が決まっていない**——**月ラベルは非対話要素**なので、**覆われても操作は壊れない**可能性がある。🟥 **ただし [DR-0089](../DR/DR-0089-overlays-do-not-cover-their-anchor.md) は「見えているのに触れない」を実測で捕まえた前例**で、**あのときも最初は「幅の問題」に見えていた**。★★★ **測る内容を先に決める**——① **nav の矩形と、その下に入る要素の一覧** ② **日付セルが 1 つでも覆われているか**（**覆われていれば当たり判定の話に繋がる**・[DR-0049](../DR/DR-0049-hit-area-reaches-44px-only-at-default-size.md)） ③ **`pointer-events` が効いているか**。🟥 **B / C は「直す」判断で、①〜③ が出るまで選べない**——**§2 へ追記してから決める** | 🟦 |
...
| **D8** | 🟨 **`collapsed` / mobile を面③ に足すか**（Q5） | A: **面③ に足す**（8 → 10） ／ B: **足さない。別の面として起票する** ／ C: 決めない | 🟦 **B**（実測後に確定・[完成バー §7](../部品の完成バー.md) に書いた） | ★★ **面③ の 8 つは「どの部品にも共通してありうる姿」**（`hover` / `disabled` / `empty` …）だが、**`collapsed` は `Sidebar` にしか無い**——**足すと「その部品だけ対象が在る面」ができ、他の 47 部品では毎回「対象 0 件で緑」になる**。🟥 **それはこの repo が 17 回踏んだ形をバーの定義に埋め込むこと**。★★★ **ただし B にも穴がある**——**別の面にすると、面の一覧が部品ごとに違うことになり、[台帳](../部品の完成バー_台帳.md) の表が 1 次元では書けなくなる**。★ **どちらも代償があるので、実測（`collapsed` を描いて何が測れたか）を見てから決める**。🟥 **[共通コンポーネント思想](../共通コンポーネント思想.md) はユーザーの持ち物なので、面の定義を増やすなら DR に理由を書く** | 🟦 |
| **D9** | 🟥 ★★ **`node tools/opened-overlay-check.mjs` をゲート 8 本目にするか** | A: **する** ／ B: しない ／ C: この回では決めない | 🟥 **ユーザー判断待ち** | 🟦 **材料は揃っている**（**赤テスト両方向済み**・**所要 0.1 秒**・**現在 7/7 緑**）。🟥 **ユーザー判断に上げる理由は部品3 D7 と同じ**——**ゲートの本数は [CLAUDE.md](../../CLAUDE.md) の規約**で、**全工程・全セッションに効く**。★★ **この回で材料が 2 つ増えた**——① 🟥 **Q3 の ③ で「機械で引いた一覧にも穴がある」ことが出た**（`sidebar.tsx` は載らない） ② ★★★ 🟥 **D11 の実測で、この検査自身が「書けば通る」形だった**（[DR-0101](../DR/DR-0101-a-comment-satisfied-the-check-for-absence.md)）。**塞いだうえで上げる** | 🟦 |

> 🆕 **以下の D10・D11 は C5-05 の実測で出た論点。**
> 「実行中に §2 に無い選択肢が出たら、その場で決めずにここへ追記してから進む」に従って、**決める前に書いた。**
>
> **実測（2026-08-09・C5-05）**:
> - 🟥 **`Sidebar.stories.tsx` の JSDoc に `expectOpened('sheet-content')` と書いただけで、静的側の担当が奪われた**
>   （出力の `sheet-content` 行が `Sheet.stories.tsx` → `Sidebar.stories.tsx` に変わった）

| # | 論点 | 選択肢 | 決定 | 根拠 | 戻せるか |
| --- | --- | --- | --- | --- | --- |
| **D10** | ★★ **`aria-controls` の引き換え（D3=B）をどこに置くか** | A: **`expectOpened` に畳み込み、開いた story 全部で自動に測る** ／ B: 専用の helper を作り、該当 2 story だけで呼ぶ ／ C: 置かない | **A** | 🟥 **B は opt-in なので「書かなければ 0 件で緑」を作る**——**まさに [DR-0096](../DR/DR-0096-overlays-that-no-story-opens-are-invisible-to-every-check.md) の穴の形**。★★ **A なら「開いた」と主張した瞬間に自動で掛かる**ので、**次に overlay を足した人が何も書かなくても測られる**。🟨 **`aria-hidden` の中は見ない**——Radix が `hideOthers()` を掛けた側のトリガは支援技術から到達できず、参照の健全性を問う意味が無い（**axe も同じ理由で飛ばしている**）。🟥 **C は却下**——**畳むだけなら「消した」**（D3 の但し書き） | 🟦 |
| **D11** | ★★★ 🟥 **静的側がコメントを主張として数える件をどうするか** | A: **コメントを落としてから走査する** ／ B: AST で解析する ／ C: `play` の中だけを見る ／ D: 直さない（作法で守る） | **A** | ★★★ 🟥 **赤テストで穴が本物と確認できた**——**`Sheet.stories.tsx` の本物の主張を消しても、`Sidebar.stories.tsx` のコメントが残っている限り検査は緑だった**（`exit 0`）。**「無いことを落とす検査」が、書けば通る形だった。**🟥 **D は却下**——**作法で守るのは 17 回踏んだ形**で、**部品4 C4-04 の「詰まったら」欄は既にこの危うさを名指ししていた**（1 回で現実になった）。🟨 **B は正確だが、検査が TypeScript のパーサに依存する**（0.1 秒・依存 0 という取り回しを失う）。🟨 **C は B の一部**。★ **A の限界は書いておく**——**文字列リテラル中の `//` も落ちる**が、**落ちる方向は「主張が減る」側だけ**なので**偽の緑は作らない** | 🟦 |
...
- [x] §0 の Q1〜Q7 に答えが出ている（**答えは実行記録 §部品5 の締めの表**）
- [x] §2 の D1〜D8 が決着し（🆕 **実行中に D10・D11 を追記して決着**）、、根拠が書かれている（🟥 **D5・D8 は実測してから**・**D9 はユーザー判断**）
- [x] 赤テスト K1〜K8 を打った（🟥 **K2 と K7 は必ず両方向 / 2 回**）
- [x] 🟥 **保留 23 件が全件、種類つきで台帳に載っている**（**未分類 0**）
- [x] 🟥 **未分類が出たら落ちることを赤テストで確認している**（K2）
- [x] **`Sidebar` の `collapsed` / mobile がバーを通っている**（K3）
- [x] 🟥 **素材層 29 件の diff が 0 行**（K1）／ **`dependencies` 12 件のまま**（K6）
- [x] 🟥 **OBS-0019 が閉じている**（§5 の見立てが外れたことを本文に残す）
- [x] 実行記録 §部品5 ／ handoff ／ 完成バー ／ 台帳 §5 が更新されている
