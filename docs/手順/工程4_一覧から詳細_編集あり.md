...
next_action: '§2 D1（フォームの土台＝段取り 未決 #4）をユーザーに 1 度問う → P4-01 から実行'
---
...
- **判断**: **A に倒れたら D6 に `form` を足し、D12 を消し、P4-01 の予測 4 を書き直す。**
  🟥 **倒れた場合も「軽いほうから始める」原則は残る**——`field` を先に入れて器を作り、`form` は後から重ねられる。
