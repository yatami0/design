...
### 🆕 D10（実行中に出た論点・H7-06b で追記）

**2 周目で `w-48` が出た。「素材層の設計をやり直すか」という問いになった。**

🟥 **先に前提を訂正した**（[DR-0060](../DR/DR-0060-vocabulary-leaks-from-four-surfaces.md)）——
**px の直当ては起きていない**（生 px / hex / インライン style は 2 周とも 0）。
出たのは**トークン由来の `w-48`** で、**逸脱の出どころは 4 面あり素材層はその 1 つ**にすぎない。
**根因は「素材層に props が無い」より「DS にコントロール幅の語彙が無い」の方が説明力が高い。**

| # | 論点 | 選択肢 | 決定 | 根拠 | 戻せるか |
| --- | --- | --- | --- | --- | --- |
| **D10** | 語彙の逸脱をどう塞ぐか | **A** フィールド幅の semantic 語彙を足す／ **B** 素材層 16 件を製品層ラッパーで包む／ **C** `DataGrid.columns` の cell を props 化／ **D** 潰さず手8 へ持っていく | 🟦 **A のみ**（ユーザー判断 2026-08-02）。D は併用 | [DR-0061](../DR/DR-0061-field-width-vocabulary.md) | 🟦 戻せる |

- **B を今やらない理由**: 🟥 **A が無いとラッパーに渡す値の語彙が決まらない**（順序）。
  🟥 **2 回ルールの証明が 1 回しかない**。**手8 で「素材層由来の赤が何件か」が出てから対象を絞る**
- 🟥 **A の実装で 1 つ穴が見つかった**——**宣言しただけでは CSS に載らない**（[DR-0062](../DR/DR-0062-shipped-vocabulary-needs-safelist.md)）。
  `@source inline()` で safelist した
