...
### 🆕 §2.5 の調査で、実行前に一部の答えが出た（2026-07-26）

| # | 調査で出た答え | 実行時に何を観測するか（差し替え後） |
|---|---|---|
| **Q1** | — | 🆕 **yes/no ではなく「`Box` に何回逃げたか」という数字で答える**（[DR-0032](../DR/DR-0032-layout-primitives-take-props-not-classname.md)）。逃げた箇所と理由を列挙する |
| **Q3** | ✅ **② 製品層のラッパーで解ける**（[DR-0030](../DR/DR-0030-touch-target-provenance-corrected.md)・[DR-0034](../DR/DR-0034-touch-target-visual-32-hit-44.md)） | 🟥 **`addon-a11y` は 44px を測らない**（axe-core 4.12.1 の `target-size` は **24px / AA**）。→ **目視 + DevTools で当たり判定を測る** |
| **Q4** | ✅ **最大 5 件しか減らない**（17 件中 11 件は任意値。[DR-0031](../DR/DR-0031-sidebar-red-is-not-state.md)） | 🟦 **観測不要になった。**D6=A（素材のまま）が確定したので、代わりに**Cmd/Ctrl+B の衝突が起きるか**を手4 で観測する |
| **Q7** | 🟦 **向け替えは技術的に可能**（[DR-0029](../DR/DR-0029-component-token-overridable-outside-layer.md)） | **見た目が実際に変わるか**は H3-08 で目視する。🟨 **バリアント（`data-size=sm`）が保たれているかも見る** |
...
### ✅ D1〜D11 は 2026-07-26 に全件確定した（根拠は DR-0032〜0037）。着手してよい。

| #      | 論点 | 選択肢 | 決定 | 根拠 | 戻せるか |
| ------ | ---- | ------ | ---- | ---- | -------- |
| **D1** | ★ **製品層のスコープ** — 素材（shadcn）と製品（自作）の境界をどこに引くか | **(a) 欠落品だけ**: Layout プリミティブ + Sidebar ラッパーのみ自作。他は `ui/` を直接使う<br>**(b) 欠落品 + 全再輸出**: 18 件を製品層から re-export し、**画面は製品層しか import しない**（窓口を 1 本にする。中身は素通し）<br>**(c) 欠落品 + 既定値ラッパー**: (b) に加え、`Button` の `h-8` → 44px のように**既定値を哲学に合わせて上書きした薄いラッパー**を置く<br>**(d) 全部作り直す**: shadcn を参考実装扱いにする | ✅ **(c)**（ユーザー判断 2026-07-26） | (d) は検証設計が崩れる（手5 の「素材を触らない」が無意味化）ので除外。**(a)↔(c) はコストではなく思想の選択**で、(c) を採ることで **未決 #11（44px）に三択目が生まれ、D5 に四択目が生まれる**。代償として**手5 の観測対象が二層になる**（→ D4） | 🟥 **不可逆に近い。**(c)→(a) は書いたラッパーを全部捨てることになる |
| **D2** | 製品層の置き場と命名 | **A** `src/components/<役割カテゴリ>/`（棚と同じ 9 カテゴリのディレクトリ）<br>**B** `src/components/common/` にフラット<br>**C** `src/components/ui/` に併置 | ✅ **A**（ユーザー判断 2026-07-26） | **C は却下**——`.prettierignore` が `src/components/ui/` を丸ごと除外している（手1 D11）ので、**自作コードが整形ゲートの外に出る**。手2b D5 が story で同じ理由で「分離」を選んだのと同型。**A なら役割 9 カテゴリがコード上に初めて実在する**（＝思想②がディレクトリとして具現する） | 🟦 戻せる |
| **D3** | **画面と story の import 先** | **A** 素材層を直 import（現状）<br>**B** 製品層経由に統一（`@/components/ui/*` を画面と story から禁止する） | 🟨 **B・仮**（ユーザー判断 2026-07-26。**「一旦仮」と明示**＝手4 で画面を組んだときに再評価する） | D1=(c) なので B が必然（ラッパーを迂回されると既定値の上書きが効かない）。**B を採るなら lint ルール（`no-restricted-imports`）で機械的に強制できる**——強制しないと規約が形骸化する（PoC の「守るための構造だけが先に立っている」と同型） | 🟦 戻せる |
| **D4** | ★ **手5 の観測対象の定義** | **A** 素材層のみ（`src/components/ui/**` を触らなければ合格）<br>**B** 素材層 + 製品層（両方触らなければ合格） | ✅ **B′ — 判定基準を層ごとに変える**（ユーザー判断 2026-07-26。→ §2.6-D4・[DR-0033](../DR/DR-0033-step5-criteria-differ-per-layer.md)） | 手5 の問いは「**部品を 1 行も触らずに見た目が変わるか**」。D1=(c) が確定したので、**製品層は値を持つ**＝「トークンで変わるべき層」なのか「手で書き換えてよい層」なのかが割れる。ユーザーの要求は **「色と余白は、共通部品でもアプリ側でも、定義したものしか使わせない」**——これは D4 単独では答えが出ず、**トークン体系そのものの設計**が要る | 🟥 **手5 の判定基準そのもの。手3 で確定させる** |
| **D5** | `exactOptionalPropertyTypes` の赤 1 件（未決 #1） | **A** 設定を弱める<br>**B** 素材を 1 行直す<br>**C** 該当部品（`DropdownMenu`）を使わない<br>**D** 🆕 **製品層のラッパーで型を吸収する** | 🟨 **A・暫定**（ユーザー判断 2026-07-26。**「一旦」＝後で考える**） | 🟥 **借金として明示的に記録する。**A は PoC の tsconfig と食い違うので、**移送時（手9）に赤が復活する**。いま A を採るのは「`build` の赤で手4 が詰まるのを避ける」ための時間稼ぎであり、**解決ではない**。D1=(c) が確定したので **D（ラッパーで吸収）は選択可能なまま残っている**——手4 以降で D を試して A を返上できる（[DR-0014](../DR/DR-0014-exact-optional-property-types-incompatible.md)） | 🟦 戻せる |
| **D6** | Sidebar の扱い（未決 #2） | **A** 素材のまま使う<br>**B** state を hook（`useSidebar`）へ切り出して製品層に置く<br>**C** 今回は使わない | ✅ **A — 素材のまま使う**（ユーザー判断 2026-07-26。→ §2.6-D6・[DR-0035](../DR/DR-0035-sidebar-stays-as-vendor.md)） | 思想は「状態は role の外へ」だが、**shadcn の Sidebar は useState + Context + Provider + cookie + キーボードショートカットを内包**（[表4](../部品カタログ.md)）。**B は素材層のコピーになりがち**で、Q2（素材を触らない）は守れても**素材と製品で同じロジックが二重化する**。Q4 で「lint 赤が減らない」なら B の動機が 1 つ消える | 🟦 戻せる |
| **D7** | ★ **touch-min 44px**（未決 #11） | **A** 哲学を守る（製品層で既定 size を上書き）<br>**B** nova の高密度を取る（哲学側を改訂）<br>**C** 用途で使い分ける（一覧の行内は 32px、主要操作は 44px） | ✅ **B + D + F の合成**（ユーザー判断 2026-07-26。→ §2.6-D7・[DR-0034](../DR/DR-0034-touch-target-visual-32-hit-44.md)） | **D1=(c) が確定したので A は選べる状態になった。**ただしユーザーの指摘どおり、**44px と `h-8` がなぜ対立項として立っているのかの根拠が本 repo に無い**——44px は `tmp-admin` 哲学の引き写し、`h-8` は nova プリセットの既定値で、**どちらも「なぜその値か」を確認していない**。🟥 **選択肢が 3 つしか無いという前提自体を疑う**（当たり判定の拡張・密度モード・ポインタ種別での分岐など）。C は「哲学の下限」を条件つきにするので**思想への変更提案になる**（書き換えはせず DR に書く） | 🟥 哲学に触れる。ユーザー判断 |
| **D8** | `--card-spacing` の向け替え（未決 #12） | **A** `--spacing-inset-md` に向け替える<br>**B** 据え置く | ✅ **A — 向け替える**（レイヤ外の 2 規則。→ §2.6-D8・[DR-0036](../DR/DR-0036-card-spacing-points-to-semantic.md)） | **A が通れば「素材を触らず semantic 層へ載る」実例が 1 件できる**（[DR-0022](../DR/DR-0022-shadcn-has-component-tokens.md)）。これは手5 の希望的観測ではなく**手3 で先に確かめられる**唯一の接続点 | 🟦 戻せる |
| **D9** | TanStack Table（未決 #3） | **A** 手3 で入れる<br>**B** 手4 へ送る<br>**C** 入れない（`Table` の素マークアップで組む） | ✅ **A**（ユーザー判断 2026-07-26） | ⚠ **手順書は B を推していたが、ユーザー判断で A。**根拠を書き換えず、判断が覆った事実として残す。A を採る以上、**手3 の成果物に `DataGrid` を足し、Q1〜Q8 とは別に「新規依存の移送コスト」を実測する**（手2b の Q5 と同型：依存件数・`^` レンジ件数・`pnpm-lock.yaml` の増分）。⚠ **手3 の問い Q1〜Q8 はどれも DataGrid を必要としない**ので、**A の作業は手3 の観測とは独立に走る**——赤が出たときの切り分けのため、**コミットを分ける** | 🟦 戻せる |
| **D10** | Provider の置き場 | **A** 製品層に集約する（`<AppProviders>`）<br>**B** 画面ごとに書く<br>**C** 手4（Patterns/Templates）へ送る | ✅ **A — 製品層に集約**（場所は手3・実装は手4。→ §2.6-D10・[DR-0037](../DR/DR-0037-providers-belong-to-product-layer.md)） | [部品カタログ 表2 の指摘 1・3](../部品カタログ.md)（「部品でないもの」の置き場が無い・`provider` フラグが要る）の**受け皿になりうるのが製品層**。未決 #4（思想への指摘）を 1 つ閉じられる可能性がある。ただし**手2b で `preview.tsx` の decorator に既に書いてある**ので、急がない | 🟦 戻せる |
| **D11** | ★ **Layout プリミティブの API 形** | **A** semantic 名を props で受ける（`<Stack gap="md">` → `var(--spacing-stack-md)`）<br>**B** `className` パススルーのみ（`<Stack className="gap-4">`）<br>**C** 両方許す | ✅ **A — props で semantic 名**（Braid 方式。→ §2.6-D11・[DR-0032](../DR/DR-0032-layout-primitives-take-props-not-classname.md)） | **思想①「数値を直書きさせない枠」の実体はここ。**B は Tailwind の数値ユーティリティ（`gap-4`）を通すので**枠が閉じない**（`no-arbitrary-value` は `gap-4` を検出しない——検出するのは `gap-[13px]` だけ）。**A なら取れる値が有限集合になる**が、**逃げ道が無いので組めない場面が出うる**（→ Q1 の答えそのもの）。C は実質 B | 🟥 **A↔B は書き直しになる。先に決める** |

---

## 2.5 調査ブロック（D4 / D6 / D7 / D8 / D10 / D11 の前段）

> **2026-07-26 のユーザー判断で、6 件が「選ぶ前に分かっていない」ものだと分かった。**
> 選択肢を並べても選べないのは**選択肢の問題ではなく、判断の土台が無い**ということなので、
> **決める前に調査を挟む。**調査は「事実を集めて構造化する」までで、**設計判断はしない**（判断は §2 へ戻す）。

### なぜこの 3 群に割ったか

**同じ根から出ている問いを、同じ調査で解く。**バラバラに 6 回調べると、同じ一次情報を 3 回読むことになる。

| 群 | 束ねる D | 共通の根 | 成果物 |
|---|---|---|---|
| **調査1 デザイントークン体系** | **D4** / D8 / D11 | 「**色と余白は、共通部品でもアプリ側でも、定義したものしか使わせない**」を成立させるには、何を・どの粒度で・どんな名前で定義し、**どう機械で強制するか**。D8（component token の向け替え）も D11（Layout の API 形）も、この体系が決まれば従属的に決まる | ✅ [デザイントークン設計.md](../デザイントークン設計.md)（[DR-0028](../DR/DR-0028-token-frame-is-not-closed.md)・[DR-0029](../DR/DR-0029-component-token-overridable-outside-layer.md)） |
| **調査2 状態と Provider の置き場** | D6 / D10 | 「**部品の外に出すもの**」の置き場。状態（`useState`）と文脈（`Context` / `Provider`）は種類が違うが、**どちらも「役割 9 カテゴリのどこにも属さない」**という同じ穴に落ちている（[部品カタログ 表2 の指摘 1・3](../部品カタログ.md)） | ✅ [状態とProviderの置き場.md](../状態とProviderの置き場.md)（[DR-0031](../DR/DR-0031-sidebar-red-is-not-state.md)） |
| **調査3 タッチターゲットとサイズ密度** | D7 | 44px（`tmp-admin` 哲学）と `h-8`=32px（nova プリセット）が**なぜ対立項として立っているのかの根拠が本 repo に無い**。両方の出どころを一次情報で確認し、**選択肢が本当に 3 つしか無いのかを確かめる** | ✅ [タッチターゲットとサイズ密度.md](../タッチターゲットとサイズ密度.md)（[DR-0030](../DR/DR-0030-touch-target-provenance-corrected.md)） |

### 調査の結果、判断の形が変わったもの（2026-07-26）

| D | 調査前の前提 | 調査後 |
|---|---|---|
| **D11** | 3 案から選ぶ | 🟥 **実質 1 案。**`className` パススルーでは枠が閉じない（`gap-7` / `w-99` が素通り。DR-0028） |
| **D4** | 「素材層のみ」か「素材層＋製品層」か | 🟥 **どちらでもない。**`@theme` は層別に閉じられないので、**素材層は開いたまま・製品層とアプリ層だけを別手段で閉じる**非対称な形になる |
| **D8** | できるかどうか不明 | 🟦 **できる。**レイヤ外の 1 規則で部品を触らず向け替えられる（DR-0029）。判断は「やるか」だけ |
| **D7** | 哲学 vs プリセットの二択 | 🟥 **思想の対立ではなかった。**選択肢は 7 つあり、D（当たり判定だけ拡張）＋ F（`pointer: coarse`）なら**どちらも捨てずに済む**（DR-0030） |
| **D6** | lint 赤の 6 割が動機 | 🟥 **動機が消えた。**赤 17 件のうち 11 件は任意値で、state を出しても減らない（DR-0031） |
| **D10** | 直感で A | 🟦 **言語化できた。**Provider は「部品」ではなく「部品が動くための前提条件」。前提条件は供給する層が持つ |

### 順序と理由

```mermaid
flowchart LR
  I1["調査1 トークン体系<br/>D4 / D8 / D11"]
  I3["調査3 タッチターゲット<br/>D7"]
  I2["調査2 状態と Provider<br/>D6 / D10"]
  H3["手3 の実装<br/>H3-01 以降"]
  ...
  I1 -. "サイズもトークンで表すなら<br/>D7 の選択肢が変わる" .-> I3
  I1 -. "D11 の API 形が決まらないと<br/>製品層の書き方が決まらない" .-> H3
```

1. **調査1 が最初。**D4 は手5 の判定基準そのもので、**これが決まらないと手3 の完了条件が定義できない**。加えて D11（API 形）は製品層の全ファイルの書き方を決めるので、**書き始める前に要る**。
2. **調査3 が次。**調査1 で「サイズもトークンで表すか」が決まると、D7 の選択肢の形が変わる（トークンで解けるなら哲学 vs プリセットの対立自体が消える可能性がある）。
3. **調査2 は最後でよい。**D6（Sidebar）は今回の題材で本当に要るかが未確認、D10（Provider）は手2b の `preview.tsx` に暫定の置き場が既にある＝**どちらも実装を止めない**。

> 🟨 **調査の出力形式は 2 段。**各群とも **md（文章 + Mermaid）が正本**で、**HTML アーティファクトは理解のための使い捨て**（PARADIGM の 3 段の関門）。
> md の無い HTML は残さない。もう一度見たくなったものだけ publish し、**md からリンクを張る**。

---

## 2.6 推奨と根拠（2026-07-26・調査後）

> ✅ **6 件とも 2026-07-26 に確定した**（ユーザー判断）。**根拠は DR-0032〜0037 に台帳化済み。**本節は判断の経緯を残すために置いておく。
> **根拠は「原則を適用したときに実際に起きたこと」に限る**——「一般にはこうすべき」は根拠にしない。

### D11 — Layout プリミティブの API 形 → 🟥 **推奨 A（props で semantic 名を受ける）**

**決め手**: 実測で **6 個書いて lint が止めたのは 3 個**だった（[DR-0028](../DR/DR-0028-token-frame-is-not-closed.md)）。
`className` を通す限り `gap-7`（28px）も `w-99`（396px）も入ってくる。**B・C は「定義したものしか使わせない」を満たせない。**

**採る形**（Braid Design System の作法）:

| 層 | `className` を受けるか |
|---|---|
| `Stack` / `Grid` / `Container` / `Spacer` / `Section` | 🟥 **受けない**。`gap` / `inset` などの props だけ |
| `Box` | 🟦 **例外として受ける**（逃げ道は 1 つに集約する） |

- 取れる値は TypeScript の union（`'sm' | 'md' | 'lg'`）で**有限集合になる**。
- 🟨 **逃げ道を Box 1 つに集約する**のは、逃げ道をゼロにすると Q1（組み切れるか）で詰まったときに手が無くなるため。
  **どれだけ Box に逃げたかが Q1 の答えそのものになる**ので、観測装置としても機能する。

**残るリスク**: 🟨 逃げ道が広すぎると枠が形骸化する。→ **`Box` の使用箇所数を実行記録に残す**（増え続けるなら設計の穴）。

---

### D4 — 手5 の観測対象 → 🟨 **推奨 B′（選択肢の形そのものを変える）**

**決め手**: `@theme` はグローバルで**層別に閉じられない**（[DR-0028](../DR/DR-0028-token-frame-is-not-closed.md) §3）。
「素材層のみ」も「素材層＋製品層」も、**どちらも「触ったか」で判定している**が、
D1=(c) で製品層は**値を持つ層**になったので、**「触ったか」では合否が定義できない。**
...
| **アプリ層** `src/app/**` | 同上 ＋ 素材層を直 import していない | 同上 ＋ `no-restricted-imports` |

つまり **D4 は「触ったか」ではなく「何を書いたか」で判定する。**

**強制の仕組み（実測で動作確認済み・2026-07-26）**:
...
> 🟥 **lint だけでは閉じない。**`cva` や定数経由の文字列は className の外で定義されるので、セレクタでは捕まらない。
> **だから D11=A（props で受ける）が主で、lint は補助**という順序になる。
> ⚠ **`eslint-plugin-tailwindcss` の `no-custom-classname` は使えない。**あれは「Tailwind に存在しないクラス」を見るルールで、
> `p-13` は**正当な Tailwind クラス**なので素通りする（実測）。

**残るリスク**: 🟨 素材層は開いたまま＝ **`p-4` 等 128 箇所は残る**。手5 では「素材層の直書きは追従しない」を**前提として明記**する（[DR-0010](../DR/DR-0010-shadcn-invents-values.md) の延長）。

---

### D7 — touch-min 44px → 🟨 **推奨 B ＋ D ＋ F の合成**（🟥 哲学に触れるので確認必須）

**決め手 3 つ**（[DR-0030](../DR/DR-0030-touch-target-provenance-corrected.md)）:

1. **tmp-admin は 44px を `nav-item` の `min-height` 1 箇所にしか適用していない。**「全コントロールの下限」と読んだのは本 repo。
2. **WCAG の適合ラインは 24px（AA）**で、nova の 32px は満たしている。44px は AAA。
...
| 要素 | 決め |
|---|---|
| **見た目** | 🟦 **nova の 32px を採る**（B）。管理画面の高密度は tmp-admin §4.4（行高 60px・罫線控えめ）とも整合 |
| **当たり判定** | 🟦 **製品層のラッパーで 44px に拡張**（D）。`position: relative` ＋ `::after { inset: -6px }` |
| **適用条件** | 🟦 **`@media (pointer: coarse)` で限定**（F）。検証対象は Redmine の管理画面＝**マウス主体**なので実コストはほぼゼロ |
| **nav-item** | 🟦 **哲学どおり `min-height: 44px` を守る**。tmp-admin が名指しで規定している唯一の箇所なので、ここだけは見た目でも守る |

**捨てる案と理由**:

- **A（見た目ごと 44px）**: nova の高密度が壊れる。しかも**哲学がそこまで要求していない**（決め手 1）。
- **C（用途で使い分け）**: 場面ごとの判断が要り、規約として機械化できない。D なら**一律に効く**。
- **G（高さを semantic token 化）**: 🟨 **却下ではなく先送り。**`h-8` は CSS 変数ではないので、まず製品層で `h-(--control-height)` に置き換える前段が要る。**必要性がまだ 1 度も証明されていない**（2 回ルール）。

**検証手段**（実測で確認済み・2026-07-26）: `addon-a11y` の axe-core 4.12.1 は `target-size` ルールを持つが、
タグは **`wcag22aa` / `wcag258`＝ 24px の AA** であり、**44px は測らない**。
→ **44px の達成は目視 + DevTools で測る**と手順書に書く。「a11y が緑だから 44px」は成り立たない。

**思想への提案**（書き換えない。DR に書く）: `apple.md` の「44×44px 以上」を **当たり判定に対する要求**と読めば、
**見た目 32px と両立する**。適用範囲が書かれていないので、範囲は本 repo が決めてよい。

---

### D8 — `--card-spacing` の向け替え → 🟦 **推奨 A（向け替える）**

**決め手**: **できると実測済み**（[DR-0029](../DR/DR-0029-component-token-overridable-outside-layer.md)）。
`card.tsx` を 1 行も触らず、カスケードレイヤの外に 1 規則書けば勝つ。**やらない理由が無い。**

**採る形**（バリアントを保つ書き分け付き）:

```css
/* tokens.css の @theme の外 */
[data-slot='card'] { --card-spacing: var(--spacing-inset-md); }
[data-slot='card'][data-size='sm'] { --card-spacing: var(--spacing-inset-sm); }
```

**なぜ手3 でやるか**: これが **3 層トークンの「参照は下向き一方通行」を成立させる最初の 1 件**になる。
成立を 1 件でも作っておかないと、手5 は「どこも変わらなかった」で終わる。

**残るリスク**: 🟨 同じ手口を `--sidebar-*` にも広げるかは**手5 で判断**（未決 #18 の再点検と同時）。手3 では Card 1 件に留める。

---

### D6 — Sidebar の扱い → 🟨 **推奨 A（素材のまま使う）**

**決め手 2 つ**（[DR-0031](../DR/DR-0031-sidebar-red-is-not-state.md)）:

1. **切り出す動機だった lint が成立しない。**赤 17 件のうち **11 件は任意値**で、state を出しても減るのは最大 5 件。
2. **React の作法上、shadcn の実装は外れていない。**サイドバーの開閉は layout state であり、Context で配るのが定石。

**採る形**:

| やること | やらないこと |
|---|---|
| 🟦 製品層 `Navigation/Sidebar.tsx` に**薄いラッパー**を置く（D1=(c) の一貫として） | 🟥 `useState` / cookie / キーボードショートカットを**書き写さない** |
| 🟦 `SidebarProvider` は製品層の `AppProviders` が持つ（D10） | 🟥 `useSidebar()` を**自作しない**（shadcn が同名で提供済み） |
| 🟦 nav-item の `min-height: 44px` はラッパーで当てる（D7） | |

**捨てる案と理由**:

- **B（state を切り出す）**: 素材層のコピーになり、**同じロジックが二重化して手5 で両方を見る羽目になる**。
  副作用（cookie・Cmd/Ctrl+B）を差し替えたいという要求は**まだ 1 度も出ていない**（2 回ルール）。
- **C（使わない）**: tmp-admin §4.2 が**サイドバー 2 セクションを規定**しており、チケット一覧の骨格に含まれる。使わない選択は無い。

**観測として残すこと**: 🟨 **Cmd/Ctrl+B の占有**は、別用途で使いたくなったとき初めて表面化する。
**手4 で画面を組むときに衝突したら、その時が B に切り替える 1 回目の証明**になる。→ 実行記録に観測項目として書く。

---

### D10 — Provider の置き場 → 🟦 **推奨 A（製品層に集約）／ただし実装は手4 でよい**

**決め手**: **Provider は「部品」ではなく「部品が動くための前提条件」**であり、前提条件は**供給する層が面倒を見る**。
D1=(c) で製品層が Tooltip をラップすると決めた以上、**Provider まで面倒を見ないとラッパーの仕事が終わっていない。**
...
- **B（画面ごとに書く）を捨てる理由**: D3=B（画面は製品層しか import しない）と噛み合わない——
  **素材層の名前（`TooltipProvider`）がアプリ層に漏れる。**
- **C（手4 へ送る）との違い**: 🟨 **場所だけ手3 で決め、中身は手4 で埋める**のが実質 A と C の折衷。
  Storybook 側は `preview.tsx` の decorator で既に動いており、**アプリ層で画面を組むのは手4** なので急がない。
...
**D11=A ＋ D4=B′ を採ると、Q1 の答え方が変わる。**
これまで Q1 は「semantic 語彙だけで組み切れるか」という yes/no だったが、
**逃げ道を `Box` 1 つに集約する**ので、**「Box に何回逃げたか」という数字で答えが出る**ようになる。
→ §0 の Q1 に**観測方法として追記する**（実行時）。
