...
| **D4** | ★ **手5 の観測対象の定義** | **A** 素材層のみ（`src/components/ui/**` を触らなければ合格）<br>**B** 素材層 + 製品層（両方触らなければ合格） | 🔍 **調査待ち → [調査1](#調査1-デザイントークン体系)** | 手5 の問いは「**部品を 1 行も触らずに見た目が変わるか**」。D1=(c) が確定したので、**製品層は値を持つ**＝「トークンで変わるべき層」なのか「手で書き換えてよい層」なのかが割れる。ユーザーの要求は **「色と余白は、共通部品でもアプリ側でも、定義したものしか使わせない」**——これは D4 単独では答えが出ず、**トークン体系そのものの設計**が要る | 🟥 **手5 の判定基準そのもの。手3 で確定させる** |
...
| **D6** | Sidebar の扱い（未決 #2） | **A** 素材のまま使う<br>**B** state を hook（`useSidebar`）へ切り出して製品層に置く<br>**C** 今回は使わない | 🔍 **調査待ち → [調査2](#調査2-状態と-provider-の置き場)** | 思想は「状態は role の外へ」だが、**shadcn の Sidebar は useState + Context + Provider + cookie + キーボードショートカットを内包**（[表4](../部品カタログ.md)）。**B は素材層のコピーになりがち**で、Q2（素材を触らない）は守れても**素材と製品で同じロジックが二重化する**。Q4 で「lint 赤が減らない」なら B の動機が 1 つ消える | 🟦 戻せる |
| **D7** | ★ **touch-min 44px**（未決 #11） | **A** 哲学を守る（製品層で既定 size を上書き）<br>**B** nova の高密度を取る（哲学側を改訂）<br>**C** 用途で使い分ける（一覧の行内は 32px、主要操作は 44px） | 🔍 **調査待ち → [調査3](#調査3-タッチターゲットとサイズ密度)** | **D1=(c) が確定したので A は選べる状態になった。**ただしユーザーの指摘どおり、**44px と `h-8` がなぜ対立項として立っているのかの根拠が本 repo に無い**——44px は `tmp-admin` 哲学の引き写し、`h-8` は nova プリセットの既定値で、**どちらも「なぜその値か」を確認していない**。🟥 **選択肢が 3 つしか無いという前提自体を疑う**（当たり判定の拡張・密度モード・ポインタ種別での分岐など）。C は「哲学の下限」を条件つきにするので**思想への変更提案になる**（書き換えはせず DR に書く） | 🟥 哲学に触れる。ユーザー判断 |
| **D8** | `--card-spacing` の向け替え（未決 #12） | **A** `--spacing-inset-md` に向け替える<br>**B** 据え置く | 🔍 **調査待ち → [調査1](#調査1-デザイントークン体系)**（何を選ぶかの前に**何の話なのかの解説が要る**） | **A が通れば「素材を触らず semantic 層へ載る」実例が 1 件できる**（[DR-0022](../DR/DR-0022-shadcn-has-component-tokens.md)）。これは手5 の希望的観測ではなく**手3 で先に確かめられる**唯一の接続点 | 🟦 戻せる |
...
| **D10** | Provider の置き場 | **A** 製品層に集約する（`<AppProviders>`）<br>**B** 画面ごとに書く<br>**C** 手4（Patterns/Templates）へ送る | 🔍 **調査待ち → [調査2](#調査2-状態と-provider-の置き場)**（ユーザーの直感は A。**根拠の言語化が調査の目的**） | [部品カタログ 表2 の指摘 1・3](../部品カタログ.md)（「部品でないもの」の置き場が無い・`provider` フラグが要る）の**受け皿になりうるのが製品層**。未決 #4（思想への指摘）を 1 つ閉じられる可能性がある。ただし**手2b で `preview.tsx` の decorator に既に書いてある**ので、急がない | 🟦 戻せる |
| **D11** | ★ **Layout プリミティブの API 形** | **A** semantic 名を props で受ける（`<Stack gap="md">` → `var(--spacing-stack-md)`）<br>**B** `className` パススルーのみ（`<Stack className="gap-4">`）<br>**C** 両方許す | 🔍 **調査待ち → [調査1](#調査1-デザイントークン体系)** | **思想①「数値を直書きさせない枠」の実体はここ。**B は Tailwind の数値ユーティリティ（`gap-4`）を通すので**枠が閉じない**（`no-arbitrary-value` は `gap-4` を検出しない——検出するのは `gap-[13px]` だけ）。**A なら取れる値が有限集合になる**が、**逃げ道が無いので組めない場面が出うる**（→ Q1 の答えそのもの）。C は実質 B | 🟥 **A↔B は書き直しになる。先に決める** |
...
| 群 | 束ねる D | 共通の根 | 成果物 |
|---|---|---|---|
| **調査1 デザイントークン体系** | **D4** / D8 / D11 | 「**色と余白は、共通部品でもアプリ側でも、定義したものしか使わせない**」を成立させるには、何を・どの粒度で・どんな名前で定義し、**どう機械で強制するか**。D8（component token の向け替え）も D11（Layout の API 形）も、この体系が決まれば従属的に決まる | `docs/デザイントークン設計.md` + HTML ① |
| **調査2 状態と Provider の置き場** | D6 / D10 | 「**部品の外に出すもの**」の置き場。状態（`useState`）と文脈（`Context` / `Provider`）は種類が違うが、**どちらも「役割 9 カテゴリのどこにも属さない」**という同じ穴に落ちている（[部品カタログ 表2 の指摘 1・3](../部品カタログ.md)） | `docs/状態とProviderの置き場.md` + HTML ② |
| **調査3 タッチターゲットとサイズ密度** | D7 | 44px（`tmp-admin` 哲学）と `h-8`=32px（nova プリセット）が**なぜ対立項として立っているのかの根拠が本 repo に無い**。両方の出どころを一次情報で確認し、**選択肢が本当に 3 つしか無いのかを確かめる** | `docs/タッチターゲットとサイズ密度.md` + HTML ③ |
