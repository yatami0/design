...
next_action: '§2 の D1〜D9 を確認する（🟥 D4 と D7 はユーザー判断待ち）。決着したら C3-01 から実行する'
---
...
> 🟥 **D4 と D7 はユーザー判断待ち**——D4 は**出荷物の依存**（工場の思想に直結）、D7 は**ゲートの本数を変える**（全工程に効く）。
...
- [ ] §0 の Q1〜Q6 に答えが出ている（🟥 **「測れなかった」も答え。理由を書く**）
- [ ] §2 の D1〜D9 が決着し、根拠が書かれている（🟥 **D4・D7 はユーザー判断**）
- [ ] 赤テスト K1〜K7 を打った（🟥 **K7 は D7=A のときだけ**）
- [ ] ★ **`DatePicker` と 3 件が完成バーを通っている**（🟨 **一発で通らなかったなら、落ちた項目を全部書く**）
- [ ] 🟥 **素材層 29 件の diff が 0 行**（K1）
- [ ] 🟥 **`dependencies` が 0 増**（K3・12 件のまま）
- [ ] ★ **`PeriodSelect` が `DatePicker` を使う側に回っている**（D6=A）か、**回れなかった理由が書かれている**
- [ ] 🟥 **[台帳 §4](../部品の完成バー_台帳.md)「バーが見ていない出荷物」が 0 件**になっている
- [ ] Q4 の DR が起票されている ／ Q5 の実測が [OBS-0008](../OBS/OBS-0008_当たり判定44pxをどう扱うか.md) に入っている
- [ ] 実行記録 §部品3 ／ handoff ／ 台帳 ／ 部品カタログが更新されている
